...
// Original: public/chunkfile.cpp
// Functions: 32
// ============================================================

#include "public\chunkfile.h"

//------------------------------------------------------------------------------
// Address: 0x1000C820
// Name: public: CChunkHandlerMap::~CChunkHandlerMap(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkHandlerMap::~CChunkHandlerMap(CChunkHandlerMap *this)
{
  ChunkHandlerInfoNode_t *m_pHandlers; // esi
  ChunkHandlerInfoNode_t *v2; // [esp-4h] [ebp-8h]

  m_pHandlers = this->m_pHandlers;
  while ( m_pHandlers != nullptr )
  {
    v2 = m_pHandlers;
    m_pHandlers = m_pHandlers->pNext;
...
//------------------------------------------------------------------------------
// Address: 0x1000C840
// Name: public: void CChunkHandlerMap::AddHandler(char const __near *,enum ChunkFileResult_t (*)(class CChunkFile __near *,void __near *),void __near *)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkHandlerMap::AddHandler(
        CChunkHandlerMap *this,
        const char *pszChunkName,
        ChunkFileResult_t (__cdecl *pfnHandler)(CChunkFile *, void *),
        void *pData)
{
  ChunkHandlerInfoNode_t *v5; // esi
  ChunkHandlerInfoNode_t *m_pHandlers; // eax

  v5 = (ChunkHandlerInfoNode_t *)operator new(nSize: 0x5Cu);
  V_strncpy(pDest: v5->Handler.szChunkName, pSrc: pszChunkName, maxLen: 80);
  v5->Handler.pfnHandler = pfnHandler;
  v5->Handler.pData = pData;
  v5->pNext = nullptr;
  m_pHandlers = this->m_pHandlers;
  if ( this->m_pHandlers != nullptr )
  {
    for ( ; m_pHandlers->pNext != nullptr; m_pHandlers = m_pHandlers->pNext )
      ;
    m_pHandlers->pNext = v5;
  }
  else
  {
    this->m_pHandlers = v5;
...
//------------------------------------------------------------------------------
// Address: 0x1000C8C0
// Name: public: enum ChunkFileResult_t (*CChunkHandlerMap::GetHandler(char const __near *,void __near * __near *))(class CChunkFile __near *,void __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t (__cdecl *__thiscall CChunkHandlerMap::GetHandler(
        CChunkHandlerMap *this,
        const char *pszChunkName,
        void **ppData))(CChunkFile *, void *)
{
  ChunkHandlerInfoNode_t *m_pHandlers; // esi
  ...
  if ( this->m_pHandlers == nullptr )
    return nullptr;
  while ( _V_stricmp(s1: m_pHandlers->Handler.szChunkName, s2: pszChunkName) != 0 )
  {
    m_pHandlers = m_pHandlers->pNext;
    if ( m_pHandlers == nullptr )
      return nullptr;
  }
  *ppData = m_pHandlers->Handler.pData;
  return m_pHandlers->Handler.pfnHandler;
}

//------------------------------------------------------------------------------
// Address: 0x1000C900
// Name: public: TokenReader::~TokenReader(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall TokenReader::~TokenReader(TokenReader *this)
{
  CUtlStreamBuffer::~CUtlStreamBuffer(this: &this->m_file);
}

//------------------------------------------------------------------------------
// Address: 0x1000C940
// Name: public: CChunkFile::~CChunkFile(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkFile::~CChunkFile(CChunkFile *this)
{
  if ( this->m_hFile != nullptr )
    fclose(stream: this->m_hFile);
  CUtlStreamBuffer::~CUtlStreamBuffer(this: &this->m_TokenReader.m_file);
}

//------------------------------------------------------------------------------
// Address: 0x1000C9A0
// Name: public: enum ChunkFileResult_t CChunkFile::Close(void)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::Close(CChunkFile *this)
{
  if ( this->m_hFile != nullptr )
  {
    fclose(stream: this->m_hFile);
...
//------------------------------------------------------------------------------
// Address: 0x1000C9D0
// Name: public: char const __near * CChunkFile::GetErrorText(enum ChunkFileResult_t)
// Source: json
//------------------------------------------------------------------------------
const char *__thiscall CChunkFile::GetErrorText(CChunkFile *this, ChunkFileResult_t eResult)
{
  ...
  switch ( eResult )
  {
    case ChunkFile_OpenFail:
      ...
      V_snprintf(pDest: szError, maxLen: 1024, pFormat: "%s", v5);
      result = TokenReader::Error(this: &this->m_TokenReader, error: szError);
      break;
    case ChunkFile_UnexpectedEOF:
      V_strncpy(pDest: szError, pSrc: "unexpected end of file", maxLen: 1024);
      result = TokenReader::Error(this: &this->m_TokenReader, error: szError);
      break;
    case ChunkFile_UnexpectedSymbol:
      V_snprintf(pDest: szError, maxLen: 1024, pFormat: "unexpected symbol '%s'", this->m_szErrorToken);
      goto LABEL_7;
    case ChunkFile_StringTooLong:
      V_strncpy(pDest: szError, pSrc: "unterminated string or string too long", maxLen: 1024);
      result = TokenReader::Error(this: &this->m_TokenReader, error: szError);
      break;
    default:
      V_snprintf(pDest: szError, maxLen: 1024, pFormat: "error %d", eResult);
LABEL_7:
      result = TokenReader::Error(this: &this->m_TokenReader, error: szError);
      break;
  }
...
//------------------------------------------------------------------------------
// Address: 0x1000CAD0
// Name: public: enum ChunkFileResult_t CChunkFile::Open(char const __near *,enum ChunkFileOpenMode_t)
// Source: json
//------------------------------------------------------------------------------
int __thiscall CChunkFile::Open(CChunkFile *this, const char *pszFileName, ChunkFileOpenMode_t eMode)
{
  _iobuf *v5; // eax

  if ( eMode == ChunkFile_Read )
  {
    if ( !TokenReader::Open(this: &this->m_TokenReader, pszFilename: pszFileName) )
      return 2;
LABEL_7:
    this->m_nCurrentDepth = 0;
    return 0;
  }
  if ( eMode == ChunkFile_Write )
  {
    v5 = fopen(file: pszFileName, mode: "wb");
    this->m_hFile = v5;
    if ( v5 == nullptr )
      return 2;
    goto LABEL_7;
...
//------------------------------------------------------------------------------
// Address: 0x1000CB30
// Name: public: void CChunkFile::PopHandlers(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkFile::PopHandlers(CChunkFile *this)
{
  int m_nHandlerStackDepth; // eax

  m_nHandlerStackDepth = this->m_nHandlerStackDepth;
  if ( m_nHandlerStackDepth > 0 )
    this->m_nHandlerStackDepth = m_nHandlerStackDepth - 1;
}

//------------------------------------------------------------------------------
// Address: 0x1000CB50
// Name: public: void CChunkFile::PushHandlers(class CChunkHandlerMap __near *)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkFile::PushHandlers(CChunkFile *this, CChunkHandlerMap *pHandlerMap)
{
  int m_nHandlerStackDepth; // eax

  m_nHandlerStackDepth = this->m_nHandlerStackDepth;
  if ( m_nHandlerStackDepth < 80 )
  {
    this->m_HandlerStack[m_nHandlerStackDepth] = pHandlerMap;
    ++this->m_nHandlerStackDepth;
  }
}

//------------------------------------------------------------------------------
// Address: 0x1000CB80
// Name: public: enum ChunkFileResult_t CChunkFile::ReadNext(char __near *,char __near *,int,enum ChunkType_t __near &)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::ReadNext(
        CChunkFile *this,
        char *szName,
        char *szValue,
        int nValueSize,
        ChunkType_t *eChunkType)
{
  ...
  char szNext[1024]; // [esp+8h] [ebp-400h] BYREF

  Token = TokenReader::NextToken(this: &this->m_TokenReader, pszStore: szName, nSize: 1024);
  if ( Token == TOKENEOF )
    return (this->m_nCurrentDepth != 0) + 4;
  switch ( Token )
  {
    case TOKENSTRINGTOOLONG:
$LN6_4:
      result = ChunkFile_StringTooLong;
      break;
    case OPERATOR:
$LN12:
      if ( _V_stricmp(s1: szName, s2: "}") != 0 )
      {
        V_strncpy(pDest: this->m_szErrorToken, pSrc: szName, maxLen: 80);
        result = ChunkFile_UnexpectedSymbol;
      }
      else
      {
        --this->m_nCurrentDepth;
        result = ChunkFile_EndOfChunk;
      }
      break;
    case STRING:
    case IDENT:
      switch ( TokenReader::NextToken(this: &this->m_TokenReader, pszStore: szNext, nSize: 1024) )
      {
        case TOKENSTRINGTOOLONG:
          goto $LN6_4;
        case TOKENEOF:
          result = ChunkFile_UnexpectedEOF;
          break;
        case OPERATOR:
          if ( _V_stricmp(s1: szNext, s2: "{") != 0 )
          {
            V_strncpy(pDest: this->m_szErrorToken, pSrc: szNext, maxLen: 80);
            result = ChunkFile_UnexpectedSymbol;
          }
          else
          {
        ...
        case STRING:
        case IDENT:
          V_strncpy(pDest: szValue, pSrc: szNext, maxLen: nValueSize);
...
//------------------------------------------------------------------------------
// Address: 0x1000CD20
// Name: public: static bool CChunkFile::ReadKeyValueBool(char const __near *,bool __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl CChunkFile::ReadKeyValueBool(const char *pszValue, bool *bBool)
{
  *bBool = atoi(nptr: pszValue) > 0;
  return 1;
}

//------------------------------------------------------------------------------
// Address: 0x1000CD40
// Name: public: static bool CChunkFile::ReadKeyValueFloat(char const __near *,float __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl CChunkFile::ReadKeyValueFloat(const char *pszValue, float *flFloat)
{
  *flFloat = atof(nptr: pszValue);
  return 1;
}

//------------------------------------------------------------------------------
// Address: 0x1000CD60
// Name: public: static bool CChunkFile::ReadKeyValueInt(char const __near *,int __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl CChunkFile::ReadKeyValueInt(const char *pszValue, int *nInt)
{
  *nInt = atoi(nptr: pszValue);
  return 1;
}

//------------------------------------------------------------------------------
// Address: 0x1000CD80
// Name: public: static bool CChunkFile::ReadKeyValueColor(char const __near *,unsigned char __near &,unsigned char __near &,unsigned char __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl CChunkFile::ReadKeyValueColor(
        const char *pszValue,
        ...
        unsigned __int8 *chBlue)
{
  ...
  int b; // [esp+0h] [ebp-8h] BYREF
  int g; // [esp+4h] [ebp-4h] BYREF

  if ( pszValue == nullptr || sscanf(string: pszValue, format: "%d %d %d", &pszValue, &g, &b) != 3 )
    return 0;
  v4 = chGreen;
  *chRed = (unsigned __int8)pszValue;
  v5 = chBlue;
  *v4 = g;
  *v5 = b;
  return 1;
...
//------------------------------------------------------------------------------
// Address: 0x1000CDD0
// Name: public: static bool CChunkFile::ReadKeyValuePoint(char const __near *,class Vector __near &)
// Source: json
//------------------------------------------------------------------------------
bool __cdecl CChunkFile::ReadKeyValuePoint(const char *pszValue, Vector *Point)
{
  return pszValue != nullptr && sscanf(string: pszValue, format: "(%f %f %f)", Point, &Point->y, &Point->z) == 3;
}

//------------------------------------------------------------------------------
// Address: 0x1000CE10
// Name: public: static bool CChunkFile::ReadKeyValueVector2(char const __near *,class Vector2D __near &)
// Source: json
//------------------------------------------------------------------------------
bool __cdecl CChunkFile::ReadKeyValueVector2(const char *pszValue, Vector2D *vec)
{
  return pszValue != nullptr && sscanf(string: pszValue, format: "[%f %f]", vec, &vec->y) == 2;
}

//------------------------------------------------------------------------------
// Address: 0x1000CE40
// Name: public: static bool CChunkFile::ReadKeyValueVector3(char const __near *,class Vector __near &)
// Source: json
//------------------------------------------------------------------------------
bool __cdecl CChunkFile::ReadKeyValueVector3(const char *pszValue, Vector *vec)
{
  return pszValue != nullptr && sscanf(string: pszValue, format: "[%f %f %f]", vec, &vec->y, &vec->z) == 3;
}

//------------------------------------------------------------------------------
// Address: 0x1000CE80
// Name: public: enum ChunkFileResult_t CChunkFile::WriteLine(char const __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteLine(CChunkFile *this, const char *pszLine)
{
  signed int m_nCurrentDepth; // eax
  ChunkFileResult_t result; // eax
  ...
  if ( pszLine != nullptr )
  {
    m_nCurrentDepth = this->m_nCurrentDepth;
    if ( m_nCurrentDepth > 0
      && fwrite(buffer: this->m_szIndent, size: 1u, count: m_nCurrentDepth, stream: this->m_hFile) != this->m_nCurrentDepth )
    {
      return ChunkFile_Fail;
    }
    v5 = strlen(pszLine);
    if ( fwrite(buffer: pszLine, size: 1u, count: v5, stream: this->m_hFile) != v5
      || fwrite(buffer: "\r\n", size: 1u, count: 2u, stream: this->m_hFile) != 2 )
    {
      return ChunkFile_Fail;
    }
  }
...
// Source: json
//------------------------------------------------------------------------------
CChunkFile *__thiscall CChunkFile::CChunkFile(CChunkFile *this)
{
  TokenReader::TokenReader(this: &this->m_TokenReader);
  this->m_hFile = nullptr;
  this->m_nCurrentDepth = 0;
  this->m_szIndent[0] = 0;
  this->m_nHandlerStackDepth = 0;
  this->m_DefaultChunkHandler = nullptr;
  return this;
}

//------------------------------------------------------------------------------
// Address: 0x1000CF80
// Name: public: enum ChunkFileResult_t CChunkFile::BeginChunk(char const __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::BeginChunk(CChunkFile *this, const char *pszChunkName)
{
  unsigned __int8 *m_szIndent; // edi
  ChunkFileResult_t result; // eax
  bool v5; // sf
  signed int m_nCurrentDepth; // esi
  char szBuf[1024]; // [esp+Ch] [ebp-400h] BYREF

  m_szIndent = (unsigned __int8 *)this->m_szIndent;
  V_snprintf(pDest: szBuf, maxLen: 1024, pFormat: "%s\r\n%s{", pszChunkName, this->m_szIndent);
  result = CChunkFile::WriteLine(this, pszLine: szBuf);
  if ( result == ChunkFile_Ok )
  {
    v5 = ++this->m_nCurrentDepth < 0;
    m_nCurrentDepth = this->m_nCurrentDepth;
    if ( !v5 )
    {
      if ( m_nCurrentDepth > 0 )
        memset(dst: m_szIndent, value: 9u, count: m_nCurrentDepth);
      m_szIndent[m_nCurrentDepth] = 0;
      return ChunkFile_Ok;
    }
  }
  return result;
...
//------------------------------------------------------------------------------
// Address: 0x1000D000
// Name: public: enum ChunkFileResult_t CChunkFile::EndChunk(void)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::EndChunk(CChunkFile *this)
{
  int m_nCurrentDepth; // eax
  int v3; // esi
  char *m_szIndent; // ebx
  ...
  if ( m_nCurrentDepth > 0 )
  {
    v3 = m_nCurrentDepth - 1;
    this->m_nCurrentDepth = m_nCurrentDepth - 1;
    m_szIndent = this->m_szIndent;
    if ( m_nCurrentDepth - 1 > 0 )
      memset(dst: (unsigned __int8 *)this->m_szIndent, value: 9u, count: m_nCurrentDepth - 1);
    m_szIndent[v3] = 0;
  }
  CChunkFile::WriteLine(this, pszLine: "}");
  return ChunkFile_Ok;
}

//------------------------------------------------------------------------------
// Address: 0x1000D050
// Name: public: enum ChunkFileResult_t CChunkFile::HandleChunk(char const __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::HandleChunk(CChunkFile *this, char *szChunkName)
{
  ChunkFileResult_t (__cdecl *m_DefaultChunkHandler)(CChunkFile *, void *, const char *); // eax
  const char *v4; // edi
  ChunkFileResult_t result; // eax
  int m_nHandlerStackDepth; // eax
  ChunkFileResult_t (__cdecl *Handler)(CChunkFile *, void *); // eax
  int v8; // edi
  ChunkFileResult_t i; // eax
  char szKey[1024]; // [esp+8h] [ebp-800h] BYREF
  char szValue[1024]; // [esp+408h] [ebp-400h] BYREF

  m_DefaultChunkHandler = this->m_DefaultChunkHandler;
  v4 = szChunkName;
  if ( m_DefaultChunkHandler == nullptr
    || (result = m_DefaultChunkHandler(a1: this, a2: this->m_pDefaultChunkHandlerData, a3: szChunkName)) != ChunkFile_Ok )
  {
    m_nHandlerStackDepth = this->m_nHandlerStackDepth;
    if ( m_nHandlerStackDepth <= 0 )
      return ChunkFile_Ok;
    Handler = CChunkHandlerMap::GetHandler(
                this: *((CChunkHandlerMap **)&this->m_pDefaultChunkHandlerData + m_nHandlerStackDepth),
                pszChunkName: v4,
                ppData: (void **)&szChunkName);
    if ( Handler == nullptr )
    {
      v8 = 1;
      while ( 1 )
      {
        for ( i = CChunkFile::ReadNext(
                    this,
                    szName: szKey,
                    szValue,
                    nValueSize: 1024,
                    eChunkType: (ChunkType_t *)&szChunkName);
              i == ChunkFile_Ok;
              i = CChunkFile::ReadNext(
                    this,
                    szName: szKey,
                    szValue,
                    nValueSize: 1024,
                    eChunkType: (ChunkType_t *)&szChunkName) )
        {
          if ( szChunkName == (char *)1 )
            ++v8;
        }
        if ( i == ChunkFile_EndOfChunk )
        {
          i = ChunkFile_Ok;
          --v8;
        }
        else if ( i == ChunkFile_EOF )
        {
    ...
    result = Handler(a1: this, a2: szChunkName);
    if ( result == ChunkFile_Ok )
      return ChunkFile_Ok;
  }
  return result;
...
//------------------------------------------------------------------------------
// Address: 0x1000D140
// Name: public: enum ChunkFileResult_t CChunkFile::ReadChunk(enum ChunkFileResult_t (*)(char const __near *,char const __near *,void __near *),void __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::ReadChunk(
        CChunkFile *this,
        ChunkFileResult_t (__cdecl *pfnKeyHandler)(const char *, const char *, void *),
        void *pData)
{
  void *v3; // ebx
  ChunkFileResult_t result; // eax
  char szValue[1024]; // [esp+Ch] [ebp-800h] BYREF
  char szName[1024]; // [esp+40Ch] [ebp-400h] BYREF

  v3 = pData;
  while ( 1 )
  {
    result = CChunkFile::ReadNext(this, szName, szValue, nValueSize: 1024, eChunkType: (ChunkType_t *)&pData);
    if ( result != ChunkFile_Ok )
      break;
    if ( pData == (void *)1 )
    {
      result = CChunkFile::HandleChunk(this, szChunkName: szName);
LABEL_8:
      if ( result != ChunkFile_Ok )
        break;
    }
    else if ( pData == nullptr && pfnKeyHandler != nullptr )
    {
      result = pfnKeyHandler(a1: szName, a2: szValue, a3: v3);
      goto LABEL_8;
    }
  }
  if ( result == ChunkFile_EndOfChunk )
    return ChunkFile_Ok;
  return result;
}
...
//------------------------------------------------------------------------------
// Address: 0x1000D1C0
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValue(char const __near *,char const __near *)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValue(CChunkFile *this, const char *pszKey, const char *pszValue)
{
  char szTemp[1024]; // [esp+4h] [ebp-400h] BYREF

  if ( pszKey == nullptr || pszValue == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: szTemp, maxLen: 1024, pFormat: "\"%s\" \"%s\"", pszKey, pszValue);
  return CChunkFile::WriteLine(this, pszLine: szTemp);
}

//------------------------------------------------------------------------------
// Address: 0x1000D220
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueBool(char const __near *,bool)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueBool(CChunkFile *this, const char *pszKey, bool bValue)
{
  char szBuf[1024]; // [esp+4h] [ebp-400h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: szBuf, maxLen: 1024, pFormat: "\"%s\" \"%d\"", pszKey, bValue);
  return CChunkFile::WriteLine(this, pszLine: szBuf);
}

//------------------------------------------------------------------------------
// Address: 0x1000D270
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueInt(char const __near *,int)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueInt(CChunkFile *this, const char *pszKey, int nValue)
{
  char szBuf[1024]; // [esp+4h] [ebp-400h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: szBuf, maxLen: 1024, pFormat: "\"%s\" \"%d\"", pszKey, nValue);
  return CChunkFile::WriteLine(this, pszLine: szBuf);
}

//------------------------------------------------------------------------------
// Address: 0x1000D2C0
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueFloat(char const __near *,float)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueFloat(CChunkFile *this, const char *pszKey, float fValue)
{
  char pDest[12]; // [esp+8h] [ebp-404h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: &pDest[4], maxLen: 1024, pFormat: "\"%s\" \"%g\"", pszKey, fValue);
  return CChunkFile::WriteLine(this, pszLine: &pDest[4]);
}

//------------------------------------------------------------------------------
// Address: 0x1000D320
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueColor(char const __near *,unsigned char,unsigned char,unsigned char)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueColor(
        CChunkFile *this,
        const char *pszKey,
        unsigned __int8 r,
        unsigned __int8 g,
        unsigned __int8 b)
{
  ...
  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: szBuf, maxLen: 1024, pFormat: "\"%s\" \"%d %d %d\"", pszKey, r, g, b);
  return CChunkFile::WriteLine(this, pszLine: szBuf);
}

//------------------------------------------------------------------------------
// Address: 0x1000D380
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValuePoint(char const __near *,class Vector const __near &)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValuePoint(CChunkFile *this, const char *pszKey, const Vector *Point)
{
  char pDest[28]; // [esp+18h] [ebp-404h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: &pDest[4], maxLen: 1024, pFormat: "\"%s\" \"(%g %g %g)\"", pszKey, Point->x, Point->y, Point->z);
  return CChunkFile::WriteLine(this, pszLine: &pDest[4]);
}

//------------------------------------------------------------------------------
// Address: 0x1000D400
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueVector2(char const __near *,class Vector2D const __near &)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueVector2(
        CChunkFile *this,
        const char *pszKey,
        const Vector2D *vec)
{
  char pDest[20]; // [esp+10h] [ebp-404h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: &pDest[4], maxLen: 1024, pFormat: "\"%s\" \"[%g %g]\"", pszKey, vec->x, vec->y);
  return CChunkFile::WriteLine(this, pszLine: &pDest[4]);
}

//------------------------------------------------------------------------------
// Address: 0x1000D470
// Name: public: enum ChunkFileResult_t CChunkFile::WriteKeyValueVector3(char const __near *,class Vector const __near &)
// Source: json
//------------------------------------------------------------------------------
ChunkFileResult_t __thiscall CChunkFile::WriteKeyValueVector3(CChunkFile *this, const char *pszKey, const Vector *vec)
{
  char pDest[28]; // [esp+18h] [ebp-404h] BYREF

  if ( pszKey == nullptr )
    return ChunkFile_Ok;
  V_snprintf(pDest: &pDest[4], maxLen: 1024, pFormat: "\"%s\" \"[%g %g %g]\"", pszKey, vec->x, vec->y, vec->z);
  return CChunkFile::WriteLine(this, pszLine: &pDest[4]);
}
        ...
        void *pData)
{
  ...
  v5 = (ChunkHandlerInfoNode_t *)MemAlloc_Alloc(nSize: 0x5Cu);
  V_strncpy(pDest: v5->Handler.szChunkName, pSrc: pszChunkName, maxLen: 80);
  v5->Handler.pfnHandler = pfnHandler;
  v5->Handler.pData = pData;
  v5->pNext = nullptr;
...
TrailDown3:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0[ecx*4]; jumptable 0042D249 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
...
TrailDown3_0:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0_0[ecx*4]; jumptable 0042E5A9 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
    ...
    if ( v10 != 0 )
    {
      while ( 1 )
      {
        ...
        if ( (unsigned __int8)(v10 - 32) > 0x58u )
          ...
          v13 = aVisualCCrtNotE[v10 + 56] & 0xF;
...
LABEL_77:
              ...
              {
                v34 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_469CAC);
                v34(a1: p_buffer, a2: &_loc_update);
              }
              if ( v81 == 103 && v33 == 0 )
              {
                v35 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_469CA8);
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
unsigned __int16 **__usercall copy_environ@<eax>(const char **oldenviron@<eax>, const char *a2@<esi>)
{
  ...
  if ( oldenviron != nullptr )
  {
    ...
    v8 = a2;
    v5 = (unsigned __int16 **)calloc(count: (unsigned int)result + 1, size: 4u);
    newenviron = v5;
    if ( v5 == nullptr )
      _amsg_exit(rterrnum: 9);
    v6 = *oldenviron;
    if ( *oldenviron != nullptr )
    {
      v7 = (char *)((char *)oldenviron - (char *)v5);
      do
      {
        *v5++ = (unsigned __int16 *)_wcsdup(pKey: v6, pValue: v8, pData: v9);
        v6 = *(const char **)((char *)v5 + (_DWORD)v7);
      }
      while ( v6 != nullptr );
    }
    *v5 = nullptr;
...
int __cdecl __crtwsetenv(unsigned __int16 **poption, int primary)
{
  ...
  {
    v6 = copy_environ(oldenviron: (const char **)_wenviron, a2: nullptr);
    _wenviron = v6;
  }
  if ( v6 == nullptr )
  {
...
LABEL_27:
  if ( primary != 0 )
  {
    ...
    v14 = (unsigned __int16 *)calloc(count: v13 + 2, size: 2u);
    if ( v14 != nullptr )
    {
      v15 = wcslen(wcs: option);
      if ( wcscpy_s(_Dst: v14, _SizeInWords: v15 + 2, _Src: option) != 0 )
        _invoke_watson(a1: (unsigned int)v14);
      v16 = &v14[v20];
      *v16 = 0;
      if ( !SetEnvironmentVariableW(lpName: v14, lpValue: !remove ? v16 + 1 : nullptr) )
      {
        retval = -1;
        *_errno() = 42;
      }
...
f_incwidth2:
      ...
      if ( started != 0 )
      {
        if ( suppress == 0 )
        {
          ...
          pFloatStr[v22] = 0;
          v62 = (char)last - 1;
          v37 = (void (__cdecl *)(int, void *, char *, _LocaleUpdate *))DecodePointer(Ptr: off_469CA4);
...
long double __cdecl _ceil_default(long double x)
{
  ...
  v1 = _ctrlfp(newctrl: newcw_0, _mask: 0xFFFFu);
  if ( (HIWORD(x) & 0x7FF0) == 0x7FF0 )
  {
    v2 = _sptype(x);
    if ( v2 > 0 )
...
int __cdecl _cenvarg(char **argv, char **envp, char **argblk, char **envblk)
{
  ...
  *argblk = v8;
        ...
        EnvironmentStringsA != nullptr) )
  {
    ...
    defined = 1;
LABEL_37:
    v31 = (char *)calloc(count: v29, size: 1u);
    v9 = (void **)argblk;
    *envblk = v31;
    if ( v31 == nullptr )
    {
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int __cdecl sub_441674(_exception *a1)
{
        ...
        unsigned int cw)
{
  ...
  v6 = 0;
  while ( dword_46AC98[2 * v6] != opcode )
  {
    if ( ++v6 >= 29 )
    {
      v7 = nullptr;
...
//------------------------------------------------------------------------------
// Address: 0x004010C0
// Name: public: void CChunkFile::SetDefaultChunkHandler(enum ChunkFileResult_t (*)(class CChunkFile __near *,void __near *,char const __near *),void __near *)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CChunkFile::SetDefaultChunkHandler(
        CChunkFile *this,
        ChunkFileResult_t (__cdecl *pHandler)(CChunkFile *, void *, const char *),
        void *pData)
{
  this->m_DefaultChunkHandler = pHandler;
  this->m_pDefaultChunkHandlerData = pData;
}

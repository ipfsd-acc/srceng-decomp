...
// Original: tier0/dlmalloc/malloc.cpp
// Functions: 395
// ============================================================

#include "tier0\dlmalloc\malloc.h"
...
// Source: json
//------------------------------------------------------------------------------
int __usercall win32munmap@<eax>(char *ptr@<ecx>, unsigned int size@<eax>)
{
  unsigned int v2; // edi
  _MEMORY_BASIC_INFORMATION minfo; // [esp+Ch] [ebp-1Ch] BYREF

  v2 = size;
  if ( size == 0 )
    return 0;
  while ( VirtualQuery(lpAddress: ptr, lpBuffer: &minfo, dwLength: 0x1Cu) != 0
       && minfo.BaseAddress == ptr
       && minfo.AllocationBase == ptr
       && minfo.State == 4096
       && minfo.RegionSize <= v2
       && VirtualFree(lpAddress: ptr, dwSize: 0, dwFreeType: 0x8000u) )
  {
...
//------------------------------------------------------------------------------
// Address: 0x10015EB0
// Name: int DoThreadLock(class CThreadFastMutex __near *)
// Source: json
//------------------------------------------------------------------------------
int __cdecl DoThreadLock(CThreadFastMutex *p)
{
  DWORD CurrentThreadId; // eax

  CurrentThreadId = GetCurrentThreadId();
  if ( CurrentThreadId != p->m_ownerID
    && _InterlockedCompareExchange((volatile signed __int32 *)p, CurrentThreadId, 0) != 0 )
  {
    _mm_pause();
    CThreadFastMutex::Lock(this: p, threadId: CurrentThreadId, nSpinSleepTime: 0);
    return 0;
  }
  else
  {
...
int __cdecl init_mparams()
{
  ...
  if ( mparams.page_size == 0 )
  {
    ...
    mparams.trim_threshold = 0x200000;
    mparams.default_mflags = 7;
    v0 = (_time64(timeptr: nullptr) ^ 0x55555550) & 0xFFFFFFF0 | 8;
    CurrentThreadId = GetCurrentThreadId();
    if ( CurrentThreadId != magic_init_mutex.m_ownerID
      && _InterlockedCompareExchange((volatile signed __int32 *)&magic_init_mutex, CurrentThreadId, 0) != 0 )
    {
      _mm_pause();
      CThreadFastMutex::Lock(this: &magic_init_mutex, threadId: CurrentThreadId, nSpinSleepTime: 0);
    }
    else
    {
      ++magic_init_mutex.m_depth;
    }
    if ( mparams.magic == 0 )
    {
      mparams.magic = v0;
      gm_.mflags = mparams.default_mflags;
    }
    if ( --magic_init_mutex.m_depth == 0 )
      _InterlockedExchange((volatile __int32 *)&magic_init_mutex, 0);
    mparams.page_size = 0x10000;
...
// Source: json
//------------------------------------------------------------------------------
mallinfo *__usercall internal_mallinfo@<eax>(unsigned int *a1@<esi>, malloc_state *result)
{
  malloc_state *v2; // ebx
  DWORD CurrentThreadId; // eax
  unsigned int v4; // edx
  ...
  unsigned int max_footprint; // ecx
  unsigned int topsize; // eax
  unsigned int top; // [esp+8h] [ebp-18h]
  unsigned int v17; // [esp+Ch] [ebp-14h]
  unsigned int nfree; // [esp+10h] [ebp-10h]
  unsigned int sum; // [esp+14h] [ebp-Ch]
  unsigned int mfree; // [esp+18h] [ebp-8h]
  malloc_segment *s; // [esp+1Ch] [ebp-4h]
  ...
  *a1 = 0;
  ...
  a1[8] = 0;
  a1[9] = 0;
  if ( mparams.page_size == 0 && init_mparams() != 0 || (result->mflags & 2) != 0 )
  {
    CurrentThreadId = GetCurrentThreadId();
    if ( CurrentThreadId != result->mutex.m_ownerID
      && _InterlockedCompareExchange((volatile signed __int32 *)&result->mutex, CurrentThreadId, 0) != 0 )
    {
      _mm_pause();
      CThreadFastMutex::Lock(this: &result->mutex, threadId: CurrentThreadId, nSpinSleepTime: 0);
    }
    else
    {
      ++result->mutex.m_depth;
    }
  }
  top = (unsigned int)result->top;
  if ( top != 0 )
  {
    v4 = result->topsize + 56;
    p_seg = &result->seg;
    v6 = v4;
    nfree = 1;
    mfree = v4;
    sum = v4;
    s = &result->seg;
    if ( result != (malloc_state *)-452 )
    {
      do
      {
        base = (unsigned int)p_seg->base;
        v8 = ((unsigned __int8)p_seg->base - 8) & 0xF;
        if ( (((unsigned __int8)p_seg->base - 8) & 0xF) != 0 )
          v8 = -v8 & 0xF;
        v9 = base + v8;
        if ( base + v8 >= base )
        {
  ...
  if ( (v2->mflags & 2) != 0 && v2->mutex.m_depth-- == 1 )
    _InterlockedExchange((volatile __int32 *)&v2->mutex, 0);
  return (mallinfo *)a1;
}
...
// Source: json
//------------------------------------------------------------------------------
int *__usercall mmap_alloc@<eax>(unsigned int m@<edi>, unsigned int nb@<eax>)
{
  unsigned int v2; // ecx
  unsigned int v3; // ebx
  char *v4; // eax
  ...
  v3 = v2 & (mparams.granularity + nb + 38);
  if ( v3 <= nb )
    return nullptr;
  v4 = (char *)VirtualAlloc(
                 lpAddress: nullptr,
                 dwSize: v2 & (mparams.granularity + nb + 38),
                 flAllocationType: 0x103000u,
                 flProtect: 4u);
  if ( v4 == nullptr || v4 == (char *)-1 )
    return nullptr;
  v5 = ((_BYTE)v4 - 8) & 0xF;
  if ( (((_BYTE)v4 - 8) & 0xF) != 0 )
    v5 = -v5 & 0xF;
  ...
  *v7 = v5 | 1;
  v7[1] = (v3 - v5 - 16) | 2;
  *(int *)((char *)v7 + v6 - 16) = m ^ mparams.magic;
  *(int *)((char *)v7 + v6 - 12) = 7;
  *(int *)((char *)v7 + v6 - 8) = 0;
  if ( (unsigned int)v4 < *(_DWORD *)(m + 16) )
    *(_DWORD *)(m + 16) = v4;
  *(_DWORD *)(m + 432) += v3;
  v8 = *(_DWORD *)(m + 432);
  if ( v8 > *(_DWORD *)(m + 436) )
...
void __usercall init_top(malloc_state *m@<esi>, malloc_chunk *p@<ecx>, unsigned int psize@<edx>)
{
  ...
  m->topsize = v5;
  v4->head = v5 | 1;
  *(unsigned int *)((char *)&v4->head + v5) = 56;
...
// Source: json
//------------------------------------------------------------------------------
malloc_chunk **__fastcall prepend_alloc(
        unsigned int nb,
        char *newbase,
        malloc_tree_chunk **m,
        malloc_tree_chunk *oldbase)
{
  int v4; // eax
  malloc_state *v5; // edi
  char *v6; // edx
  ...
  unsigned int head; // ecx
  unsigned int v15; // ecx
  malloc_chunk *fd; // eax
  malloc_chunk *bk; // edx
  malloc_chunk **v18; // ecx
  malloc_tree_chunk *prev_foot; // edi
  malloc_chunk *v20; // eax
  char *p_head; // ecx
  char *child; // eax
  malloc_state *v23; // edx
  ...
  malloc_chunk *v33; // ebx
  char v34; // cl
  unsigned int v35; // edx
  char *least_addr; // edi
  malloc_chunk *v37; // eax
  malloc_chunk *p; // [esp+Ch] [ebp-Ch]
  unsigned int nsize; // [esp+10h] [ebp-8h]
  unsigned int qsize; // [esp+14h] [ebp-4h]
  malloc_tree_chunk **C; // [esp+20h] [ebp+8h]
  malloc_tree_chunk **Ca; // [esp+20h] [ebp+8h]
  malloc_tree_chunk *XP; // [esp+24h] [ebp+Ch]

  v4 = ((_BYTE)newbase - 8) & 0xF;
  v5 = (malloc_state *)m;
  if ( (((_BYTE)newbase - 8) & 0xF) != 0 )
    v4 = -v4 & 0xF;
  v6 = &newbase[v4];
  p = (malloc_chunk *)v6;
  if ( (((_BYTE)oldbase - 8) & 0xF) != 0 )
    v7 = -(((_BYTE)oldbase - 8) & 0xF) & 0xF;
  else
    v7 = 0;
  v8 = (malloc_chunk *)((char *)oldbase + v7);
  v9 = (malloc_chunk *)&v6[nb];
  v10 = (char *)v8 - v6 - nb;
  *((_DWORD *)v6 + 1) = nb | 3;
  *(_DWORD *)&v6[nb] = (unsigned int)m ^ mparams.magic;
  qsize = v10;
  if ( v8 == (malloc_chunk *)m[6] )
  {
    v11 = (unsigned int)m[3] + v10;
    m[3] = (malloc_tree_chunk *)v11;
    m[6] = (malloc_tree_chunk *)v9;
    v9->head = v11 | 1;
    return (malloc_chunk **)(v6 + 8);
  }
  if ( v8 == (malloc_chunk *)m[5] )
  {
    m[2] = (malloc_tree_chunk *)((char *)m[2] + v10);
    v13 = (unsigned int)m[2];
    m[5] = (malloc_tree_chunk *)v9;
    v9->head = v13 | 1;
    *(unsigned int *)((char *)&v9->prev_foot + v13) = v13;
    return (malloc_chunk **)(v6 + 8);
  }
  head = v8->head;
  if ( (head & 2) == 0 )
  {
    nsize = head & 0xFFFFFFF8;
    v15 = (head & 0xFFFFFFF8) >> 3;
    if ( v15 < 0x20 )
    {
      fd = v8->fd;
      bk = v8->bk;
      if ( fd == bk )
      {
        *m = (malloc_tree_chunk *)(~(1 << v15) & (unsigned int)*m);
LABEL_64:
        v5 = (malloc_state *)m;
      ...
      v18 = (malloc_chunk **)&m[2 * v15 + 10];
      if ( (fd == (malloc_chunk *)v18 || fd >= (malloc_chunk *)m[4])
        && (bk == (malloc_chunk *)v18 || bk >= (malloc_chunk *)m[4]) )
      {
        fd->bk = bk;
        bk->fd = fd;
        goto LABEL_64;
    ...
    prev_foot = (malloc_tree_chunk *)v8->bk;
    XP = (malloc_tree_chunk *)v8[1].fd;
    if ( prev_foot != (malloc_tree_chunk *)v8 )
    {
      v20 = v8->fd;
      if ( v20 >= (malloc_chunk *)m[4] )
      {
        v20->bk = (malloc_chunk *)prev_foot;
        prev_foot->fd = (malloc_tree_chunk *)v20;
        goto LABEL_36;
      }
      if ( !Plat_IsInDebugSession() )
LABEL_35:
...
LABEL_34:
    ...
    prev_foot = (malloc_tree_chunk *)v8[1].head;
    p_head = (char *)&v8[1].head;
    if ( prev_foot != nullptr
      || (prev_foot = (malloc_tree_chunk *)v8[1].prev_foot, p_head = (char *)&v8[1], prev_foot != nullptr) )
    {
      while ( 1 )
      {
        child = (char *)&prev_foot->child[1];
        if ( prev_foot->child[1] == nullptr )
        {
          child = (char *)prev_foot->child;
          if ( prev_foot->child[0] == nullptr )
            break;
        }
        prev_foot = *(malloc_tree_chunk **)child;
        p_head = child;
      }
      if ( p_head < (char *)m[4] )
      {
...
LABEL_36:
    if ( XP == nullptr )
      goto LABEL_64;
    v23 = (malloc_state *)m;
    v24 = &m[(int)v8[1].bk + 76];
    if ( v8 == (malloc_chunk *)*v24 )
    {
      *v24 = prev_foot;
      if ( prev_foot == nullptr )
      {
        m[1] = (malloc_tree_chunk *)(~(1 << (int)v8[1].bk) & (unsigned int)m[1]);
        goto LABEL_64;
      }
    }
    else
    {
      ...
      if ( (malloc_chunk *)XP->child[0] == v8 )
        XP->child[0] = prev_foot;
      else
        XP->child[1] = prev_foot;
      if ( prev_foot == nullptr )
        goto LABEL_64;
      v23 = (malloc_state *)m;
    }
    if ( (char *)prev_foot < v23->least_addr )
    {
      if ( !Plat_IsInDebugSession() )
        goto LABEL_63;
      goto LABEL_62;
    ...
    if ( v25 != 0 )
    {
      if ( (char *)v25 < v23->least_addr )
      {
        if ( Plat_IsInDebugSession() )
          __debugbreak();
        abort();
      }
      prev_foot->child[0] = (malloc_tree_chunk *)v25;
      *(_DWORD *)(v25 + 24) = prev_foot;
    }
    v26 = v8[1].head;
    if ( v26 == 0 )
      goto LABEL_64;
    if ( v26 >= (unsigned int)m[4] )
    {
      prev_foot->child[1] = (malloc_tree_chunk *)v26;
      *(_DWORD *)(v26 + 24) = prev_foot;
      goto LABEL_64;
    }
    if ( Plat_IsInDebugSession() )
LABEL_62:
...
LABEL_65:
  ...
  *(unsigned int *)((char *)&v9->prev_foot + v10) = v10;
  if ( v10 >> 3 >= 0x20 )
  {
    ...
    v9[1].head = 0;
    v9[1].prev_foot = 0;
    v9[1].bk = (malloc_chunk *)v30;
    treemap = v5->treemap;
    if ( ((1 << v30) & treemap) == 0 )
    {
      v5->treemap = treemap | (1 << v30);
      *C = (malloc_tree_chunk *)v9;
      v9[1].fd = (malloc_chunk *)C;
      v9->bk = v9;
      v9->fd = v9;
      return &p->fd;
    }
    ...
    if ( v30 == 31 )
      ...
      v34 = 25 - (v30 >> 1);
    v35 = v10 << v34;
    if ( (v33->head & 0xFFFFFFF8) == v10 )
    {
LABEL_85:
      least_addr = v5->least_addr;
      v37 = v33->fd;
      if ( v33 >= (malloc_chunk *)least_addr && v37 >= (malloc_chunk *)least_addr )
      {
        v37->bk = v9;
        v33->fd = v9;
        v9->fd = v37;
    ...
    else
    {
      while ( 1 )
      {
        Ca = (malloc_tree_chunk **)(&v33[1].prev_foot + (v35 >> 31));
        v35 *= 2;
        if ( *Ca == nullptr )
          break;
        v33 = (malloc_chunk *)*Ca;
        if ( ((*Ca)->head & 0xFFFFFFF8) == v10 )
          goto LABEL_85;
      }
      if ( (char *)Ca >= v5->least_addr )
      {
        *Ca = (malloc_tree_chunk *)v9;
        v9[1].fd = v33;
        v9->bk = v9;
        v9->fd = v9;
        return &p->fd;
  ...
  v28 = (int)&v5->smallbins[2 * v27];
  if ( ((1 << v27) & v5->smallmap) != 0 )
  {
    v29 = v5->smallbins[2 * v27 + 2];
    if ( (char *)v29 < v5->least_addr )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    }
    v5->smallbins[2 * v27 + 2] = v9;
    v29->bk = v9;
    v9->fd = v29;
    v9->bk = (malloc_chunk *)v28;
    return &p->fd;
  }
  else
  {
    v5->smallmap |= 1 << v27;
    v5->smallbins[2 * v27 + 2] = v9;
    v5->smallbins[2 * v27 + 3] = v9;
    v9->fd = (malloc_chunk *)v28;
    v9->bk = (malloc_chunk *)v28;
    return &p->fd;
  }
}
...
// Source: json
//------------------------------------------------------------------------------
void __usercall add_segment(unsigned int m@<eax>, malloc_tree_chunk **tbase, unsigned int tsize, unsigned int mmapped)
{
  malloc_chunk *v4; // esi
  malloc_segment *v5; // ecx
  int v6; // edx
  int v7; // ecx
  malloc_chunk *v8; // edx
  int v9; // ebx
  ...
  unsigned int *p_head; // ecx
  malloc_chunk **p_fd; // edi
  unsigned int *v15; // ebx
  unsigned int v16; // edx
  unsigned int v17; // ecx
  malloc_chunk **v18; // edi
  malloc_chunk *v19; // ecx
  unsigned int v20; // ecx
  unsigned int v21; // edi
  malloc_chunk *v22; // edi
  char v23; // cl
  unsigned int v24; // ebx
  char *v25; // eax
  malloc_chunk *fd; // ecx
  char *old_end; // [esp+Ch] [ebp-4h]
  malloc_tree_chunk **C; // [esp+18h] [ebp+8h]
  char *Ca; // [esp+18h] [ebp+8h]

  v4 = *(malloc_chunk **)(m + 24);
  v5 = (malloc_segment *)(m + 452);
  do
  {
    if ( (char *)v4 >= v5->base && (char *)v4 < &v5->base[v5->size] )
      break;
    v5 = v5->next;
  }
  while ( v5 != nullptr );
  v6 = (int)&v5->base[v5->size];
  v7 = ((_BYTE)v6 - 71) & 0xF;
  old_end = (char *)v6;
  if ( (((_BYTE)v6 - 71) & 0xF) != 0 )
    v7 = -v7 & 0xF;
  v8 = (malloc_chunk *)(v7 + v6 - 63);
  if ( v8 < &v4[1] )
    v8 = *(malloc_chunk **)(m + 24);
  v9 = ((_BYTE)tbase - 8) & 0xF;
  if ( (((_BYTE)tbase - 8) & 0xF) != 0 )
    v9 = -v9 & 0xF;
  v10 = tsize - 56 - v9;
  v11 = (char *)tbase + v9;
  *(_DWORD *)(m + 12) = v10;
  *(_DWORD *)(m + 24) = (char *)tbase + v9;
  *((_DWORD *)v11 + 1) = v10 | 1;
  *(_DWORD *)&v11[v10 + 4] = 56;
  *(_DWORD *)(m + 28) = mparams.trim_threshold;
  magic = mparams.magic;
  v8->head = 35;
  v8[2].prev_foot = m ^ magic;
  *(malloc_chunk *)&v8->fd = *(malloc_chunk *)(m + 452);
  *(_DWORD *)(m + 452) = tbase;
  *(_DWORD *)(m + 456) = tsize;
  *(_DWORD *)(m + 460) = &v8->fd;
  p_head = &v8[2].head;
  *(_DWORD *)(m + 464) = mmapped;
  p_fd = &v8[2].fd;
  for ( v8[2].head = 7; p_fd < (malloc_chunk **)old_end; v15[1] = 7 )
  {
    v15 = p_head;
    p_head = (unsigned int *)p_fd++;
  }
  if ( v8 != v4 )
  {
    v16 = (char *)v8 - (char *)v4;
    *(unsigned int *)((char *)&v4->head + v16) &= ~1u;
    v4->head = v16 | 1;
    v17 = v16 >> 3;
    *(unsigned int *)((char *)&v4->prev_foot + v16) = v16;
    if ( v16 >> 3 >= 0x20 )
    {
      v20 = v16 >> 8;
      if ( v16 >> 8 != 0 )
      ...
      C = (malloc_tree_chunk **)(m + 4 * v20 + 304);
      v4[1].bk = (malloc_chunk *)v20;
      v4[1].head = 0;
      v4[1].prev_foot = 0;
      v21 = *(_DWORD *)(m + 4);
      if ( (v21 & (1 << v20)) == 0 )
      {
        *(_DWORD *)(m + 4) = (1 << v20) | v21;
        *C = (malloc_tree_chunk *)v4;
        v4[1].fd = (malloc_chunk *)C;
        v4->bk = v4;
        v4->fd = v4;
        return;
      }
      ...
      if ( v20 == 31 )
        ...
        v23 = 25 - (v20 >> 1);
      v24 = v16 << v23;
      if ( (v22->head & 0xFFFFFFF8) == v16 )
      {
LABEL_34:
        v25 = *(char **)(m + 16);
        fd = v22->fd;
        if ( v22 >= (malloc_chunk *)v25 && fd >= (malloc_chunk *)v25 )
        {
          fd->bk = v4;
          v22->fd = v4;
          v4->bk = v22;
      ...
      else
      {
        while ( 1 )
        {
          Ca = (char *)(&v22[1].prev_foot + (v24 >> 31));
          v24 *= 2;
          if ( *(_DWORD *)Ca == 0 )
            break;
          v22 = *(malloc_chunk **)Ca;
          if ( (*(_DWORD *)(*(_DWORD *)Ca + 4) & 0xFFFFFFF8) == v16 )
            goto LABEL_34;
        }
        if ( (unsigned int)Ca >= *(_DWORD *)(m + 16) )
        {
          *(_DWORD *)Ca = v4;
          v4[1].fd = v22;
          v4->bk = v4;
    ...
    v18 = (malloc_chunk **)(m + 8 * v17 + 40);
    if ( (*(_DWORD *)m & (1 << v17)) != 0 )
    {
      v19 = *(malloc_chunk **)(m + 8 * v17 + 48);
      if ( (unsigned int)v19 < *(_DWORD *)(m + 16) )
      {
        if ( Plat_IsInDebugSession() )
          __debugbreak();
        abort();
      ...
      v18[2] = v4;
      v19->bk = v4;
      v4->bk = (malloc_chunk *)v18;
      v4->fd = v19;
    }
    else
    {
      *(_DWORD *)m |= 1 << v17;
      *(_DWORD *)(m + 8 * v17 + 48) = v4;
      *(_DWORD *)(m + 8 * v17 + 52) = v4;
      v4->bk = (malloc_chunk *)v18;
      v4->fd = (malloc_chunk *)(m + 8 * v17 + 40);
    }
  }
}
...
int *__usercall sys_alloc@<eax>(malloc_state *m@<eax>, unsigned int nb)
{
  ...
  unsigned int magic; // ebx
  malloc_chunk **smallbins; // ecx
  int i; // edx
  malloc_chunk *v12; // ecx
  malloc_segment *p_seg; // edx
  unsigned int sflags; // ecx
  unsigned int size; // ecx
  malloc_segment *v16; // ecx
  unsigned int topsize; // eax
  malloc_chunk *top; // ecx
  unsigned int v19; // eax
  unsigned int *v20; // edx
  unsigned int v21; // edx
  malloc_tree_chunk *base; // edx

  init_mparams();
  v3 = nb;
  if ( (m->mflags & 1) == 0 || nb < mparams.mmap_threshold || (result = mmap_alloc((unsigned int)m, nb)) == nullptr )
  {
    v5 = ~(mparams.granularity - 1);
    v6 = v5 & (mparams.granularity + nb + 64);
    if ( v6 <= nb )
      return nullptr;
    v7 = (char *)VirtualAlloc(
                   lpAddress: nullptr,
                   dwSize: v5 & (mparams.granularity + nb + 64),
                   flAllocationType: 0x3000u,
                   flProtect: 4u);
    if ( v7 == nullptr || v7 == (char *)-1 )
      return nullptr;
    m->footprint += v6;
    ...
    if ( m->top == nullptr )
    {
      ...
      if ( m == &gm_ )
      {
        init_top(m, p: (malloc_chunk *)v7, psize: v6 - 56);
      }
      else
      {
        v12 = (malloc_chunk *)((char *)m + (m[-1].exts & 0xFFFFFFF8) - 8);
        init_top(m, p: v12, psize: v7 - (char *)v12 + v6 - 56);
      }
LABEL_32:
      topsize = m->topsize;
      if ( nb < topsize )
      {
        ...
        v19 = topsize - nb;
        m->topsize = v19;
        v20 = (unsigned int *)((char *)&top->prev_foot + nb);
        m->top = (malloc_chunk *)((char *)top + nb);
        v20[1] = v19 | 1;
        top->head = nb | 3;
        *v20 = (unsigned int)m ^ magic;
        return (int *)&top->fd;
      }
      return nullptr;
    }
    p_seg = &m->seg;
    if ( m != (malloc_state *)-452 )
    {
      while ( v7 != &p_seg->base[p_seg->size] )
      {
        p_seg = p_seg->next;
        if ( p_seg == nullptr )
          goto LABEL_25;
      ...
      if ( (sflags & 8) == 0 && (sflags & 1) != 0 )
      {
        if ( m->top >= (malloc_chunk *)p_seg->base )
        {
          ...
          {
            p_seg->size = v6 + size;
            init_top(m, p: m->top, psize: v6 + m->topsize);
LABEL_31:
            magic = mparams.magic;
            goto LABEL_32;
          }
...
LABEL_25:
    ...
    if ( m != (malloc_state *)-452 )
    {
      ...
      if ( (v21 & 8) == 0 && (v21 & 1) != 0 )
      {
        base = (malloc_tree_chunk *)v16->base;
        v16->size += v6;
        v16->base = v7;
        return (int *)prepend_alloc(nb: v3, newbase: v7, (malloc_tree_chunk **)m, oldbase: base);
      }
    }
LABEL_30:
    add_segment((unsigned int)m, tbase: (malloc_tree_chunk **)v7, tsize: v6, mmapped: 1u);
    goto LABEL_31;
  }
  return result;
}
...
unsigned int __cdecl release_unused_segments(malloc_state *m)
{
  ...
  char *K; // [esp+4h] [ebp-20h]
  malloc_segment *next; // [esp+8h] [ebp-1Ch]
  malloc_segment *pred; // [esp+Ch] [ebp-18h]
  unsigned int size; // [esp+10h] [ebp-14h]
  int nsegs; // [esp+14h] [ebp-10h]
  unsigned int released; // [esp+1Ch] [ebp-8h]
  malloc_segment *v35; // [esp+20h] [ebp-4h]
  unsigned int psize; // [esp+2Ch] [ebp+8h]
...
LABEL_79:
  ...
  do
  {
    ...
    if ( (sflags & 1) == 0 || (sflags & 8) != 0 )
      goto LABEL_77;
    v4 = ((_BYTE)base - 8) & 0xF;
    if ( (((_BYTE)base - 8) & 0xF) != 0 )
      v4 = -v4 & 0xF;
    v5 = (malloc_chunk *)&base[v4];
    v6 = *(_DWORD *)&base[v4 + 4];
    psize = v6 & 0xFFFFFFF8;
    if ( (v6 & 2) != 0 || (char *)v5 + (v6 & 0xFFFFFFF8) < &base[v35->size - 56] )
      goto LABEL_77;
    if ( v5 == m->dv )
    {
      m->dv = nullptr;
    ...
    if ( bk == v5 )
    {
      bk = (malloc_chunk *)v5[1].head;
      p_head = (char *)&v5[1].head;
      if ( bk != nullptr || (bk = (malloc_chunk *)v5[1].prev_foot, p_head = (char *)&v5[1], bk != nullptr) )
      {
        while ( 1 )
        {
          v11 = (char *)&bk[1].head;
...
LABEL_24:
    ...
    if ( fd == nullptr )
      goto LABEL_52;
    v12 = v5[1].bk;
    v13 = v5 == (malloc_chunk *)m->treebins[(_DWORD)v12];
...
LABEL_67:
        ...
        else
        {
          while ( 1 )
          {
            ...
            if ( (*(_DWORD *)(*(_DWORD *)v24 + 4) & 0xFFFFFFF8) == psize )
              goto LABEL_67;
          }
          if ( v24 < m->least_addr )
          {
...
LABEL_76:
      ...
      else
      {
        ...
        *v20 = (malloc_tree_chunk *)v5;
        v5[1].fd = (malloc_chunk *)v20;
        v5->bk = v5;
        v5->fd = v5;
      }
    }
...
BOOL __usercall sys_trim@<eax>(malloc_state *m@<eax>, unsigned int pad@<ecx>)
{
  ...
  malloc_segment *p_seg; // eax
  unsigned int v12; // edx
  malloc_chunk *v13; // ecx
  unsigned int released; // [esp+4h] [ebp-4h]
    ...
    if ( top != nullptr )
    {
      ...
      if ( topsize > v6 )
      {
        ...
        if ( (sflags & 8) == 0 && (sflags & 1) != 0 )
        {
          ...
          {
            p_seg = &m->seg;
            while ( (char *)p_seg < v8->base || (char *)p_seg >= &v8->base[size] )
            {
              p_seg = p_seg->next;
              if ( p_seg == nullptr )
              {
                if ( win32munmap(ptr: &v8->base[size - v7], size: v7) == 0 )
                {
                  released = v7;
                  if ( v7 != 0 )
                  {
      ...
      v2 = release_unused_segments(m) + released;
      if ( v2 == 0 && m->topsize > m->trim_check )
        m->trim_check = -1;
    }
  }
  return v2 != 0;
...
// Source: json
//------------------------------------------------------------------------------
malloc_tree_chunk **__cdecl tmalloc_large(malloc_tree_chunk **m, unsigned int nb)
{
  unsigned int v2; // edi
  ...
  malloc_state *v21; // edx
  bool v22; // zf
  malloc_tree_chunk **v23; // eax
  malloc_tree_chunk *v24; // eax
  unsigned int v25; // eax
  unsigned int v28; // ecx
  malloc_chunk **v29; // ebx
  malloc_chunk *v30; // eax
  unsigned int v31; // eax
  unsigned int v32; // ecx
  unsigned int v33; // edx
  unsigned int v34; // edx
  malloc_chunk *v35; // eax
  unsigned int v36; // edx
  char *v37; // esi
  malloc_chunk *v38; // ecx
  unsigned int J; // [esp+Ch] [ebp-Ch]
  char *Ja; // [esp+Ch] [ebp-Ch]
  char XP; // [esp+10h] [ebp-8h]
  malloc_tree_chunk *XPa; // [esp+10h] [ebp-8h]
  malloc_tree_chunk *v; // [esp+14h] [ebp-4h]
  malloc_tree_chunk **C; // [esp+20h] [ebp+8h]
  malloc_tree_chunk **Ca; // [esp+20h] [ebp+8h]
  char *Cb; // [esp+20h] [ebp+8h]
  ...
  if ( v7 != nullptr )
  {
    ...
    while ( 1 )
    {
      v10 = (v7->head & 0xFFFFFFF8) - v2;
      if ( v10 < v5 )
      {
        v3 = v7;
        v5 = (v7->head & 0xFFFFFFF8) - v2;
        if ( v10 == 0 )
          break;
      }
      v11 = v7->child[1];
      v7 = v7->child[v9 >> 31];
      if ( v11 != nullptr && v11 != v7 )
        J = (unsigned int)v11;
      v2 = nb;
      if ( v7 == nullptr )
      {
        v7 = (malloc_tree_chunk *)J;
        v = v3;
        goto LABEL_20;
      }
      v9 *= 2;
...
LABEL_20:
  ...
  if ( v7 != nullptr )
  {
    ...
    {
LABEL_26:
      if ( (v7->head & 0xFFFFFFF8) - v2 < v5 )
      {
        v5 = (v7->head & 0xFFFFFFF8) - v2;
        v3 = v7;
      }
      if ( v7->child[0] != nullptr )
        v7 = v7->child[0];
      else
        v7 = v7->child[1];
    }
  ...
  if ( v3 == nullptr )
    return nullptr;
LABEL_34:
  if ( v5 >= (unsigned int)m[2] - v2 )
    return nullptr;
  Ja = (char *)m[4];
  if ( v3 < (malloc_tree_chunk *)Ja || (v14 = (malloc_chunk *)((char *)v3 + v2), v3 >= (malloc_tree_chunk *)v14) )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
  ...
  if ( bk != v3 )
  {
    fd = (unsigned int)v3->fd;
    if ( fd >= (unsigned int)Ja )
    {
      *(_DWORD *)(fd + 12) = bk;
      bk->fd = (malloc_tree_chunk *)fd;
      goto LABEL_52;
    }
    if ( !Plat_IsInDebugSession() )
LABEL_51:
...
LABEL_50:
  ...
  bk = v3->child[1];
  v17 = (char *)&v3->child[1];
  if ( bk != nullptr || (bk = v3->child[0], v17 = (char *)v3->child, bk != nullptr) )
  {
    while ( 1 )
    {
      child = &bk->child[1];
  ...
  if ( XPa != nullptr )
  {
    ...
    if ( v22 )
    {
      *v23 = bk;
      if ( bk == nullptr )
      {
        m[1] = (malloc_tree_chunk *)(~(1 << v->index) & (unsigned int)m[1]);
        goto LABEL_80;
      }
    }
    else
    ...
    else
    {
      ...
      v25 = (unsigned int)v19->child[1];
      if ( v25 == 0 )
        goto LABEL_80;
      if ( v25 >= (unsigned int)m[4] )
      {
        bk->child[1] = (malloc_tree_chunk *)v25;
        *(_DWORD *)(v25 + 24) = bk;
        goto LABEL_80;
      }
      if ( !Plat_IsInDebugSession() )
...
LABEL_80:
  if ( v5 < 0x10 )
  {
    ...
    *(unsigned int *)((char *)&v->prev_foot + v5 + nb) = (unsigned int)m ^ mparams.magic;
    return &v->fd;
  }
  v->head = nb | 3;
  v14->prev_foot = (unsigned int)m ^ mparams.magic;
  v14->head = v5 | 1;
  v28 = v5 >> 3;
  *(unsigned int *)((char *)&v14->prev_foot + v5) = v5;
  if ( v5 >> 3 >= 0x20 )
  {
    v31 = v5 >> 8;
    if ( v5 >> 8 != 0 )
    ...
    v14[1].head = 0;
    v14[1].prev_foot = 0;
    v14[1].bk = (malloc_chunk *)v32;
    v34 = (unsigned int)m[1];
    if ( ((1 << v32) & v34) == 0 )
    {
      m[1] = (malloc_tree_chunk *)(v34 | (1 << v32));
      *C = (malloc_tree_chunk *)v14;
      v14[1].fd = (malloc_chunk *)C;
      v14->bk = v14;
      v14->fd = v14;
      return &v->fd;
    }
    ...
    if ( v32 == 31 )
      ...
      Ca = (malloc_tree_chunk **)(25 - (v32 >> 1));
    v36 = v5 << (char)Ca;
    if ( (v35->head & 0xFFFFFFF8) == v5 )
    {
LABEL_103:
      v37 = (char *)m[4];
      v38 = v35->fd;
      if ( v35 >= (malloc_chunk *)v37 && v38 >= (malloc_chunk *)v37 )
      {
        v38->bk = v14;
        v35->fd = v14;
        v14->bk = v35;
    ...
    else
    {
      while ( 1 )
      {
        Cb = (char *)(&v35[1].prev_foot + (v36 >> 31));
        v36 *= 2;
        if ( *(_DWORD *)Cb == 0 )
          break;
        v35 = *(malloc_chunk **)Cb;
        if ( (*(_DWORD *)(*(_DWORD *)Cb + 4) & 0xFFFFFFF8) == v5 )
          goto LABEL_103;
      }
      if ( Cb >= (char *)m[4] )
      {
  ...
  v29 = (malloc_chunk **)&m[2 * v28 + 10];
  if ( ((unsigned int)*m & (1 << v28)) != 0 )
  {
    v30 = (malloc_chunk *)m[2 * v28 + 12];
    if ( v30 < (malloc_chunk *)m[4] )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    }
    m[2 * v28 + 12] = (malloc_tree_chunk *)v14;
    v30->bk = v14;
    v14->fd = v30;
    v14->bk = (malloc_chunk *)v29;
    return &v->fd;
  }
  else
  {
    *m = (malloc_tree_chunk *)((1 << v28) | (unsigned int)*m);
    m[2 * v28 + 12] = (malloc_tree_chunk *)v14;
    m[2 * v28 + 13] = (malloc_tree_chunk *)v14;
    v14->fd = (malloc_chunk *)v29;
    v14->bk = (malloc_chunk *)v29;
    return &v->fd;
  }
}
...
// Source: json
//------------------------------------------------------------------------------
malloc_tree_chunk **__usercall tmalloc_small@<eax>(unsigned int m@<edi>, malloc_chunk *nb)
{
  unsigned int v2; // edx
  malloc_tree_chunk *v3; // ecx
  unsigned int v4; // edx
  malloc_tree_chunk *v5; // ebx
  char *v6; // ecx
  malloc_tree_chunk *bk; // esi
  malloc_chunk *parent; // eax
  malloc_tree_chunk *fd; // eax
  char *v10; // ecx
  char *child; // eax
  unsigned int index; // ecx
  bool v13; // zf
  malloc_tree_chunk **v14; // ecx
  malloc_tree_chunk *v15; // eax
  malloc_tree_chunk *v16; // eax
  unsigned int v17; // ecx
  unsigned int *v18; // eax
  unsigned int v19; // edx
  unsigned int magic; // ecx
  unsigned int v22; // eax
  malloc_chunk *v23; // edx
  unsigned int v24; // ecx
  int v25; // eax
  malloc_chunk *J; // [esp+0h] [ebp-Ch]
  malloc_chunk *DV; // [esp+4h] [ebp-8h]
  unsigned int rsize; // [esp+8h] [ebp-4h]
  malloc_chunk *F; // [esp+14h] [ebp+8h]

  _BitScanForward(&v2, *(_DWORD *)(m + 4) & -*(_DWORD *)(m + 4));
  v3 = *(malloc_tree_chunk **)(m + 4 * v2 + 304);
  v4 = (v3->head & 0xFFFFFFF8) - (_DWORD)nb;
  rsize = v4;
LABEL_2:
  v5 = v3;
  while ( 1 )
  {
    v3 = v3->child[0] != nullptr ? v3->child[0] : v3->child[1];
    if ( v3 == nullptr )
      break;
    if ( (v3->head & 0xFFFFFFF8) - (unsigned int)nb < v4 )
    {
      rsize = (v3->head & 0xFFFFFFF8) - (_DWORD)nb;
      v4 = rsize;
      goto LABEL_2;
    }
  }
  v6 = *(char **)(m + 16);
  if ( v5 < (malloc_tree_chunk *)v6
    || (J = (malloc_chunk *)((char *)nb + (_DWORD)v5), v5 >= (malloc_tree_chunk *)((char *)nb + (int)v5)) )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
  ...
  bk = v5->bk;
  parent = (malloc_chunk *)v5->parent;
  DV = parent;
  if ( bk != v5 )
  {
    fd = v5->fd;
    if ( fd >= (malloc_tree_chunk *)v6 )
    {
      fd->bk = bk;
      bk->fd = fd;
LABEL_26:
...
LABEL_25:
  ...
  bk = v5->child[1];
  v10 = (char *)&v5->child[1];
  if ( bk != nullptr || (bk = v5->child[0], v10 = (char *)v5->child, bk != nullptr) )
  {
    while ( 1 )
    {
    ...
    if ( (unsigned int)v10 >= *(_DWORD *)(m + 16) )
    {
      *(_DWORD *)v10 = 0;
      goto LABEL_26;
    }
...
LABEL_27:
  if ( parent == nullptr )
    goto LABEL_54;
  index = v5->index;
  v13 = v5 == *(malloc_tree_chunk **)(m + 4 * index + 304);
  v14 = (malloc_tree_chunk **)(m + 4 * index + 304);
  if ( v13 )
  {
    *v14 = bk;
    if ( bk == nullptr )
    {
      *(_DWORD *)(m + 4) &= ~(1 << v5->index);
      goto LABEL_54;
    }
  }
  else
  {
    if ( (unsigned int)parent < *(_DWORD *)(m + 16) )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    }
    if ( (malloc_tree_chunk *)parent[1].prev_foot == v5 )
      parent[1].prev_foot = (unsigned int)bk;
    else
      parent[1].head = (unsigned int)bk;
    if ( bk == nullptr )
      goto LABEL_54;
  }
  if ( (unsigned int)bk < *(_DWORD *)(m + 16) )
  {
    if ( !Plat_IsInDebugSession() )
      goto LABEL_53;
    goto LABEL_52;
  }
  bk->parent = (malloc_tree_chunk *)parent;
  v15 = v5->child[0];
  if ( v15 != nullptr )
  {
    if ( (unsigned int)v15 < *(_DWORD *)(m + 16) )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
  ...
  if ( v16 != nullptr )
  {
    if ( (unsigned int)v16 >= *(_DWORD *)(m + 16) )
    {
      bk->child[1] = v16;
      v16->parent = bk;
      goto LABEL_54;
...
LABEL_54:
  ...
  {
    magic = mparams.magic;
    v5->head = (unsigned int)nb | 3;
    J->prev_foot = m ^ magic;
    J->head = v4 | 1;
    *(unsigned int *)((char *)&J->prev_foot + v4) = v4;
    v22 = *(_DWORD *)(m + 8);
    if ( v22 != 0 )
    {
      v23 = *(malloc_chunk **)(m + 20);
      v24 = v22 >> 3;
      v25 = 1 << (v22 >> 3);
      F = (malloc_chunk *)(m + 8 * v24 + 40);
      if ( (*(_DWORD *)m & v25) != 0 )
      {
        if ( *(_DWORD *)(m + 8 * v24 + 48) < *(_DWORD *)(m + 16) )
        {
          if ( Plat_IsInDebugSession() )
            __debugbreak();
          abort();
        }
        F = *(malloc_chunk **)(m + 8 * v24 + 48);
      }
      else
      {
        *(_DWORD *)m |= v25;
      }
      *(_DWORD *)(m + 8 * v24 + 48) = v23;
      F->bk = v23;
      v23->fd = F;
      v23->bk = (malloc_chunk *)(m + 8 * v24 + 40);
      v4 = rsize;
    }
    *(_DWORD *)(m + 8) = v4;
    *(_DWORD *)(m + 20) = J;
  ...
  {
    v17 = (unsigned int)nb + v4;
    v18 = (unsigned int *)((char *)&nb->prev_foot + (_DWORD)v5 + v4);
    v19 = mparams.magic;
    v5->head = v17 | 3;
    v18[1] |= 1u;
    *v18 = m ^ v19;
...
// Source: json
//------------------------------------------------------------------------------
void *__cdecl dlmalloc(unsigned int bytes)
{
  DWORD CurrentThreadId; // ecx
  unsigned int v2; // esi
  unsigned int v3; // edi
  ...
  malloc_chunk *fd; // ecx
  malloc_chunk **v8; // eax
  unsigned int magic; // edx
  void *result; // eax
  unsigned int dvsize; // ecx
  unsigned int v12; // edx
  char v13; // cl
  unsigned int v14; // edi
  malloc_chunk *v15; // ebx
  malloc_chunk **v16; // eax
  malloc_chunk *v17; // edx
  unsigned int v18; // edx
  unsigned int v19; // edi
  unsigned int v20; // ecx
  malloc_chunk *v21; // esi
  malloc_chunk **v22; // ebx
  malloc_chunk **v23; // ecx
  unsigned int v24; // eax
  malloc_chunk *v25; // edx
  malloc_chunk *v26; // ecx
  unsigned int v27; // eax
  ...
  malloc_chunk *v32; // ecx
  unsigned int v33; // eax
  malloc_chunk *r; // [esp+Ch] [ebp-10h]
  malloc_chunk *DV; // [esp+10h] [ebp-Ch]

  if ( mparams.page_size == 0 && init_mparams() != 0 || (gm_.mflags & 2) != 0 )
  {
    CurrentThreadId = GetCurrentThreadId();
    if ( CurrentThreadId != gm_.mutex.m_ownerID
      && _InterlockedCompareExchange((volatile signed __int32 *)&gm_.mutex, CurrentThreadId, 0) != 0 )
    {
      _mm_pause();
      CThreadFastMutex::Lock(this: &gm_.mutex, threadId: CurrentThreadId, nSpinSleepTime: 0);
    }
    else
    {
      ++gm_.mutex.m_depth;
...
LABEL_46:
      ...
      if ( v2 > gm_.dvsize )
      {
        ...
        if ( v2 >= gm_.topsize )
        {
          result = sys_alloc(m: &gm_, nb: v2);
        }
        else
        {
          gm_.topsize -= v2;
          top = gm_.top;
          v32 = (malloc_chunk *)((char *)gm_.top + v2);
          gm_.top = v32;
          v32->head = (topsize - v2) | 1;
          v33 = (unsigned int)&gm_ ^ mparams.magic;
          top->head = v2 | 3;
          v32->prev_foot = v33;
          result = &top->fd;
        }
    ...
    result = tmalloc_large(m: (malloc_tree_chunk **)&gm_, nb: v2);
LABEL_45:
    if ( result != nullptr )
      goto postaction;
    goto LABEL_46;
  ...
  if ( bytes >= 7 )
    ...
    v2 = 16;
  v3 = v2 >> 3;
  v4 = gm_.smallmap >> (v2 >> 3);
  if ( (v4 & 3) != 0 )
  {
    v5 = ((v4 & 1) == 0) + v3;
    v6 = gm_.smallbins[2 * v5 + 2];
    fd = v6->fd;
    v8 = &gm_.smallbins[2 * v5];
    if ( v8 == (malloc_chunk **)fd )
    {
      gm_.smallmap &= ~(1 << v5);
    }
    else
    {
      ...
      v8[2] = fd;
      fd->bk = (malloc_chunk *)v8;
    }
    magic = mparams.magic;
    v6->head = (8 * v5) | 3;
    *(&v6->head + 2 * v5) |= 1u;
    *(&v6->prev_foot + 2 * v5) = (unsigned int)&gm_ ^ magic;
    result = &v6->fd;
    goto postaction;
  }
  dvsize = gm_.dvsize;
...
LABEL_47:
    ...
    if ( dvsize - v2 < 0x10 )
    {
      ...
      gm_.dv = nullptr;
      v25->head = v29;
      *(unsigned int *)((char *)&v25->head + v28) |= 1u;
      *(unsigned int *)((char *)&v25->prev_foot + v28) = (unsigned int)&gm_ ^ mparams.magic;
    }
    else
    {
      v26 = (malloc_chunk *)((char *)gm_.dv + v2);
      gm_.dvsize = v24;
      gm_.dv = v26;
      v26->head = v24 | 1;
      *(unsigned int *)((char *)&v26->prev_foot + v24) = v24;
      v27 = (unsigned int)&gm_ ^ mparams.magic;
      v25->head = v2 | 3;
      v26->prev_foot = v27;
    }
    result = &v25->fd;
  ...
  if ( v4 == 0 )
  {
    if ( gm_.treemap == 0 )
      goto LABEL_46;
    result = tmalloc_small(m: (unsigned int)&gm_, nb: (malloc_chunk *)v2);
    goto LABEL_45;
  }
  _BitScanForward(&v12, (2 * (-1 << v3)) & (v4 << v3) & -((2 * (-1 << v3)) & (v4 << v3)));
  v13 = v12;
  v14 = 8 * v12;
  v15 = gm_.smallbins[2 * v12 + 2];
  v16 = &gm_.smallbins[2 * v12];
  r = (malloc_chunk *)&v15->fd;
  v17 = v15->fd;
  if ( v16 == (malloc_chunk **)v17 )
  {
    gm_.smallmap &= ~(1 << v13);
  }
  else
  {
    ...
    v16[2] = v17;
    v17->bk = (malloc_chunk *)v16;
  }
  v18 = mparams.magic;
  v19 = v14 - v2;
  v20 = v2;
  v21 = (malloc_chunk *)((char *)v15 + v2);
  v15->head = v20 | 3;
  v21->prev_foot = (unsigned int)&gm_ ^ v18;
  v21->head = v19 | 1;
  *(unsigned int *)((char *)&v21->prev_foot + v19) = v19;
  if ( gm_.dvsize != 0 )
  {
    DV = gm_.dv;
    v22 = &gm_.smallbins[2 * (gm_.dvsize >> 3)];
    if ( ((1 << (gm_.dvsize >> 3)) & gm_.smallmap) != 0 )
    {
      v23 = (malloc_chunk **)v22[2];
      if ( (char *)v23 < gm_.least_addr )
      {
        if ( Plat_IsInDebugSession() )
          __debugbreak();
        abort();
    ...
    else
    {
      ...
      v23 = &gm_.smallbins[2 * (gm_.dvsize >> 3)];
    }
    v22[2] = gm_.dv;
    v23[3] = DV;
    DV->fd = (malloc_chunk *)v23;
    DV->bk = (malloc_chunk *)v22;
  }
  result = r;
  gm_.dvsize = v19;
  gm_.dv = v21;
postaction:
  if ( (gm_.mflags & 2) != 0 && --gm_.mutex.m_depth == 0 )
    _InterlockedExchange((volatile __int32 *)&gm_.mutex, 0);
  return result;
}
...
// Source: json
//------------------------------------------------------------------------------
void __cdecl dlfree(malloc_tree_chunk **mem)
{
  malloc_chunk *v1; // ebx
  unsigned int v2; // esi
  unsigned int v3; // ecx
  unsigned int head; // edx
  unsigned int v5; // eax
  malloc_chunk *v6; // edi
  unsigned int prev_foot; // edx
  unsigned int v8; // edx
  unsigned int v9; // edi
  unsigned int v10; // eax
  ...
  unsigned int v28; // ecx
  malloc_chunk *v29; // eax
  malloc_chunk *v30; // edx
  int v31; // ecx
  malloc_tree_chunk *v32; // edi
  malloc_chunk *v33; // eax
  malloc_chunk *v34; // edx
  malloc_tree_chunk **child; // eax
  malloc_chunk *v36; // eax
  malloc_tree_chunk **v37; // eax
  unsigned int v38; // eax
  unsigned int v39; // eax
  unsigned int v40; // edi
  unsigned int v41; // ecx
  int v42; // edi
  malloc_chunk *v43; // eax
  unsigned int v44; // eax
  unsigned int v45; // ecx
  unsigned int v46; // edx
  int v47; // eax
  malloc_tree_chunk *v48; // eax
  unsigned int v49; // edx
  unsigned int v50; // edx
  malloc_chunk *v51; // ecx
  unsigned int K; // [esp+0h] [ebp-Ch]
  unsigned int psize; // [esp+4h] [ebp-8h]
  unsigned int psizea; // [esp+4h] [ebp-8h]
  malloc_chunk *next; // [esp+8h] [ebp-4h]
  malloc_tree_chunk **C; // [esp+14h] [ebp+8h]
  malloc_tree_chunk **Ca; // [esp+14h] [ebp+8h]
  malloc_tree_chunk **Cb; // [esp+14h] [ebp+8h]
  malloc_tree_chunk **Cc; // [esp+14h] [ebp+8h]
  malloc_tree_chunk **Cd; // [esp+14h] [ebp+8h]

  if ( mem != nullptr )
  {
    v1 = (malloc_chunk *)(mem - 2);
    v2 = mparams.magic ^ *(unsigned int *)((char *)mem + ((unsigned int)*(mem - 1) & 0xFFFFFFF8) - 8);
    if ( *(_DWORD *)(v2 + 0x24) != mparams.magic )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    }
    if ( (mparams.page_size != 0 || init_mparams() == 0) && (*(_BYTE *)(v2 + 440) & 2) == 0
      || DoThreadLock(p: (CThreadFastMutex *)(v2 + 444)) == 0 )
    {
      v3 = *(_DWORD *)(v2 + 16);
      ...
      if ( (head & 2) == 0 )
        goto erroraction;
      v5 = head & 0xFFFFFFF8;
      v6 = (malloc_chunk *)((char *)v1 + (head & 0xFFFFFFF8));
      psize = head & 0xFFFFFFF8;
      next = v6;
      if ( (head & 1) != 0 )
        goto LABEL_29;
      prev_foot = v1->prev_foot;
      if ( (v1->prev_foot & 1) != 0 )
      {
        v8 = prev_foot & 0xFFFFFFFE;
        v9 = v5 + v8 + 16;
        if ( win32munmap(ptr: (char *)v1 - v8, size: v9) == 0 )
          *(_DWORD *)(v2 + 432) -= v9;
        goto postaction_0;
      }
      v1 = (malloc_chunk *)((char *)v1 - prev_foot);
      v10 = prev_foot + v5;
      psize = v10;
      C = (malloc_tree_chunk **)v1;
      if ( (unsigned int)v1 < v3 )
        goto erroraction;
      if ( v1 == *(malloc_chunk **)(v2 + 20) )
      {
        if ( (v6->head & 3) == 3 )
        {
          *(_DWORD *)(v2 + 8) = v10;
          v6->head &= ~1u;
          v1->head = v10 | 1;
          *(unsigned int *)((char *)&v1->prev_foot + v10) = v10;
postaction_0:
          if ( (*(_BYTE *)(v2 + 440) & 2) != 0 )
          {
            v22 = (*(_DWORD *)(v2 + 448))-- == 1;
            if ( v22 )
              _InterlockedExchange((volatile __int32 *)(v2 + 444), 0);
          }
          return;
        }
        goto LABEL_28;
      ...
      if ( v11 < 0x20 )
      {
        ...
        v14 = v2 + 8 * v11 + 40;
        if ( (fd == (malloc_chunk *)v14 || (unsigned int)fd >= v3)
          && (bk == (malloc_chunk *)v14 || (unsigned int)bk >= v3) )
        {
          fd->bk = bk;
          bk->fd = fd;
          goto LABEL_28;
...
LABEL_27:
      ...
      v17 = v1->bk;
      K = (unsigned int)v1[1].fd;
      if ( v17 == v1 )
      {
        v17 = (malloc_chunk *)v1[1].head;
        v19 = (malloc_chunk *)((char *)v1 + 20);
        if ( v17 == nullptr )
        {
          v17 = (malloc_chunk *)v1[1].prev_foot;
          v19 = v1 + 1;
          if ( v17 == nullptr )
            goto LABEL_53;
        }
        while ( 1 )
        {
          v20 = (malloc_chunk *)((char *)v17 + 20);
          if ( v17[1].head == 0 )
          {
            v20 = v17 + 1;
            if ( v17[1].prev_foot == 0 )
              break;
          }
          v17 = (malloc_chunk *)v20->prev_foot;
          v19 = v20;
        }
        if ( (unsigned int)v19 >= v3 )
        {
          v19->prev_foot = 0;
          goto LABEL_53;
        }
...
LABEL_53:
          if ( K == 0 )
            goto LABEL_28;
          v21 = v1[1].bk;
          v22 = v1 == *(malloc_chunk **)(v2 + 4 * (_DWORD)v21 + 304);
          v23 = (malloc_chunk **)(v2 + 4 * (_DWORD)v21 + 304);
          if ( v22 )
          {
            *v23 = v17;
            if ( v17 == nullptr )
            {
              *(_DWORD *)(v2 + 4) &= ~(1 << (int)v1[1].bk);
              goto LABEL_29;
            }
          }
          else
          ...
          if ( (unsigned int)v17 < *(_DWORD *)(v2 + 16) )
          {
            if ( Plat_IsInDebugSession() )
              __debugbreak();
            abort();
          }
          v17[1].fd = (malloc_chunk *)K;
          v24 = v1[1].prev_foot;
          if ( v24 != 0 )
          {
            if ( v24 < *(_DWORD *)(v2 + 16) )
...
LABEL_29:
            if ( v1 < next )
            {
              ...
              if ( (v15 & 1) != 0 )
              {
                if ( (v15 & 2) != 0 )
                {
                  next->head = v15 & 0xFFFFFFFE;
                  v1->head = psize | 1;
                  *(unsigned int *)((char *)&v1->prev_foot + psize) = psize;
                  v40 = psize;
                  goto LABEL_139;
                }
                if ( next == *(malloc_chunk **)(v2 + 24) )
                {
                  *(_DWORD *)(v2 + 12) += psize;
                  v16 = *(_DWORD *)(v2 + 12);
                  *(_DWORD *)(v2 + 24) = v1;
                  ...
                  if ( v16 > *(_DWORD *)(v2 + 28) )
                    sys_trim(m: (malloc_state *)v2, pad: 0);
                  goto postaction_0;
                }
                if ( next == *(malloc_chunk **)(v2 + 20) )
                {
                  *(_DWORD *)(v2 + 8) += psize;
                  v26 = *(_DWORD *)(v2 + 8);
                  *(_DWORD *)(v2 + 20) = v1;
                  v1->head = v26 | 1;
                  *(unsigned int *)((char *)&v1->prev_foot + v26) = v26;
                  goto postaction_0;
                }
                v27 = v15 & 0xFFFFFFF8;
                psizea = v27 + psize;
                ...
                if ( v27 >> 3 < 0x20 )
                {
                  ...
                  v31 = v2 + 8 * v28 + 40;
                  if ( v29 != (malloc_chunk *)v31 && (unsigned int)v29 < *(_DWORD *)(v2 + 16)
                    || v30 != (malloc_chunk *)v31 && (unsigned int)v30 < *(_DWORD *)(v2 + 16) )
                  {
                    if ( Plat_IsInDebugSession() )
LABEL_134:
                      __debugbreak();
LABEL_135:
                ...
                v32 = (malloc_tree_chunk *)next->bk;
                Ca = (malloc_tree_chunk **)next[1].fd;
                if ( v32 == (malloc_tree_chunk *)next )
                {
                  v32 = (malloc_tree_chunk *)next[1].head;
                  v34 = (malloc_chunk *)((char *)next + 20);
                  if ( v32 == nullptr )
                  {
                    v32 = (malloc_tree_chunk *)next[1].prev_foot;
                    v34 = next + 1;
                    if ( v32 == nullptr )
                      goto LABEL_109;
                  }
                  ...
                  {
                    child = &v32->child[1];
                    if ( v32->child[1] == nullptr )
                    {
                      child = v32->child;
                      if ( v32->child[0] == nullptr )
                        break;
                    }
                    v32 = *child;
                    v34 = (malloc_chunk *)child;
                  }
                  if ( (unsigned int)v34 >= *(_DWORD *)(v2 + 16) )
                  {
                    v34->prev_foot = 0;
                    goto LABEL_109;
                  }
                ...
                {
                  v33 = next->fd;
                  if ( (unsigned int)v33 >= *(_DWORD *)(v2 + 16) )
                  {
                    v33->bk = (malloc_chunk *)v32;
                    v32->fd = (malloc_tree_chunk *)v33;
LABEL_109:
                    if ( Ca == nullptr )
                      goto LABEL_136;
                    v36 = next[1].bk;
                    v22 = next == *(malloc_chunk **)(v2 + 4 * (_DWORD)v36 + 304);
                    v37 = (malloc_tree_chunk **)(v2 + 4 * (_DWORD)v36 + 304);
                    if ( v22 )
                    {
                      *v37 = v32;
                      if ( v32 == nullptr )
                      {
                        *(_DWORD *)(v2 + 4) &= ~(1 << (int)next[1].bk);
                        goto LABEL_136;
                      }
                    }
                    else
                    {
                      if ( (unsigned int)Ca < *(_DWORD *)(v2 + 16) )
                      {
                        if ( Plat_IsInDebugSession() )
                          __debugbreak();
                        abort();
                      }
                      if ( Ca[4] == (malloc_tree_chunk *)next )
                        Ca[4] = v32;
                      else
                        Ca[5] = v32;
                      if ( v32 == nullptr )
                    ...
                    if ( (unsigned int)v32 < *(_DWORD *)(v2 + 16) )
                    {
                      if ( !Plat_IsInDebugSession() )
                        goto LABEL_135;
                      goto LABEL_134;
                    }
                    v32->parent = (malloc_tree_chunk *)Ca;
                    v38 = next[1].prev_foot;
                    if ( v38 != 0 )
                    {
                      if ( v38 < *(_DWORD *)(v2 + 16) )
                      {
                      ...
                      v32->child[0] = (malloc_tree_chunk *)v38;
                      *(_DWORD *)(v38 + 24) = v32;
                    }
                    v39 = next[1].head;
                    if ( v39 != 0 )
                    {
                      ...
                      v32->child[1] = (malloc_tree_chunk *)v39;
                      *(_DWORD *)(v39 + 24) = v32;
                    }
LABEL_136:
                    v40 = psizea;
                    v1->head = psizea | 1;
                    *(unsigned int *)((char *)&v1->prev_foot + psizea) = psizea;
                    if ( v1 == *(malloc_chunk **)(v2 + 20) )
                    {
                      *(_DWORD *)(v2 + 8) = psizea;
                      goto postaction_0;
...
LABEL_139:
                    ...
                    {
                      v42 = v2 + 8 * v41 + 40;
                      if ( ((1 << v41) & *(_DWORD *)v2) != 0 )
                      {
                        v43 = *(malloc_chunk **)(v2 + 8 * v41 + 48);
                        if ( (unsigned int)v43 < *(_DWORD *)(v2 + 16) )
                        {
                          if ( Plat_IsInDebugSession() )
                            __debugbreak();
                          abort();
                        }
                        *(_DWORD *)(v2 + 8 * v41 + 48) = v1;
                        v43->bk = v1;
                        v1->fd = v43;
                        v1->bk = (malloc_chunk *)v42;
                      }
                      else
                      {
                        *(_DWORD *)v2 |= 1 << v41;
                        *(_DWORD *)(v2 + 8 * v41 + 48) = v1;
                        *(_DWORD *)(v2 + 8 * v41 + 52) = v1;
                        v1->fd = (malloc_chunk *)v42;
                        v1->bk = (malloc_chunk *)v42;
                      }
                      goto postaction_0;
                    }
                    ...
                    Cb = (malloc_tree_chunk **)(v2 + 4 * v45 + 304);
                    v1[1].bk = (malloc_chunk *)v45;
                    v1[1].head = 0;
                    v1[1].prev_foot = 0;
                    v47 = *(_DWORD *)(v2 + 4);
                    if ( (v47 & (1 << v45)) == 0 )
                    {
                      *(_DWORD *)(v2 + 4) = (1 << v45) | v47;
                      *Cb = (malloc_tree_chunk *)v1;
                      v1[1].fd = (malloc_chunk *)Cb;
                      v1->bk = v1;
                      v1->fd = v1;
                      goto LABEL_170;
                    }
                    ...
                    if ( v45 == 31 )
                      ...
                      Cc = (malloc_tree_chunk **)(25 - (v45 >> 1));
                    v49 = v40 << (char)Cc;
                    if ( (v48->head & 0xFFFFFFF8) == v40 )
                    {
LABEL_160:
                      v50 = *(_DWORD *)(v2 + 16);
                      v51 = (malloc_chunk *)v48->fd;
                      if ( (unsigned int)v48 >= v50 && (unsigned int)v51 >= v50 )
                      {
                        v51->bk = v1;
                        v48->fd = (malloc_tree_chunk *)v1;
                        v1->fd = v51;
                        v1->bk = (malloc_chunk *)v48;
                        v1[1].fd = nullptr;
                        goto LABEL_170;
                      }
                      if ( Plat_IsInDebugSession() )
                    ...
                    else
                    {
                      while ( 1 )
                      {
                        ...
                        if ( *Cd == nullptr )
                          break;
                        v48 = *Cd;
                        if ( ((*Cd)->head & 0xFFFFFFF8) == v40 )
                          goto LABEL_160;
                      }
                      if ( (unsigned int)Cd >= *(_DWORD *)(v2 + 16) )
                      {
                        *Cd = (malloc_tree_chunk *)v1;
                        v1[1].fd = (malloc_chunk *)v48;
                        v1->bk = v1;
                        v1->fd = v1;
LABEL_170:
                        v22 = (*(_DWORD *)(v2 + 32))-- == 1;
                        if ( v22 )
                          release_unused_segments(m: (malloc_state *)v2);
                        goto postaction_0;
                      }
                      if ( Plat_IsInDebugSession() )
LABEL_168:
...
//------------------------------------------------------------------------------
// Address: 0x10018070
// Name: _dlmalloc_usable_size
// Source: json
//------------------------------------------------------------------------------
unsigned int __cdecl dlmalloc_usable_size(_DWORD *mem)
{
  int v1; // eax

  if ( mem != nullptr && ((v1 = *(mem - 1)) & 2) != 0 )
    return (v1 & 0xFFFFFFF8) - 8;
  else
    return 0;
}
...
malloc_state *__usercall init_user_mstate@<eax>(char *tbase@<eax>, unsigned int tsize)
{
  ...
  int v4; // eax
  char *v5; // esi
  unsigned int default_mflags; // eax
  unsigned int magic; // edx
  char *v8; // eax
  int i; // edx
  char *v10; // edx
  ...
  v3 = tbase - 8;
  v4 = ((_BYTE)tbase - 8) & 0xF;
  if ( v4 != 0 )
    v16 = -v4 & 0xF;
  else
    v16 = 0;
  v5 = &tbase[v16 + 8];
  memset(dst: (unsigned __int8 *)v5, value: 0, count: 0x1F0u);
  default_mflags = mparams.default_mflags;
  magic = mparams.magic;
  *(_DWORD *)&tbase[v16 + 4] = 499;
  *((_DWORD *)v5 + 4) = tbase;
  *((_DWORD *)v5 + 113) = tbase;
  *((_DWORD *)v5 + 9) = magic;
  ...
  *((_DWORD *)v5 + 118) = 0;
  ...
  v10 = &tbase[v16 + (*(_DWORD *)&tbase[v16 + 4] & 0xFFFFFFF8)];
  v11 = v3 - v10 + tsize - 48;
  v12 = ((tbase[v16 + 4] & 0xF8) + (_BYTE)v16 + (_BYTE)tbase + 8 - 16) & 0xF;
  if ( (((tbase[v16 + 4] & 0xF8) + (_BYTE)v16 + (_BYTE)tbase + 8 - 16) & 0xF) != 0 )
    v12 = -v12 & 0xF;
  v13 = &v10[v12];
  v14 = v11 - v12;
  *(_DWORD *)&tbase[v16 + 32] = v13;
  *(_DWORD *)&tbase[v16 + 20] = v14;
  *((_DWORD *)v13 + 1) = v14 | 1;
  *(_DWORD *)&v13[v14 + 4] = 56;
  *(_DWORD *)&tbase[v16 + 36] = mparams.trim_threshold;
  return (malloc_state *)&tbase[v16 + 8];
}
...
// Source: json
//------------------------------------------------------------------------------
malloc_state *__cdecl create_mspace(unsigned int capacity, int locked)
{
  ...
  init_mparams();
  if ( capacity >= -552 - mparams.page_size )
    return nullptr;
  if ( capacity != 0 )
    granularity = capacity + 552;
  else
    granularity = mparams.granularity;
  v3 = ~(mparams.granularity - 1) & (mparams.granularity + granularity - 1);
  v4 = (char *)VirtualAlloc(lpAddress: nullptr, dwSize: v3, flAllocationType: 0x3000u, flProtect: 4u);
  if ( v4 == nullptr || v4 == (char *)-1 )
    return nullptr;
  result = init_user_mstate(tbase: v4, tsize: v3);
  mflags = result->mflags;
  result->seg.sflags = 1;
  if ( locked != 0 )
    result->mflags = mflags | 2;
  else
    result->mflags = mflags & 0xFFFFFFFD;
  return result;
}
...
// Source: json
//------------------------------------------------------------------------------
void *__cdecl mspace_malloc(malloc_tree_chunk **msp, unsigned int bytes)
{
  ...
  malloc_chunk *v9; // esi
  char *v10; // ecx
  malloc_tree_chunk *fd; // edx
  void *result; // eax
  unsigned int v13; // ebx
  char *v14; // ecx
  malloc_chunk *v15; // edx
  unsigned int v16; // ebx
  unsigned int v17; // edx
  malloc_chunk *v18; // esi
  unsigned int v19; // eax
  unsigned int v20; // ecx
  int v21; // eax
  ...
  malloc_chunk *v27; // edx
  unsigned int v28; // ecx
  unsigned int *v29; // ebx
  CThreadFastMutex *v30; // edi
  malloc_chunk **p_fd; // [esp+4h] [ebp-10h]
  malloc_chunk *DV; // [esp+8h] [ebp-Ch]
  malloc_chunk *p; // [esp+1Ch] [ebp+8h]
  malloc_chunk *pa; // [esp+1Ch] [ebp+8h]

  magic = mparams.magic;
  if ( msp[9] != (malloc_tree_chunk *)mparams.magic )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
...
LABEL_9:
  CurrentThreadId = GetCurrentThreadId();
  if ( (malloc_tree_chunk *)CurrentThreadId != msp[111]
    && _InterlockedCompareExchange((volatile signed __int32 *)msp + 111, CurrentThreadId, 0) != 0 )
  {
    _mm_pause();
    CThreadFastMutex::Lock(this: (CThreadFastMutex *)(msp + 111), threadId: CurrentThreadId, nSpinSleepTime: 0);
  }
  else
  {
    msp[112] = (malloc_tree_chunk *)((char *)msp[112] + 1);
  }
  magic = mparams.magic;
LABEL_14:
  if ( bytes > 0xE8 )
  {
    if ( bytes < 0xFFFFFFC0 )
    {
      v5 = (bytes + 23) & 0xFFFFFFF0;
      if ( msp[1] != nullptr )
      {
        result = tmalloc_large(m: msp, nb: v5);
        goto LABEL_52;
      }
    }
    else
...
LABEL_54:
    v22 = (unsigned int)msp[2];
    if ( v5 > v22 )
    {
      v26 = (unsigned int)msp[3];
      if ( v5 >= v26 )
      {
        result = sys_alloc(m: (malloc_state *)msp, nb: v5);
      }
      else
      {
        v27 = (malloc_chunk *)msp[6];
        v28 = v26 - v5;
        msp[3] = (malloc_tree_chunk *)v28;
        v29 = (unsigned int *)((char *)&v27->prev_foot + v5);
        msp[6] = (malloc_tree_chunk *)((char *)v27 + v5);
        v29[1] = v28 | 1;
        v27->head = v5 | 3;
        *v29 = (unsigned int)msp ^ magic;
        result = &v27->fd;
      }
    }
    else
    {
      v23 = (malloc_chunk *)msp[5];
      v24 = v22 - v5;
      if ( v22 - v5 < 0x10 )
      {
        msp[2] = nullptr;
        msp[5] = nullptr;
        v23->head = v22 | 3;
        *(unsigned int *)((char *)&v23->head + v22) |= 1u;
        *(unsigned int *)((char *)&v23->prev_foot + v22) = (unsigned int)msp ^ magic;
      }
      else
      {
        v25 = (unsigned int *)((char *)&v23->prev_foot + v5);
        msp[5] = (malloc_tree_chunk *)((char *)v23 + v5);
        msp[2] = (malloc_tree_chunk *)v24;
        v25[1] = v24 | 1;
        *(unsigned int *)((char *)v25 + v24) = v24;
        v23->head = v5 | 3;
        *v25 = (unsigned int)msp ^ magic;
      }
      result = &v23->fd;
    }
    goto postaction_1;
  ...
  if ( bytes >= 7 )
    ...
    v5 = 16;
  v6 = v5 >> 3;
  v7 = (unsigned int)*msp >> (v5 >> 3);
  if ( (v7 & 3) != 0 )
  {
    v8 = ((v7 & 1) == 0) + v6;
    v9 = (malloc_chunk *)msp[2 * v8 + 12];
    v10 = (char *)&msp[2 * v8 + 10];
    fd = (malloc_tree_chunk *)v9->fd;
    if ( v10 == (char *)fd )
    {
      *msp = (malloc_tree_chunk *)((unsigned int)*msp & ~(1 << v8));
    }
    else
    {
      if ( fd < msp[4] )
      ...
      msp[2 * v8 + 12] = fd;
      fd->bk = (malloc_tree_chunk *)v10;
    }
    v9->head = (8 * v8) | 3;
    *(&v9->head + 2 * v8) |= 1u;
    *(&v9->prev_foot + 2 * v8) = (unsigned int)msp ^ magic;
    result = &v9->fd;
    goto postaction_1;
  }
  if ( v5 <= (unsigned int)msp[2] )
    goto LABEL_54;
  if ( v7 == 0 )
  {
    if ( msp[1] != nullptr )
    {
      result = tmalloc_small(m: (unsigned int)msp, nb: (malloc_chunk *)v5);
LABEL_52:
      if ( result != nullptr )
        goto postaction_1;
      magic = mparams.magic;
  ...
  _BitScanForward(&v7, (2 * (-1 << v6)) & (v7 << v6) & -((2 * (-1 << v6)) & (v7 << v6)));
  v13 = v7;
  v14 = (char *)&msp[2 * v7 + 10];
  p = (malloc_chunk *)msp[2 * v7 + 12];
  p_fd = &p->fd;
  v15 = p->fd;
  if ( v14 == (char *)v15 )
  {
    *msp = (malloc_tree_chunk *)((unsigned int)*msp & ~(1 << v13));
  }
  else
  {
    if ( v15 < (malloc_chunk *)msp[4] )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    ...
    *((_DWORD *)v14 + 2) = v15;
    v15->bk = (malloc_chunk *)v14;
  }
  v16 = 8 * v13 - v5;
  v17 = v5;
  v18 = (malloc_chunk *)((char *)p + v5);
  p->head = v17 | 3;
  v18->prev_foot = (unsigned int)msp ^ magic;
  v18->head = v16 | 1;
  *(unsigned int *)((char *)&v18->prev_foot + v16) = v16;
  v19 = (unsigned int)msp[2];
  if ( v19 != 0 )
  {
    DV = (malloc_chunk *)msp[5];
    v20 = v19 >> 3;
    v21 = 1 << (v19 >> 3);
    pa = (malloc_chunk *)&msp[2 * v20 + 10];
    if ( ((unsigned int)*msp & v21) != 0 )
    {
      if ( msp[2 * v20 + 12] < msp[4] )
      {
        if ( Plat_IsInDebugSession() )
          __debugbreak();
        abort();
      }
      pa = (malloc_chunk *)msp[2 * v20 + 12];
    }
    else
    {
      *msp = (malloc_tree_chunk *)(v21 | (unsigned int)*msp);
    }
    msp[2 * v20 + 12] = (malloc_tree_chunk *)DV;
    pa->bk = DV;
    DV->fd = pa;
    DV->bk = (malloc_chunk *)&msp[2 * v20 + 10];
  }
  result = p_fd;
  msp[2] = (malloc_tree_chunk *)v16;
  msp[5] = (malloc_tree_chunk *)v18;
postaction_1:
  if ( ((_BYTE)msp[110] & 2) != 0 )
  {
    v30 = (CThreadFastMutex *)(msp + 111);
    if ( v30->m_depth-- == 1 )
      _InterlockedExchange((volatile __int32 *)v30, 0);
  }
  return result;
}
...
// Source: json
//------------------------------------------------------------------------------
void __cdecl mspace_free(void *msp, malloc_tree_chunk **mem)
{
  malloc_chunk *v2; // ebx
  unsigned int v3; // esi
  unsigned int v4; // ecx
  ...
  unsigned int v17; // eax
  malloc_chunk *v18; // edi
  malloc_chunk *v19; // eax
  malloc_chunk *v20; // edx
  malloc_chunk *v21; // eax
  malloc_chunk *v22; // eax
  bool v23; // zf
  malloc_chunk **v24; // eax
  unsigned int v25; // eax
  ...
  unsigned int v29; // ecx
  malloc_chunk *v30; // eax
  malloc_chunk *v31; // edx
  int v32; // ecx
  malloc_tree_chunk *v33; // edi
  malloc_chunk *v34; // eax
  malloc_chunk *v35; // edx
  malloc_tree_chunk **child; // eax
  malloc_chunk *v37; // eax
  malloc_tree_chunk **v38; // eax
  unsigned int v39; // eax
  unsigned int v40; // eax
  unsigned int v41; // edi
  unsigned int v42; // ecx
  int v43; // edi
  malloc_chunk *v44; // eax
  ...
  malloc_chunk *v52; // ecx
  unsigned int K; // [esp+0h] [ebp-Ch]
  unsigned int psize; // [esp+4h] [ebp-8h]
  unsigned int psizea; // [esp+4h] [ebp-8h]
  malloc_chunk *next; // [esp+8h] [ebp-4h]
  malloc_tree_chunk **C; // [esp+18h] [ebp+Ch]
  malloc_tree_chunk **Ca; // [esp+18h] [ebp+Ch]
  malloc_tree_chunk **Cb; // [esp+18h] [ebp+Ch]
  malloc_tree_chunk **Cc; // [esp+18h] [ebp+Ch]
  malloc_tree_chunk **Cd; // [esp+18h] [ebp+Ch]

  if ( mem != nullptr )
  {
    v2 = (malloc_chunk *)(mem - 2);
    v3 = mparams.magic ^ *(unsigned int *)((char *)mem + ((unsigned int)*(mem - 1) & 0xFFFFFFF8) - 8);
    if ( *(_DWORD *)(v3 + 0x24) != mparams.magic )
    {
      if ( Plat_IsInDebugSession() )
        __debugbreak();
      abort();
    }
    if ( (mparams.page_size != 0 || init_mparams() == 0) && (*(_BYTE *)(v3 + 440) & 2) == 0
      || DoThreadLock(p: (CThreadFastMutex *)(v3 + 444)) == 0 )
    {
      v4 = *(_DWORD *)(v3 + 16);
      ...
      if ( (head & 2) == 0 )
        goto erroraction_0;
      v6 = head & 0xFFFFFFF8;
      v7 = (malloc_chunk *)((char *)v2 + (head & 0xFFFFFFF8));
      psize = head & 0xFFFFFFF8;
      next = v7;
      if ( (head & 1) != 0 )
        goto LABEL_29;
      prev_foot = v2->prev_foot;
      if ( (v2->prev_foot & 1) != 0 )
      {
        v9 = prev_foot & 0xFFFFFFFE;
        v10 = v6 + v9 + 16;
        if ( win32munmap(ptr: (char *)v2 - v9, size: v10) == 0 )
          *(_DWORD *)(v3 + 432) -= v10;
        goto postaction_2;
      }
      v2 = (malloc_chunk *)((char *)v2 - prev_foot);
      v11 = prev_foot + v6;
      psize = v11;
      C = (malloc_tree_chunk **)v2;
      if ( (unsigned int)v2 < v4 )
        goto erroraction_0;
      if ( v2 == *(malloc_chunk **)(v3 + 20) )
      {
        if ( (v7->head & 3) == 3 )
        {
          *(_DWORD *)(v3 + 8) = v11;
          v7->head &= ~1u;
          v2->head = v11 | 1;
          *(unsigned int *)((char *)&v2->prev_foot + v11) = v11;
postaction_2:
          if ( (*(_BYTE *)(v3 + 440) & 2) != 0 )
          {
            v23 = (*(_DWORD *)(v3 + 448))-- == 1;
            if ( v23 )
              _InterlockedExchange((volatile __int32 *)(v3 + 444), 0);
          }
          return;
        }
        goto LABEL_28;
      ...
      if ( v12 < 0x20 )
      {
        ...
        v15 = v3 + 8 * v12 + 40;
        if ( (fd == (malloc_chunk *)v15 || (unsigned int)fd >= v4)
          && (bk == (malloc_chunk *)v15 || (unsigned int)bk >= v4) )
        {
          fd->bk = bk;
          bk->fd = fd;
          goto LABEL_28;
...
LABEL_27:
      ...
      v18 = v2->bk;
      K = (unsigned int)v2[1].fd;
      if ( v18 == v2 )
      {
        v18 = (malloc_chunk *)v2[1].head;
        v20 = (malloc_chunk *)((char *)v2 + 20);
        if ( v18 == nullptr )
        {
          v18 = (malloc_chunk *)v2[1].prev_foot;
          v20 = v2 + 1;
          if ( v18 == nullptr )
            goto LABEL_53;
        }
        while ( 1 )
        {
          v21 = (malloc_chunk *)((char *)v18 + 20);
          if ( v18[1].head == 0 )
          {
            v21 = v18 + 1;
            if ( v18[1].prev_foot == 0 )
              break;
          }
          v18 = (malloc_chunk *)v21->prev_foot;
          v20 = v21;
        }
        if ( (unsigned int)v20 >= v4 )
        {
          v20->prev_foot = 0;
          goto LABEL_53;
        }
...
LABEL_53:
          if ( K == 0 )
            goto LABEL_28;
          v22 = v2[1].bk;
          v23 = v2 == *(malloc_chunk **)(v3 + 4 * (_DWORD)v22 + 304);
          v24 = (malloc_chunk **)(v3 + 4 * (_DWORD)v22 + 304);
          if ( v23 )
          {
            *v24 = v18;
            if ( v18 == nullptr )
            {
              *(_DWORD *)(v3 + 4) &= ~(1 << (int)v2[1].bk);
              goto LABEL_29;
            }
          }
          else
          ...
          if ( (unsigned int)v18 < *(_DWORD *)(v3 + 16) )
          {
            if ( Plat_IsInDebugSession() )
              __debugbreak();
            abort();
          }
          v18[1].fd = (malloc_chunk *)K;
          v25 = v2[1].prev_foot;
          if ( v25 != 0 )
          {
            if ( v25 < *(_DWORD *)(v3 + 16) )
...
LABEL_29:
            if ( v2 < next )
            {
              ...
              if ( (v16 & 1) != 0 )
              {
                if ( (v16 & 2) != 0 )
                {
                  next->head = v16 & 0xFFFFFFFE;
                  v2->head = psize | 1;
                  *(unsigned int *)((char *)&v2->prev_foot + psize) = psize;
                  v41 = psize;
                  goto LABEL_139;
                }
                if ( next == *(malloc_chunk **)(v3 + 24) )
                {
                  *(_DWORD *)(v3 + 12) += psize;
                  v17 = *(_DWORD *)(v3 + 12);
                  *(_DWORD *)(v3 + 24) = v2;
                  ...
                  if ( v17 > *(_DWORD *)(v3 + 28) )
                    sys_trim(m: (malloc_state *)v3, pad: 0);
                  goto postaction_2;
                }
                if ( next == *(malloc_chunk **)(v3 + 20) )
                {
                  *(_DWORD *)(v3 + 8) += psize;
                  v27 = *(_DWORD *)(v3 + 8);
                  *(_DWORD *)(v3 + 20) = v2;
                  v2->head = v27 | 1;
                  *(unsigned int *)((char *)&v2->prev_foot + v27) = v27;
                  goto postaction_2;
                }
                v28 = v16 & 0xFFFFFFF8;
                psizea = v28 + psize;
                ...
                if ( v28 >> 3 < 0x20 )
                {
                  ...
                  v32 = v3 + 8 * v29 + 40;
                  if ( v30 != (malloc_chunk *)v32 && (unsigned int)v30 < *(_DWORD *)(v3 + 16)
                    || v31 != (malloc_chunk *)v32 && (unsigned int)v31 < *(_DWORD *)(v3 + 16) )
                  {
                    if ( Plat_IsInDebugSession() )
LABEL_134:
                      __debugbreak();
LABEL_135:
                ...
                v33 = (malloc_tree_chunk *)next->bk;
                Ca = (malloc_tree_chunk **)next[1].fd;
                if ( v33 == (malloc_tree_chunk *)next )
                {
                  v33 = (malloc_tree_chunk *)next[1].head;
                  v35 = (malloc_chunk *)((char *)next + 20);
                  if ( v33 == nullptr )
                  {
                    v33 = (malloc_tree_chunk *)next[1].prev_foot;
                    v35 = next + 1;
                    if ( v33 == nullptr )
                      goto LABEL_109;
                  }
                  ...
                  {
                    child = &v33->child[1];
                    if ( v33->child[1] == nullptr )
                    {
                      child = v33->child;
                      if ( v33->child[0] == nullptr )
                        break;
                    }
                    v33 = *child;
                    v35 = (malloc_chunk *)child;
                  }
                  if ( (unsigned int)v35 >= *(_DWORD *)(v3 + 16) )
                  {
                    v35->prev_foot = 0;
                    goto LABEL_109;
                  }
                ...
                {
                  v34 = next->fd;
                  if ( (unsigned int)v34 >= *(_DWORD *)(v3 + 16) )
                  {
                    v34->bk = (malloc_chunk *)v33;
                    v33->fd = (malloc_tree_chunk *)v34;
LABEL_109:
                    if ( Ca == nullptr )
                      goto LABEL_136;
                    v37 = next[1].bk;
                    v23 = next == *(malloc_chunk **)(v3 + 4 * (_DWORD)v37 + 304);
                    v38 = (malloc_tree_chunk **)(v3 + 4 * (_DWORD)v37 + 304);
                    if ( v23 )
                    {
                      *v38 = v33;
                      if ( v33 == nullptr )
                      {
                        *(_DWORD *)(v3 + 4) &= ~(1 << (int)next[1].bk);
                        goto LABEL_136;
                      }
                    }
                    else
                    {
                      if ( (unsigned int)Ca < *(_DWORD *)(v3 + 16) )
                      {
                        if ( Plat_IsInDebugSession() )
                          __debugbreak();
                        abort();
                    ...
                    if ( (unsigned int)v33 < *(_DWORD *)(v3 + 16) )
                    {
                      if ( !Plat_IsInDebugSession() )
                        goto LABEL_135;
                      goto LABEL_134;
                    }
                    v33->parent = (malloc_tree_chunk *)Ca;
                    v39 = next[1].prev_foot;
                    if ( v39 != 0 )
                    {
                      if ( v39 < *(_DWORD *)(v3 + 16) )
                      {
                      ...
                      v33->child[0] = (malloc_tree_chunk *)v39;
                      *(_DWORD *)(v39 + 24) = v33;
                    }
                    v40 = next[1].head;
                    if ( v40 != 0 )
                    {
                      ...
                      v33->child[1] = (malloc_tree_chunk *)v40;
                      *(_DWORD *)(v40 + 24) = v33;
                    }
LABEL_136:
                    v41 = psizea;
                    v2->head = psizea | 1;
                    *(unsigned int *)((char *)&v2->prev_foot + psizea) = psizea;
                    if ( v2 == *(malloc_chunk **)(v3 + 20) )
                    {
                      *(_DWORD *)(v3 + 8) = psizea;
                      goto postaction_2;
...
LABEL_139:
                    ...
                    {
                      v43 = v3 + 8 * v42 + 40;
                      if ( ((1 << v42) & *(_DWORD *)v3) != 0 )
                      {
                        v44 = *(malloc_chunk **)(v3 + 8 * v42 + 48);
                        if ( (unsigned int)v44 < *(_DWORD *)(v3 + 16) )
                        {
                          if ( Plat_IsInDebugSession() )
                            __debugbreak();
                          abort();
                        }
                        *(_DWORD *)(v3 + 8 * v42 + 48) = v2;
                        v44->bk = v2;
                        v2->fd = v44;
                        v2->bk = (malloc_chunk *)v43;
                      }
                      else
                      {
                        *(_DWORD *)v3 |= 1 << v42;
                        *(_DWORD *)(v3 + 8 * v42 + 48) = v2;
                        *(_DWORD *)(v3 + 8 * v42 + 52) = v2;
                        v2->fd = (malloc_chunk *)v43;
                        v2->bk = (malloc_chunk *)v43;
                      }
                      goto postaction_2;
                    }
                    ...
                    if ( v41 >> 8 != 0 )
                    {
                      if ( v45 <= 0xFFFF )
                      {
                        _BitScanReverse(&v47, v45);
                        v46 = ((v41 >> (v47 + 7)) & 1) + 2 * v47;
                      }
                      else
                      {
                        v46 = 31;
                    ...
                    Cb = (malloc_tree_chunk **)(v3 + 4 * v46 + 304);
                    v2[1].bk = (malloc_chunk *)v46;
                    v2[1].head = 0;
                    v2[1].prev_foot = 0;
                    v48 = *(_DWORD *)(v3 + 4);
                    if ( (v48 & (1 << v46)) == 0 )
                    {
                      *(_DWORD *)(v3 + 4) = (1 << v46) | v48;
                      *Cb = (malloc_tree_chunk *)v2;
                      v2[1].fd = (malloc_chunk *)Cb;
                      v2->bk = v2;
                      v2->fd = v2;
                      goto LABEL_170;
                    }
                    ...
                    if ( v46 == 31 )
                      ...
                      Cc = (malloc_tree_chunk **)(25 - (v46 >> 1));
                    v50 = v41 << (char)Cc;
                    if ( (v49->head & 0xFFFFFFF8) == v41 )
                    {
LABEL_160:
                      v51 = *(_DWORD *)(v3 + 16);
                      v52 = (malloc_chunk *)v49->fd;
                      if ( (unsigned int)v49 >= v51 && (unsigned int)v52 >= v51 )
                      {
                        v52->bk = v2;
                        v49->fd = (malloc_tree_chunk *)v2;
                        v2->fd = v52;
                        v2->bk = (malloc_chunk *)v49;
                        v2[1].fd = nullptr;
                        goto LABEL_170;
                      }
                      if ( Plat_IsInDebugSession() )
                    ...
                    else
                    {
                      while ( 1 )
                      {
                        ...
                        if ( *Cd == nullptr )
                          break;
                        v49 = *Cd;
                        if ( ((*Cd)->head & 0xFFFFFFF8) == v41 )
                          goto LABEL_160;
                      }
                      if ( (unsigned int)Cd >= *(_DWORD *)(v3 + 16) )
                      {
                        *Cd = (malloc_tree_chunk *)v2;
                        v2[1].fd = (malloc_chunk *)v49;
                        v2->bk = v2;
                        v2->fd = v2;
LABEL_170:
                        v23 = (*(_DWORD *)(v3 + 32))-- == 1;
                        if ( v23 )
                          release_unused_segments(m: (malloc_state *)v3);
                        goto postaction_2;
                      }
                      if ( Plat_IsInDebugSession() )
LABEL_168:
...
// Source: json
//------------------------------------------------------------------------------
BOOL __cdecl mspace_trim(malloc_state *msp, unsigned int pad)
{
  DWORD CurrentThreadId; // eax
  BOOL result; // eax

  if ( msp->magic != mparams.magic )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
  }
  if ( mparams.page_size == 0 && init_mparams() != 0 || (msp->mflags & 2) != 0 )
  {
    CurrentThreadId = GetCurrentThreadId();
    if ( CurrentThreadId != msp->mutex.m_ownerID
      && _InterlockedCompareExchange((volatile signed __int32 *)&msp->mutex, CurrentThreadId, 0) != 0 )
    {
      _mm_pause();
      CThreadFastMutex::Lock(this: &msp->mutex, threadId: CurrentThreadId, nSpinSleepTime: 0);
    }
    else
    {
      ++msp->mutex.m_depth;
  ...
  result = sys_trim(m: msp, pad);
  if ( (msp->mflags & 2) != 0 && msp->mutex.m_depth-- == 1 )
    _InterlockedExchange((volatile __int32 *)&msp->mutex, 0);
  return result;
}

//------------------------------------------------------------------------------
// Address: 0x10018CD0
// Name: _mspace_footprint
// Source: json
//------------------------------------------------------------------------------
unsigned int __cdecl mspace_footprint(_DWORD *msp)
{
  if ( msp[9] != mparams.magic )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
...
//------------------------------------------------------------------------------
// Address: 0x10018D10
// Name: _mspace_max_footprint
// Source: json
//------------------------------------------------------------------------------
unsigned int __cdecl mspace_max_footprint(_DWORD *msp)
{
  if ( msp[9] != mparams.magic )
  {
    if ( Plat_IsInDebugSession() )
...
//------------------------------------------------------------------------------
// Address: 0x10018D50
// Name: _mspace_mallinfo
// Source: json
//------------------------------------------------------------------------------
mallinfo *__cdecl mspace_mallinfo(mallinfo *result, malloc_state *msp)
{
  unsigned int v3[10]; // [esp+4h] [ebp-28h] BYREF
  ...
  *result = *internal_mallinfo(a1: v3, result: msp);
  return result;
}
...
unsigned __int8 *__cdecl internal_realloc(malloc_state *m, unsigned __int8 *oldmem, unsigned int bytes)
{
  ...
  unsigned int v22; // eax
  malloc_tree_chunk **extra; // [esp+4h] [ebp-Ch]
  unsigned int v24; // [esp+8h] [ebp-8h]
  malloc_chunk *newp; // [esp+Ch] [ebp-4h]

  if ( bytes >= 0xFFFFFFC0 )
    return nullptr;
  if ( mparams.page_size == 0 && init_mparams() != 0 || (m->mflags & 2) != 0 )
  {
    CurrentThreadId = GetCurrentThreadId();
    if ( CurrentThreadId != m->mutex.m_ownerID
      && _InterlockedCompareExchange((volatile signed __int32 *)&m->mutex, CurrentThreadId, 0) != 0 )
    {
      _mm_pause();
      CThreadFastMutex::Lock(this: &m->mutex, threadId: CurrentThreadId, nSpinSleepTime: 0);
    }
    else
    {
      ++m->mutex.m_depth;
  ...
  v7 = v5 & 0xFFFFFFF8;
  v24 = v5 & 0xFFFFFFF8;
  v8 = (char *)&oldmem[(v5 & 0xFFFFFFF8) - 8];
  newp = nullptr;
  extra = nullptr;
  if ( (char *)(oldmem - 8) < m->least_addr || (v5 & 2) == 0 || v6 >= v8 || (v8[4] & 1) == 0 )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
  ...
  if ( v10 != 0 || (*v6 & 1) == 0 )
  {
    if ( v7 < v9 )
    {
      ...
      if ( topsize + v7 > v9 )
      {
        *((_DWORD *)v6 + 1) = v9 | v10 | 2;
        magic = mparams.magic;
        *(_DWORD *)&v6[v9 + 4] |= 1u;
        v17 = v7 + topsize - v9;
        v18 = (unsigned int *)&v6[v9];
        *v18 = (unsigned int)m ^ magic;
        v18[1] = v17 | 1;
        m->top = (malloc_chunk *)&v6[v9];
        m->topsize = v17;
    ...
    else
    {
      ...
      if ( v11 >= 0x10 )
      {
        ...
        *((_DWORD *)v6 + 1) = v9 | v10 | 2;
        v13 = mparams.magic;
        v12[1] |= 1u;
        v14 = (unsigned int)m ^ v13;
        *v12 = v14;
        v12[1] = v11 | v12[1] & 1 | 2;
        *(unsigned int *)((char *)v12 + v11 + 4) |= 1u;
        *(unsigned int *)((char *)v12 + v11) = v14;
        extra = (malloc_tree_chunk **)(v12 + 2);
      }
    }
    v7 = v24;
    goto LABEL_31;
  }
  if ( (v9 & 0xFFFFFFF8) < 0x100 || v7 < v9 + 4 || v7 - v9 > 2 * mparams.granularity )
    newp = nullptr;
  else
    newp = (malloc_chunk *)(oldmem - 8);
LABEL_31:
  if ( (m->mflags & 2) != 0 && m->mutex.m_depth-- == 1 )
    _InterlockedExchange((volatile __int32 *)&m->mutex, 0);
  if ( newp != nullptr )
  {
    if ( extra != nullptr )
    {
  ...
  {
    if ( m == &gm_ )
      v20 = (unsigned __int8 *)dlmalloc(bytes);
    else
      v20 = (unsigned __int8 *)mspace_malloc(msp: (malloc_tree_chunk **)m, bytes);
    v21 = v20;
    if ( v20 != nullptr )
    {
      v22 = v7 - 8;
      if ( v7 - 8 >= bytes )
        v22 = bytes;
      memcpy(dst: v21, src: oldmem, count: v22);
      if ( m == &gm_ )
      {
        dlfree(mem: (malloc_tree_chunk **)oldmem);
        return v21;
      }
      mspace_free(msp: m, mem: (malloc_tree_chunk **)oldmem);
    }
    return v21;
  }
}
...
// Source: json
//------------------------------------------------------------------------------
unsigned __int8 *__cdecl dlrealloc(unsigned __int8 *oldmem, unsigned int bytes)
{
  if ( oldmem == nullptr )
    return (unsigned __int8 *)dlmalloc(bytes);
  if ( *(_DWORD *)((mparams.magic ^ *(_DWORD *)&oldmem[(*((_DWORD *)oldmem - 1) & 0xFFFFFFF8) - 8]) + 0x24) != mparams.magic )
  {
    if ( Plat_IsInDebugSession() )
      __debugbreak();
    abort();
  }
  return internal_realloc(
           m: (malloc_state *)(mparams.magic ^ *(_DWORD *)&oldmem[(*((_DWORD *)oldmem - 1) & 0xFFFFFFF8) - 8]),
           oldmem,
           bytes);
}
...
TrailDown3:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0[ecx*4]; jumptable 10019119 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
...
TrailDown3_0:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0_0[ecx*4]; jumptable 1001A079 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int __cdecl _heapwalk(_heapinfo *_entry)
{
  int *pentry; // eax
  _PROCESS_HEAP_ENTRY Entry; // [esp+10h] [ebp-3Ch] BYREF
  int retval; // [esp+2Ch] [ebp-20h]
  int errflag; // [esp+30h] [ebp-1Ch]
  ...
  if ( pentry != nullptr )
  {
    if ( _entry->_useflag != 1 )
      goto nextBlock;
    if ( HeapValidate(hHeap: _crtheap, dwFlags: 0, lpMem: pentry) )
    {
      Entry.wFlags = 4;
      goto nextBlock;
    }
  ...
  if ( !HeapWalk(hHeap: _crtheap, lpEntry: &Entry) )
    return -3;
  while ( (Entry.wFlags & 3) != 0 )
  {
nextBlock:
    ms_exc.registration.TryLevel = 0;
    errflag = !HeapWalk(hHeap: _crtheap, lpEntry: &Entry);
    ms_exc.registration.TryLevel = -2;
    if ( errflag == 1 )
      return (GetLastError() != 259) - 5;
  }
  _entry->_pentry = (int *)Entry.lpData;
  _entry->_size = Entry.cbData;
  _entry->_useflag = (Entry.wFlags & 4) != 0;
  return retval;
}
...
char *__cdecl asctime(const tm *tb)
{
  ...
  if ( asctimebuf != nullptr
    ...
    && (dword_1003A028[tm_mon] - _days[tm_mon] >= tm_mday
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
LPVOID __cdecl operator new[](unsigned int cb)
{
  return operator new(size: cb);
}
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
unsigned int __cdecl fread(void *buffer, unsigned int elementSize, unsigned int count, _iobuf *stream)
{
...
LABEL_6:
  ...
  while ( (int)v6 < (int)dword_10039420 );
  return 0;
}
...
int __cdecl _open_osfhandle(void *osfhandle, __int16 flags)
{
  ...
  if ( (flags & 0x80u) != 0 )
    v2 |= 0x10u;
  FileType = GetFileType(hFile: osfhandle);
  switch ( FileType )
  {
    case 0u:
      LastError = GetLastError();
  ...
  _set_osfhnd(fh: v6, value: osfhandle);
  v8 = &__pioinfo[v7 >> 5];
  v9 = (v7 & 0x1F) << 6;
  *(&(*v8)->osfile + v9) = v2 | 1;
  *((_BYTE *)*v8 + v9 + 36) &= 0x80u;
  *((_BYTE *)*v8 + v9 + 36) &= ~0x80u;
  _unlock_fhandle(fh: v7);
  return v7;
}
...
void __cdecl _NMSG_WRITE(int rterrnum)
{
  ...
  if ( v1 != nullptr )
  {
    if ( _set_error_mode(em: 3) == 1 || _set_error_mode(em: 3) == 0 && *(_DWORD *)&__app_type == 1 )
    {
    ...
    else if ( rterrnum != 252 )
    {
      if ( wcscpy_s(_Dst: outmsg, _SizeInWords: 0x314u, _Src: aRuntimeErrorPr) != 0
        ...
        && (v2 = wcslen(wcs: &outmsg[25]),
            wcsncpy_s(
              _Dst: &__app_type + v2,
              _SizeInWords: 763 - (&__app_type + v2 - &outmsg[25]),
              _Src: L"...",
              _Count: 3u) != 0)
...
LABEL_77:
              ...
              {
                v34 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_10039FB4);
                v34(a1: p_buffer, a2: &_loc_update);
              }
              if ( v81 == 103 && v33 == 0 )
              {
                v35 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_10039FB0);
...
void _ioterm()
{
  ...
  while ( (int)v0 < (int)&__sse2_available );
}
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
LPVOID __cdecl _malloc_crt(unsigned int cb)
{
  DWORD v1; // esi
  LPVOID v2; // edi
  int v3; // eax
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
LPVOID __cdecl _realloc_crt(void *ptr, unsigned int size)
{
  DWORD v2; // esi
  LPVOID v3; // edi
  int v4; // eax
...
// Source: linker_block_proximity
//------------------------------------------------------------------------------
void __cdecl CallDestructExceptionObject(_EXCEPTION_RECORD *pExcept, unsigned __int8 fThrowNotAllowed)
{
  ...
  {
    __DestructExceptionObject((EHExceptionRecord *)pExcept, fThrowNotAllowed);
  }
}
    ...
    do
    {
      ...
      while ( nCatchableTypes > 0 )
      {
        if ( __TypeMatch(
               ...
               pThrow: pExcept->params.pThrowInfo) != 0 )
        {
        ...
        unsigned int NLGCode)
{
  ...
  if ( pExcept->ExceptionCode == -529697949 && pExcept->NumberParameters == 3 )
  {
    ...
    {
      __DestructExceptionObject(pExcept, fThrowNotAllowed: 0);
    }
  }
        ...
        const _s_CatchableType *pConv)
{
  ...
  if ( pType != nullptr && pType->name[0] != 0 )
  {
    ...
    if ( dispCatchObj != 0 || (pCatch->adjectives & 0x80000000) != 0 )
    {
      ...
      {
        if ( _ValidateRead(code: (int (__stdcall *)())pExcept->params.pExceptionObject) && _ValidateRead(code: v6) )
        {
...
LABEL_11:
      ...
      else
      {
        ...
        {
          if ( _ValidateRead(code: v11) && _ValidateRead(code: v6) )
          {
            memmove(
        ...
        {
          if ( _ValidateRead(code: v11)
            && _ValidateRead(code: v6)
            && _ValidateRead(code: (int (__stdcall *)())pConv->copyFunction) )
          {
            return ((pConv->properties & 4) != 0) + 1;
          }
        }
        else if ( _ValidateRead(code: v11) && _ValidateRead(code: v6) )
        {
        ...
        EHRegistrationNode *pMarkerRN)
{
  ...
  if ( pConv != nullptr )
    __BuildCatchObject(pExcept, pRN, pCatch, pConv);
        ...
        EHRegistrationNode *pMarkerRN)
{
  ...
  if ( pExcept->ExceptionCode != -2147483645
    && (_getptd()->_translator == nullptr
     || *(p_translator = &_getptd()->_translator) == (void *)_encoded_null()
        ...
        if ( IsInExceptionSpec(pESTypeList: curexcspec, pExcept: v28) == 0 )
        {
          ...
          if ( curexcspec->nCount > 0 )
          {
            do
            {
              ...
              {
                __DestructExceptionObject(pExcept, fThrowNotAllowed: 1u);
...
LABEL_45:
            ...
            while ( 1 )
            {
              ...
              pCatchable = *v23;
              if ( __TypeMatch(pCatch, pCatchable, pThrow: pThrowInfo) != 0 )
                break;
              --catchables;
              ++v23;
              if ( catchables <= 0 )
...
NextTryBlock:
  ...
  if ( recursive != 0 )
    __DestructExceptionObject(pExcept: curexception, fThrowNotAllowed: 1u);

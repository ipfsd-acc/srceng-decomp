...
//------------------------------------------------------------------------------
// Address: 0x10272230
// Name: void SetBitBufErrorHandler(void (*)(enum BitBufErrorType,char const __near *))
// Source: json
//------------------------------------------------------------------------------
void __cdecl SetBitBufErrorHandler(void (__cdecl *fn)(BitBufErrorType, const char *))
{
...
//------------------------------------------------------------------------------
// Address: 0x10272240
// Name: public: CBitWriteMasksInit::CBitWriteMasksInit(void)
// Source: json
//------------------------------------------------------------------------------
CBitWriteMasksInit *__thiscall CBitWriteMasksInit::CBitWriteMasksInit(CBitWriteMasksInit *this)
{
  int v1; // esi
  int *v2; // edx
  int v3; // ecx
  ...
  {
    v3 = v1 + 1;
    v4 = `GetBitForBitnum'::`2'::bitsForBitnum[v1 & 0x1F] - 1;
    v5 = v1 + 1;
    for ( i = 11; i != 0; --i )
    {
      *(v2 - 1) = v4;
      if ( v5 - 1 < 0x20 )
        *(v2 - 1) = v4 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[((_BYTE)v5 - 1) & 0x1F] - 1);
      *v2 = v4;
      if ( v5 < 0x20 )
        *v2 = v4 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[v5 & 0x1F] - 1);
      v2[1] = v4;
      if ( v5 + 1 < 0x20 )
        v2[1] = v4 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[((_BYTE)v5 + 1) & 0x1F] - 1);
      v2 += 3;
  ...
  while ( !v7 );
  for ( j = 0; j < 0x20; ++j )
    g_ExtraMasks[j] = `GetBitForBitnum'::`2'::bitsForBitnum[j & 0x1F] - 1;
  return this;
}

//------------------------------------------------------------------------------
// Address: 0x10272310
// Name: public: bf_write::bf_write(void)
// Source: json
//------------------------------------------------------------------------------
bf_write *__thiscall bf_write::bf_write(bf_write *this)
{
  this->m_pData = nullptr;
  this->m_nDataBytes = 0;
  this->m_nDataBits = -1;
  this->m_iCurBit = 0;
  *(_WORD *)&this->m_bOverflow = 256;
...
//------------------------------------------------------------------------------
// Address: 0x10272330
// Name: public: void bf_write::StartWriting(void __near *,int,int,int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::StartWriting(bf_write *this, unsigned __int8 *pData, int nBytes, int iStartBit, int nBits)
{
  this->m_pData = pData;
  this->m_nDataBytes = nBytes & 0xFFFFFFFC;
  this->m_bOverflow = false;
  if ( nBits == -1 )
    this->m_nDataBits = 8 * (nBytes & 0xFFFFFFFC);
  else
    this->m_nDataBits = nBits;
  this->m_iCurBit = iStartBit;
}

//------------------------------------------------------------------------------
// Address: 0x10272370
// Name: public: void bf_write::Reset(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::Reset(bf_write *this)
{
  this->m_iCurBit = 0;
  this->m_bOverflow = false;
}
...
//------------------------------------------------------------------------------
// Address: 0x10272380
// Name: public: void bf_write::WriteUBitVar(unsigned int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteUBitVar(bf_write *this, unsigned int n)
{
  if ( n >= 0x10 )
  {
    if ( n >= 0x100 )
    {
      if ( n >= 0x1000 )
      {
        bf_write::WriteUBitLong(this, curData: n & 0xF | 0x30, numbits: 6, bCheckRange: true);
        bf_write::WriteUBitLong(this, curData: n >> 4, numbits: 28, bCheckRange: true);
      }
      else
      {
        bf_write::WriteUBitLong(this, curData: n & 0xF | (4 * (n & 0xFF0 | 8)), numbits: 14, bCheckRange: true);
      }
    }
    else
    {
      bf_write::WriteUBitLong(this, curData: n & 0xF | (4 * (n & 0xF0 | 4)), numbits: 10, bCheckRange: true);
    }
  }
  else
  {
    bf_write::WriteUBitLong(this, curData: n, numbits: 6, bCheckRange: true);
  }
}

//------------------------------------------------------------------------------
// Address: 0x10272420
// Name: public: bool bf_write::WriteBits(void const __near *,int)
// Source: json
//------------------------------------------------------------------------------
bool __thiscall bf_write::WriteBits(bf_write *this, unsigned int pInData, int nBits)
{
  unsigned int *v3; // ebx
  int v5; // edi
  int m_iCurBit; // eax
  ...
  bool v14; // zf
  int nBitsLeft; // [esp+Ch] [ebp-Ch]
  char iBitsLeft; // [esp+10h] [ebp-8h]
  char iBitsRight; // [esp+14h] [ebp-4h]
  unsigned int iDWord; // [esp+20h] [ebp+8h]
  unsigned int iDWorda; // [esp+20h] [ebp+8h]
  int iBitsChanging; // [esp+24h] [ebp+Ch]

  v3 = (unsigned int *)pInData;
  v5 = nBits;
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    if ( (pInData & 3) != 0 )
    {
      do
      {
        if ( v5 < 8 )
          break;
        bf_write::WriteUBitLong(this, curData: *(unsigned __int8 *)v3, numbits: 8, bCheckRange: false);
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        v9 = this->m_iCurBit & 0x1F;
        iBitsRight = v9;
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_106C7448[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {
          iBitsChanging = ((unsigned int)(v5 - v12) >> 5) + 1;
          nBitsLeft = -32 * iBitsChanging + v5;
          do
          {
            v13 = *v3;
            ++iDWord;
            *(_DWORD *)&this->m_pData[4 * iDWord - 4] = v11 | (*v3 << iBitsRight);
            this->m_iCurBit += 32;
            ++v3;
            v14 = iBitsChanging-- == 1;
            v11 = v13 >> iBitsLeft;
        ...
        *(_DWORD *)&this->m_pData[4 * v10] = v11;
        if ( v5 >= 32 )
        {
          bf_write::WriteUBitLong(this, curData: *v3++, numbits: 32, bCheckRange: false);
          v5 -= 32;
        }
      }
      else
      {
        _V_memcpy(dest: &this->m_pData[m_iCurBit >> 3], src: v3, count: v5 >> 3);
        v8 = 8 * (v5 >> 3);
        v3 = (unsigned int *)((char *)v3 + (v5 >> 3));
        v5 -= v8;
        this->m_iCurBit += v8;
      }
    }
    ...
    if ( v5 != 0 )
      bf_write::WriteUBitLong(this, curData: *(unsigned __int8 *)v3, numbits: v5, bCheckRange: false);
...
//------------------------------------------------------------------------------
// Address: 0x102725A0
// Name: public: bool bf_write::WriteBitsFromBuffer(class bf_read __near *,int)
// Source: json
//------------------------------------------------------------------------------
BOOL __thiscall bf_write::WriteBitsFromBuffer(bf_write *this, bf_read *pIn, int nBits)
{
  int v3; // edi
  bf_write *v4; // ebx
...
LABEL_21:
      m_iCurBit = v4->m_iCurBit;
      m_nDataBits = v4->m_nDataBits;
      if ( m_iCurBit + 32 <= m_nDataBits )
      {
        v19 = v4->m_iCurBit & 0x1F;
        v20 = m_iCurBit >> 5;
        *(_DWORD *)&v4->m_pData[4 * (v4->m_iCurBit >> 5)] = (v8 << (v4->m_iCurBit & 0x1F))
                                                          | *(_DWORD *)&v4->m_pData[4 * (v4->m_iCurBit >> 5)]
                                                          & dword_106C7448[33 * v19];
        v4 = this;
        v21 = 32 - (m_iCurBit & 0x1F);
        if ( v19 > 0 )
          *(_DWORD *)&this->m_pData[4 * v20 + 4] = (v8 >> v21)
                                                 | *(_DWORD *)&this->m_pData[4 * v20 + 4] & dword_106C7448[-v21];
        this->m_iCurBit += 32;
      }
      else
      {
        v4->m_iCurBit = m_nDataBits;
        v4->m_bOverflow = true;
      }
      if ( --v39 == 0 )
      {
  ...
  v22 = pIn->m_nBitsAvail;
  if ( v22 < v3 )
  {
    v30 = pIn->m_pBufferEnd;
    v31 = pIn->m_nInBufWord;
    v32 = nBits - v22;
    v33 = pIn->m_pDataIn;
    if ( v33 == v30 )
    {
...
LABEL_43:
        ...
        {
          v34 = pIn->m_nInBufWord;
          v35 = (v34 & CBitBuffer::s_nMaskTable[v32]) << pIn->m_nBitsAvail;
          pIn->m_nBitsAvail = 32 - v32;
          v36 = v35 | v31;
          v4 = this;
          v37 = v34 >> v32;
          v26 = v36;
          pIn->m_nInBufWord = v37;
        }
        v3 = nBits;
        goto LABEL_47;
      }
      pIn->m_nInBufWord = *v33;
    }
    pIn->m_pDataIn = v33 + 1;
    goto LABEL_43;
  }
  v23 = pIn->m_nInBufWord;
  v24 = v23 & CBitBuffer::s_nMaskTable[v3];
  v25 = v22 - v3;
  pIn->m_nBitsAvail = v25;
  if ( v25 != 0 )
  {
    pIn->m_nInBufWord = v23 >> v3;
    v26 = v24;
  }
  else
  {
    v27 = pIn->m_pDataIn;
    v28 = pIn->m_pBufferEnd;
    pIn->m_nBitsAvail = 32;
    if ( v27 == v28 )
    {
      pIn->m_pDataIn = v27 + 1;
...
LABEL_47:
  bf_write::WriteUBitLong(this: v4, curData: v26, numbits: v3, bCheckRange: true);
  return !v4->m_bOverflow && !pIn->m_bOverflow;
}

//------------------------------------------------------------------------------
// Address: 0x10272860
// Name: public: void bf_write::WriteBitAngle(float,int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitAngle(bf_write *this, float fAngle, int numbits)
{
  unsigned int v3; // edx

  v3 = `GetBitForBitnum'::`2'::bitsForBitnum[numbits & 0x1F];
  bf_write::WriteUBitLong(
    this,
    curData: (v3 - 1) & (__int64)(fAngle * 0.0027777778 * (double)v3),
    numbits,
    bCheckRange: true);
}
...
//------------------------------------------------------------------------------
// Address: 0x102728C0
// Name: public: void bf_write::WriteBitCellCoord(float,int,enum EBitCoordType)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitCellCoord(bf_write *this, float f, int bits, EBitCoordType coordType)
{
  bool v5; // bl
  unsigned int v7; // edi
  bool bIntegral_3; // [esp+13h] [ebp+13h]

  bIntegral_3 = coordType == kCW_Integral;
  v5 = coordType == kCW_LowPrecision;
  if ( coordType == kCW_LowPrecision )
    v7 = abs32((int)(float)(f * 8.0)) & 7;
  else
    v7 = abs32((int)(float)(f * 32.0)) & 0x1F;
  bf_write::WriteUBitLong(
    this,
    curData: (int)COERCE_FLOAT(LODWORD(f) & _mask__AbsFloat_),
    numbits: bits,
    bCheckRange: true);
  if ( !bIntegral_3 )
    bf_write::WriteUBitLong(this, curData: v7, numbits: 2 * !v5 + 3, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x10272970
// Name: public: void bf_write::WriteBitFloat(float)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitFloat(bf_write *this, unsigned int val)
{
  bf_write::WriteUBitLong(this, curData: val, numbits: 32, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x10272990
// Name: public: void bf_write::WriteByte(unsigned int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteByte(bf_write *this, unsigned int val)
{
  bf_write::WriteUBitLong(this, curData: val, numbits: 8, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x102729B0
// Name: public: void bf_write::WriteWord(unsigned int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteWord(bf_write *this, unsigned int val)
{
  bf_write::WriteUBitLong(this, curData: val, numbits: 16, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x102729D0
// Name: public: void bf_write::WriteLongLong(__int64)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteLongLong(bf_write *this, __int64 val)
{
  bf_write::WriteUBitLong(this, curData: val, numbits: 32, bCheckRange: true);
  bf_write::WriteUBitLong(this, curData: HIDWORD(val), numbits: 32, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x10272A00
// Name: public: void bf_write::WriteFloat(float)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteFloat(bf_write *this, float val)
{
  bf_write::WriteBits(this, pInData: (unsigned int)&val, nBits: 32);
}

//------------------------------------------------------------------------------
// Address: 0x10272A20
// Name: public: bool bf_write::WriteBytes(void const __near *,int)
// Source: json
//------------------------------------------------------------------------------
bool __thiscall bf_write::WriteBytes(bf_write *this, const void *pBuf, int nBytes)
{
  return bf_write::WriteBits(this, pInData: (unsigned int)pBuf, nBits: 8 * nBytes);
}

//------------------------------------------------------------------------------
// Address: 0x10272A40
// Name: public: bf_write::bf_write(char const __near *,void __near *,int,int)
// Source: json
//------------------------------------------------------------------------------
bf_write *__thiscall bf_write::bf_write(
        bf_write *this,
...
//------------------------------------------------------------------------------
// Address: 0x10272A90
// Name: public: bf_write::bf_write(void __near *,int,int)
// Source: json
//------------------------------------------------------------------------------
bf_write *__thiscall bf_write::bf_write(bf_write *this, unsigned __int8 *pData, int nBytes, int nBits)
{
  bf_write *result; // eax
...
//------------------------------------------------------------------------------
// Address: 0x10272AE0
// Name: public: void bf_write::WriteSBitLong(int,int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteSBitLong(bf_write *this, int data, int numbits)
{
  int v4; // ecx
  int m_iCurBit; // ecx
  ...
  if ( data >= 0 )
  {
    bf_write::WriteUBitLong(this, curData: data, numbits: numbits - 1, bCheckRange: true);
    m_iCurBit = this->m_iCurBit;
    if ( m_iCurBit + 1 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
    {
      this->m_pData[m_iCurBit >> 3] &= ~(1 << (m_iCurBit & 7));
      ++this->m_iCurBit;
    }
  }
  else
  {
    bf_write::WriteUBitLong(this, curData: data + 0x80000000, numbits: numbits - 1, bCheckRange: false);
    v4 = this->m_iCurBit;
    if ( v4 + 1 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
    {
      this->m_pData[v4 >> 3] |= 1 << (v4 & 7);
      ++this->m_iCurBit;
    }
  }
}
...
//------------------------------------------------------------------------------
// Address: 0x10272B70
// Name: public: void bf_write::WriteBitCoordMP(float,enum EBitCoordType)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitCoordMP(bf_write *this, const float f, EBitCoordType coordType)
{
  bool v3; // bl
  double v5; // xmm1_8
  int v7; // edi
  ...
  unsigned int fractval; // [esp+Ch] [ebp-8h]
  bool bLowPrecision; // [esp+13h] [ebp-1h]
  BOOL signbit; // [esp+1Ch] [ebp+8h]
  bool bInBounds_3; // [esp+23h] [ebp+Fh]
  ...
  if ( coordType == kCW_LowPrecision )
    ...
    v5 = 0.03125;
  signbit = -v5 >= f;
  v7 = (int)COERCE_FLOAT(LODWORD(f) & _mask__AbsFloat_);
  if ( coordType == kCW_LowPrecision )
    v8 = abs32((int)(float)(f * 8.0)) & 7;
  else
    v8 = abs32((int)(float)(f * 32.0)) & 0x1F;
  m_iCurBit = this->m_iCurBit;
  fractval = v8;
  v10 = v7 < 2048;
  bInBounds_3 = v7 < 2048;
  ...
  if ( v3 )
  {
    if ( v12 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
    {
      if ( v7 != 0 )
        this->m_pData[v11 >> 3] |= 1 << (v11 & 7);
      else
        this->m_pData[v11 >> 3] &= ~(1 << (v11 & 7));
      ++this->m_iCurBit;
    }
    if ( v7 != 0 )
    {
      bf_write::WriteOneBit(this, nValue: signbit);
      v13 = v7 - 1;
      if ( bInBounds_3 )
        bf_write::WriteUBitLong(this, curData: v13, numbits: 11, bCheckRange: true);
      else
        bf_write::WriteUBitLong(this, curData: v13, numbits: 14, bCheckRange: true);
    }
  }
  else
  {
    ...
    v16 = this->m_iCurBit;
    if ( v16 + 1 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
    {
      v17 = &this->m_pData[v16 >> 3];
      v18 = 1 << (v16 & 7);
      if ( signbit )
        *v17 |= v18;
      else
    ...
    if ( v7 != 0 )
    {
      v19 = v7 - 1;
      if ( v10 )
        bf_write::WriteUBitLong(this, curData: v19, numbits: 11, bCheckRange: true);
      else
        bf_write::WriteUBitLong(this, curData: v19, numbits: 14, bCheckRange: true);
    }
    bf_write::WriteUBitLong(this, curData: fractval, numbits: 2 * !bLowPrecision + 3, bCheckRange: true);
  }
}

//------------------------------------------------------------------------------
// Address: 0x10272D80
// Name: public: void bf_write::WriteBitCoord(float)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitCoord(bf_write *this, float f)
{
  int m_iCurBit; // ecx
  float v4; // xmm0_4
  unsigned int v5; // edi
  ...
  char v12; // dl
  BOOL signbit; // [esp+0h] [ebp-4h]
  int intval; // [esp+Ch] [ebp+8h]

  signbit = f <= -0.03125;
  m_iCurBit = this->m_iCurBit;
  v4 = f * 32.0;
  intval = (int)COERCE_FLOAT(LODWORD(f) & _mask__AbsFloat_);
  v5 = abs32((int)v4) & 0x1F;
  if ( m_iCurBit + 1 > this->m_nDataBits )
    this->m_bOverflow = true;
  if ( !this->m_bOverflow )
  {
    v6 = &this->m_pData[m_iCurBit >> 3];
    v7 = 1 << (m_iCurBit & 7);
    if ( intval != 0 )
      *v6 |= v7;
    else
  ...
  v8 = this->m_iCurBit;
  if ( v8 + 1 > this->m_nDataBits )
    this->m_bOverflow = true;
  if ( !this->m_bOverflow )
  {
    if ( v5 != 0 )
      this->m_pData[v8 >> 3] |= 1 << (v8 & 7);
    else
      this->m_pData[v8 >> 3] &= ~(1 << (v8 & 7));
    ++this->m_iCurBit;
  }
  ...
  {
    v10 = this->m_iCurBit;
    if ( v10 + 1 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
    {
      v11 = &this->m_pData[v10 >> 3];
      v12 = 1 << (v10 & 7);
      if ( signbit )
        *v11 |= v12;
      else
    ...
    if ( v9 != 0 )
      bf_write::WriteUBitLong(this, curData: v9 - 1, numbits: 14, bCheckRange: true);
    if ( v5 != 0 )
      bf_write::WriteUBitLong(this, curData: v5, numbits: 5, bCheckRange: true);
  }
}

//------------------------------------------------------------------------------
// Address: 0x10272EB0
// Name: public: void bf_write::WriteBitVec3Coord(class Vector const __near &)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitVec3Coord(bf_write *this, const Vector *fa)
{
  const Vector *v2; // ebx
  float y; // xmm0_4
  float z; // xmm0_4
  ...
  unsigned __int8 *v14; // eax
  char v15; // bl
  int f; // [esp+10h] [ebp-8h]
  int v17; // [esp+14h] [ebp-4h]

  v2 = fa;
  if ( fa->x >= 0.03125 || (v17 = 0, fa->x <= -0.03125) )
    v17 = 1;
  y = fa->y;
  if ( y >= 0.03125 || (f = 0, y <= -0.03125) )
    f = 1;
  z = fa->z;
  v6 = z >= 0.03125 || z <= -0.03125;
  m_iCurBit = this->m_iCurBit;
  if ( m_iCurBit + 1 > this->m_nDataBits )
    this->m_bOverflow = true;
  if ( !this->m_bOverflow )
  {
    v8 = &this->m_pData[m_iCurBit >> 3];
    v9 = 1 << (m_iCurBit & 7);
    if ( v17 != 0 )
      *v8 |= v9;
    else
  ...
  v13 = this->m_iCurBit;
  if ( v13 + 1 > this->m_nDataBits )
    this->m_bOverflow = true;
  if ( !this->m_bOverflow )
  {
    v14 = &this->m_pData[v13 >> 3];
    v15 = 1 << (v13 & 7);
    if ( v6 )
      *v14 |= v15;
    else
  ...
  if ( v17 != 0 )
    bf_write::WriteBitCoord(this, f: v2->x);
  if ( f != 0 )
    bf_write::WriteBitCoord(this, f: v2->y);
  if ( v6 )
    bf_write::WriteBitCoord(this, f: v2->z);
}

//------------------------------------------------------------------------------
// Address: 0x10273010
// Name: public: void bf_write::WriteBitNormal(float)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteBitNormal(bf_write *this, float f)
{
  unsigned int v3; // edi
  int m_iCurBit; // ecx
  ...
  if ( !this->m_bOverflow )
  {
    v5 = &this->m_pData[m_iCurBit >> 3];
    v6 = 1 << (m_iCurBit & 7);
    if ( f > -0.0004885197850512946 )
      *v5 &= ~v6;
    else
      *v5 |= v6;
    ++this->m_iCurBit;
  }
  bf_write::WriteUBitLong(this, curData: v3, numbits: 11, bCheckRange: true);
}

//------------------------------------------------------------------------------
// Address: 0x102730A0
// Name: public: void bf_write::WriteChar(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteChar(bf_write *this, int val)
{
  bf_write::WriteSBitLong(this, data: val, numbits: 8);
}

//------------------------------------------------------------------------------
// Address: 0x102730C0
// Name: public: void bf_write::WriteShort(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteShort(bf_write *this, int val)
{
  bf_write::WriteSBitLong(this, data: val, numbits: 16);
}

//------------------------------------------------------------------------------
// Address: 0x102730E0
// Name: public: void bf_write::WriteLong(long)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::WriteLong(bf_write *this, int val)
{
  bf_write::WriteSBitLong(this, data: val, numbits: 32);
}

//------------------------------------------------------------------------------
// Address: 0x10273100
// Name: public: bool bf_write::WriteString(char const __near *)
// Source: json
//------------------------------------------------------------------------------
BOOL __thiscall bf_write::WriteString(bf_write *this, const char *pStr)
{
  const char *v2; // edi
  int m_iCurBit; // ecx
  ...
  {
    do
      bf_write::WriteSBitLong(this, data: *v2++, numbits: 8);
    while ( *(v2 - 1) != 0 );
    return !this->m_bOverflow;
  }
  else
  {
    bf_write::WriteUBitLong(this, curData: 0, numbits: 7, bCheckRange: true);
    m_iCurBit = this->m_iCurBit;
    if ( m_iCurBit + 1 > this->m_nDataBits )
      this->m_bOverflow = true;
    if ( !this->m_bOverflow )
...
CBitWriteMasksInit *__thiscall CBitWriteMasksInit::CBitWriteMasksInit(CBitWriteMasksInit *this)
{
  ...
  {
    v4 = v1 + 1;
    v5 = `GetBitForBitnum'::`2'::bitsForBitnum[v1 & 0x1F] - 1;
    v6 = v1 + 1;
    for ( j = 11; j != 0; --j )
    {
      *(v2 - 1) = v5;
      if ( v6 - 1 < 0x20 )
        *(v2 - 1) = v5 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[((_BYTE)v6 - 1) & 0x1F] - 1);
      *v2 = v5;
      if ( v6 < 0x20 )
        *v2 = v5 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[v6 & 0x1F] - 1);
      v2[1] = v5;
      if ( v6 + 1 < 0x20 )
        v2[1] = v5 | ~(`GetBitForBitnum'::`2'::bitsForBitnum[((_BYTE)v6 + 1) & 0x1F] - 1);
      v2 += 3;
  ...
  for ( k = 0; k < 0x20; ++k )
    g_ExtraMasks[k] = `GetBitForBitnum'::`2'::bitsForBitnum[k & 0x1F] - 1;
  return this;
}
...
bool __thiscall bf_write::WriteBits(bf_write *this, unsigned int pInData, int nBits)
{
  ...
  unsigned int iBitsLeft; // [esp+Ch] [ebp-8h]
  unsigned int iDWord; // [esp+18h] [ebp+4h]
  int nBitsa; // [esp+1Ch] [ebp+8h]
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v4 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        v9 = this->m_iCurBit & 0x1F;
        v10 = m_iCurBit >> 5;
        v11 = dword_100510A0[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v4 >= v12 )
        {
          nBitsa = ((unsigned int)(v4 - v12) >> 5) + 1;
          iBitsLeft = -32 * nBitsa + v4;
          do
          {
            v13 = *v3;
            ++iDWord;
            *(_DWORD *)&this->m_pData[4 * iDWord - 4] = v11 | (*v3 << v9);
            this->m_iCurBit += 32;
            ++v3;
            v14 = nBitsa-- == 1;
            v11 = v13 >> (32 - v9);
      ...
      {
        _V_memcpy(dest: &this->m_pData[m_iCurBit >> 3], src: (unsigned __int8 *)v3, count: v4 >> 3);
        v8 = 8 * (v4 >> 3);
        v3 = (unsigned int *)((char *)v3 + (v4 >> 3));
        v4 -= v8;
        this->m_iCurBit += v8;
      }
    }
    ...
    if ( v4 != 0 )
      bf_write::WriteUBitLong(this, curData: *(unsigned __int8 *)v3, numbits: v4, bCheckRange: false);
    return !this->m_bOverflow;
  }
  else
  {
...
//------------------------------------------------------------------------------
// Address: 0x10018690
// Name: public: void bf_write::SetDebugName(char const __near *)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::SetDebugName(bf_write *this, const char *pDebugName)
{
  this->m_pDebugName = pDebugName;
}
...
//------------------------------------------------------------------------------
// Address: 0x100186A0
// Name: public: void bf_write::SeekToBit(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall bf_write::SeekToBit(bf_write *this, int bitPos)
{
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v4 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        v9 = this->m_iCurBit & 0x1F;
        v10 = m_iCurBit >> 5;
        v11 = dword_1004F848[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v4 >= v12 )
        {
      ...
      {
        _V_memcpy(dest: &this->m_pData[m_iCurBit >> 3], src: v3, count: v4 >> 3);
        v8 = 8 * (v4 >> 3);
        v3 = (unsigned int *)((char *)v3 + (v4 >> 3));
        v4 -= v8;
        this->m_iCurBit += v8;
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        ...
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_421920[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {
...
TrailDown3:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0[ecx*4]; jumptable 0040B1B9 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
...
char *__cdecl asctime(const tm *tb)
{
  ...
  if ( asctimebuf != nullptr
    ...
    && (dword_41FAD8[tm_mon] - _days[tm_mon] >= tm_mday
    ...
    else if ( rterrnum != 252 )
    {
      ...
      if ( GetModuleFileNameW(hModule: nullptr, lpFilename: &outmsg[25], nSize: 0x104u) == 0
        ...
        && (v2 = wcslen(wcs: &outmsg[25]),
            wcsncpy_s(
              _Dst: (unsigned __int16 *)&__argv + v2,
              _SizeInWords: 763 - (((char *)&__argv + 2 * v2 - (char *)&outmsg[25]) >> 1),
              _Src: L"...",
              _Count: 3u) != 0)
...
LABEL_77:
              ...
              {
                v34 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_41F464);
                v34(a1: p_buffer, a2: &_loc_update);
              }
              if ( v81 == 103 && v33 == 0 )
              {
                v35 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_41F460);
...
//------------------------------------------------------------------------------
// Address: 0x00419D80
// Name: _dynamic_initializer_for__g_CurRespondAddr__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
CIPAddr *dynamic_initializer_for__g_CurRespondAddr__()
{
  return CIPAddr::CIPAddr(this: &g_CurRespondAddr);
}

//------------------------------------------------------------------------------
// Address: 0x00419D90
// Name: _dynamic_initializer_for__g_ConsoleOutputMutex__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int dynamic_initializer_for__g_ConsoleOutputMutex__()
{
  CThreadMutex::CThreadMutex(this: &g_ConsoleOutputMutex);
  return atexit(func: dynamic_atexit_destructor_for__g_ConsoleOutputMutex__);
}

//------------------------------------------------------------------------------
// Address: 0x00419DB0
// Name: _dynamic_initializer_for__g_Waiting_Argv__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int dynamic_initializer_for__g_Waiting_Argv__()
{
  return atexit(func: dynamic_atexit_destructor_for__g_Waiting_Argv__);
}

//------------------------------------------------------------------------------
// Address: 0x00419DC0
// Name: _dynamic_initializer_for__g_ServicesBrowsers__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int dynamic_initializer_for__g_ServicesBrowsers__()
{
  return atexit(func: dynamic_atexit_destructor_for__g_ServicesBrowsers__);
}

//------------------------------------------------------------------------------
// Address: 0x00419DD0
// Name: _dynamic_initializer_for__g_JobMemories__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
int dynamic_initializer_for__g_JobMemories__()
{
  return atexit(func: dynamic_atexit_destructor_for__g_JobMemories__);
}
...
//------------------------------------------------------------------------------
// Address: 0x00419DF0
// Name: _dynamic_atexit_destructor_for__g_ConsoleOutputMutex__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
void __cdecl dynamic_atexit_destructor_for__g_ConsoleOutputMutex__()
{
  CThreadMutex::~CThreadMutex(this: &g_ConsoleOutputMutex);
}
...
//------------------------------------------------------------------------------
// Address: 0x00419E00
// Name: _dynamic_atexit_destructor_for__g_Waiting_Argv__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
void __cdecl dynamic_atexit_destructor_for__g_Waiting_Argv__()
{
  CUtlVector<char,CUtlMemory<char,int>>::~CUtlVector<char,CUtlMemory<char,int>>(this: (CUtlVector<char,CUtlMemory<char,int> > *)&g_Waiting_Argv);
}

//------------------------------------------------------------------------------
// Address: 0x00419E10
// Name: _dynamic_atexit_destructor_for__g_ServicesBrowsers__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
void __cdecl dynamic_atexit_destructor_for__g_ServicesBrowsers__()
{
  CUtlVector<char,CUtlMemory<char,int>>::~CUtlVector<char,CUtlMemory<char,int>>(this: (CUtlVector<char,CUtlMemory<char,int> > *)&g_ServicesBrowsers);
}

//------------------------------------------------------------------------------
// Address: 0x00419E20
// Name: _dynamic_atexit_destructor_for__g_JobMemories__
// Source: linker_block_proximity
//------------------------------------------------------------------------------
void __cdecl dynamic_atexit_destructor_for__g_JobMemories__()
{
  CUtlLinkedList<CJobMemory,int,0,int,CUtlMemory<UtlLinkedListElem_t<CJobMemory,int>,int>>::RemoveAll(this: &g_JobMemories);
  if ( g_JobMemories.m_Memory.m_nGrowSize >= 0 )
  {
    if ( g_JobMemories.m_Memory.m_pMemory != nullptr )
    {
      _g_pMemAlloc->Free_2(this: _g_pMemAlloc, a2: g_JobMemories.m_Memory.m_pMemory);
      g_JobMemories.m_Memory.m_pMemory = nullptr;
    }
    g_JobMemories.m_Memory.m_nAllocationCount = 0;
  }
}
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        ...
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_106EE148[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {
      ...
      {
        _V_memcpy(dest: &this->m_pData[m_iCurBit >> 3], src: (unsigned __int8 *)v3, count: v5 >> 3);
        v8 = 8 * (v5 >> 3);
        v3 = (unsigned int *)((char *)v3 + (v5 >> 3));
        v5 -= v8;
        this->m_iCurBit += v8;
...
void __thiscall bf_write::WriteBitVec3Coord(bf_write *this, const Vector *fa)
{
  ...
  unsigned __int8 *v14; // eax
  char v15; // bl
  int yflag; // [esp+10h] [ebp-8h]
  int xflag; // [esp+14h] [ebp-4h]

  v2 = fa;
  if ( fa->x >= 0.03125 || (xflag = 0, fa->x <= -0.03125) )
    xflag = 1;
  y = fa->y;
  if ( y >= 0.03125 || (yflag = 0, y <= -0.03125) )
    yflag = 1;
  z = fa->z;
  v6 = z >= 0.03125 || z <= -0.03125;
  m_iCurBit = this->m_iCurBit;
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        ...
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_106D9790[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        ...
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_10096B00[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {
  ...
  if ( nBits + this->m_iCurBit <= this->m_nDataBits )
  {
    ...
    if ( v5 >= 32 )
    {
      ...
      if ( (m_iCurBit & 7) != 0 )
      {
        ...
        iBitsLeft = 32 - v9;
        v10 = m_iCurBit >> 5;
        v11 = dword_10058B48[33 * v9] & *(_DWORD *)&this->m_pData[4 * v10];
        v12 = 32 - v9 + 32;
        iDWord = v10;
        if ( v5 >= v12 )
        {

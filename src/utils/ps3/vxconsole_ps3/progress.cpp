...
// Original: utils/ps3/vxconsole_ps3/progress.cpp
// Functions: 9
// ============================================================

#include "utils\ps3\vxconsole_ps3\progress.h"
...
// Source: json
//------------------------------------------------------------------------------
void __thiscall CProgress::SetMeter(CProgress *this, WPARAM currentPos, LPARAM range)
{
  ...
  if ( this->m_hWnd != nullptr && this->m_hWndPercent != nullptr )
  {
    ...
    if ( m_hWndMeter != nullptr )
    {
      if ( range >= 0 )
      {
        SendMessageA(hWnd: m_hWndMeter, Msg: 0x406u, wParam: 0, lParam: range);
        this->m_range = range;
      }
...
void __thiscall CProgress::Open(CProgress *this, const char *title, bool canCancel, bool bHasMeter)
{
  ...
  if ( Window != nullptr )
  {
    ...
    if ( bHasMeter )
    {
      ...
      this->m_bMetersHidden = false;
    }
    else
    {
      this->m_hWndPercent = nullptr;
      this->m_hWndMeter = nullptr;
      this->m_bMetersHidden = true;
    }
...
//------------------------------------------------------------------------------
// Address: 0x00409D20
// Name: public: void CProgress::HideMeter(bool)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CProgress::HideMeter(CProgress *this, bool bHide)
{
  HWND__ *m_hWndPercent; // eax
  HWND__ *m_hWndMeter; // eax

  if ( bHide != this->m_bMetersHidden )
  {
    m_hWndPercent = this->m_hWndPercent;
    if ( m_hWndPercent != nullptr )
      ShowWindow(hWnd: m_hWndPercent, nCmdShow: bHide ? 0 : 8);
    m_hWndMeter = this->m_hWndMeter;
    if ( m_hWndMeter != nullptr )
      ShowWindow(hWnd: m_hWndMeter, nCmdShow: bHide ? 0 : 8);
    this->m_bMetersHidden = bHide;
  }
}

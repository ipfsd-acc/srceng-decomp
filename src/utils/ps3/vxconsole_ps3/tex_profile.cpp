...
// Original: utils/ps3/vxconsole_ps3/tex_profile.cpp
// Functions: 14
// ============================================================
...
void __cdecl TexProfile_LoadConfig()
{
  ...
  Sys_GetRegistryString(keyName: "texProfileSamplesWindowRect", value: buff, defValue: &str, valueLen: 0x100u);
  if ( sscanf(
         string: buff,
         format: "%d %d %d %d",
  ...
  Sys_GetRegistryString(keyName: "texProfileHistoryWindowRect", value: buff, defValue: &str, valueLen: 0x100u);
  if ( sscanf(
         string: buff,
         format: "%d %d %d %d",
...
int __cdecl rc_SetTexProfileData(char *commandPtr)
{
  ...
  unsigned int *v8; // eax
  unsigned int v9; // edx
  unsigned int localCounters[64]; // [esp+4h] [ebp-10Ch] BYREF
  unsigned __int64 counters; // [esp+104h] [ebp-Ch] BYREF
  bool newPeaks; // [esp+10Fh] [ebp-1h]

  Token = GetToken(ppTokenStream: &commandPtr);
  if ( *Token == 0 )
    return -1;
  sscanf(string: Token, format: "%lx", &counters);
  if ( g_texProfile_numCounters != 0 )
    CPS3TargetConnection::DmGetMemory(
      this: &g_PS3CurrentTargetConn,
      pAddressOnConsoleToReadFrom: counters,
      nBytesToRead: 4 * g_texProfile_numCounters - 4,
      pLocalDestinationBuffer: localCounters);
  ...
  if ( g_texProfile_numCounters > 0 )
  {
    v7 = (unsigned int *)&unk_470008;
    v8 = &g_texProfile_counters[0].samples[v3];
    do
    {
      if ( v6 == v4 - 1 )
      {
...
void __cdecl TexProfileSamples_Draw(HDC__ *hdc, tagRECT *clientRect)
{
  ...
  HPEN__ *hGreyPen; // [esp+C0h] [ebp-34h]
  int windowWidth; // [esp+C4h] [ebp-30h]
  int tickWidth; // [esp+C8h] [ebp-2Ch]
  float sample; // [esp+CCh] [ebp-28h]
  int v31; // [esp+D0h] [ebp-24h]
  HPEN__ *hBlackPen; // [esp+D4h] [ebp-20h]
  tagRECT rect; // [esp+D8h] [ebp-1Ch] BYREF
  int windowHeight; // [esp+E8h] [ebp-Ch]
  int y; // [esp+ECh] [ebp-8h]
...
void __cdecl TexProfileHistory_Draw(HDC__ *hdc, tagRECT *clientRect)
{
  ...
  HPEN__ *hPenOld; // [esp+A0h] [ebp-34h]
  HFONT__ *hFontOld; // [esp+A4h] [ebp-30h]
  int h; // [esp+A8h] [ebp-2Ch]
  HPEN__ *hBlackPen; // [esp+ACh] [ebp-28h]
  int windowHeight; // [esp+B0h] [ebp-24h]
  HBRUSH__ *hBrushOld; // [esp+B4h] [ebp-20h]
  HPEN__ *hGreyPen; // [esp+B8h] [ebp-1Ch]
  int tickHeight; // [esp+BCh] [ebp-18h]
  int j; // [esp+C0h] [ebp-14h]
  int v25; // [esp+C4h] [ebp-10h]
  int x; // [esp+C8h] [ebp-Ch]
  int i; // [esp+CCh] [ebp-8h]
  int numTicks; // [esp+D0h] [ebp-4h]
  ...
  v4 = clientRect->right - clientRect->left;
  tickHeight = (int)(float)((float)((float)((float)((float)(g_texProfile_history_scale + 5) * 0.1) * 2.7) + 0.30000001)
                          * 100.0);
  windowHeight = v3;
  numTicks = v3 / tickHeight + 2;
  if ( numTicks < 0 )
    numTicks = 1;
  SetBkColor(hdc, color: g_backgroundColor);
  y = v3;
  ...
  if ( g_texProfile_numSamples != 0 )
  {
    ...
    if ( v4 - 51 >= 0 )
    {
      ...
      do
      {
        ...
        if ( v7 - 1 > 0 )
        {
          v8 = (COLORREF *)&dword_47004C;
          numTicks = 0;
          v25 = (int)&dword_47004C;
          do
          {
            if ( v8 != (COLORREF *)64 )
            {
              v9 = (int)((double)g_texProfile_counters[numTicks + i % 512] * 0.00000023841858 * (double)tickHeight);
              h = v9;
              if ( v9 != 0 )
              {
                if ( v9 > windowHeight )
                ...
                if ( g_texProfile_history_colors )
                  ...
                  v10 = 5263440;
                SolidBrush = CreateSolidBrush(color: v10);
                v12 = (HBRUSH__ *)SelectObject(hdc, h: SolidBrush);
                v13 = ya - h;
                hBrushOld = v12;
...
LABEL_6:
  ...
  if ( v6 )
  {
    TexProfileSamples_Draw(hdc: v8, clientRect: &rect);
  ...
  {
    if ( bIsHistory_3 )
      TexProfileHistory_Draw(hdc: v8, clientRect: &rect);
    EndPaint(hWnd: hwnd, lpPaint: &ps);
    return 0;
  }
}
...
int __cdecl rc_SetTexProfile(char *commandPtr)
{
  ...
  int v9; // eax
  int v10; // edi
  unsigned __int64 retAddr; // [esp+4h] [ebp-24h] BYREF
  unsigned __int64 profileList; // [esp+Ch] [ebp-1Ch] BYREF
  unsigned __int64 numProfiles; // [esp+14h] [ebp-14h] BYREF
  int retVal; // [esp+1Ch] [ebp-Ch]
  unsigned int v16; // [esp+20h] [ebp-8h]
  int xboxRetVal; // [esp+24h] [ebp-4h] BYREF

  Token = GetToken(ppTokenStream: &commandPtr);
  if ( *Token == 0 )
    return -1;
  sscanf(string: Token, format: "%lx", &numProfiles);
  v2 = GetToken(ppTokenStream: &commandPtr);
  if ( *v2 == 0 )
    return -1;
  sscanf(string: v2, format: "%lx", &profileList);
  v3 = GetToken(ppTokenStream: &commandPtr);
  if ( *v3 == 0 )
    return -1;
  sscanf(string: v3, format: "%lx", &retAddr);
  v4 = (unsigned __int8 *)operator new(nSize: 68 * numProfiles);
  retVal = (int)v4;
  memset(dst: v4, value: 0, count: 68 * numProfiles);
  CPS3TargetConnection::DmGetMemory(
    this: &g_PS3CurrentTargetConn,
    pAddressOnConsoleToReadFrom: profileList,
    nBytesToRead: 68 * numProfiles,
    pLocalDestinationBuffer: v4);
  g_texProfile_numCounters = numProfiles;
  if ( (int)numProfiles > 63 )
  ...
  if ( g_texProfile_numCounters > 0 )
  {
    ...
    {
      v16 = *v7;
      *v7 = _byteswap_ulong(v16);
      memset(dst: (unsigned __int8 *)v6 - 2052, value: 0, count: 0x848u);
      V_strncpy(pDest: v6, pSrc: (const char *)v7 - 64, maxLen: 64);
      *((_DWORD *)v6 + 16) = *v7;
      ++v5;
      v7 += 17;
      v6 += 2120;
  ...
  v8 = 1060 * g_texProfile_numCounters;
  memset(dst: (unsigned __int8 *)&g_texProfile_counters[530 * g_texProfile_numCounters], value: 0, count: 0x848u);
  v9 = g_texProfile_numCounters;
  *(_DWORD *)&dword_47000C[v8 * 2] = *(_DWORD *)aTota;
  ++v9;
  word_470010[v8] = 108;
  v10 = v9 - 1;
  dword_47004C[530 * v5] = 0xFFFFFF;
  g_texProfile_numCounters = v9;
  retVal = v9 - 1;
  xboxRetVal = _byteswap_ulong(v9 - 1);
  ...
  DebugCommand(
    pStrFormat: "0x%8.8x = SetTexProfile( 0x%8.8x, 0x%8.8x )\n",
    v10,
    (_DWORD)numProfiles,
    HIDWORD(numProfiles));
  free(pMem: v4);

...
// Original: utils/ps3/vxconsole_ps3/show_memdump.cpp
// Functions: 17
// ============================================================
...
void __cdecl ShowMemDump_LoadConfig()
{
  ...
  Sys_GetRegistryString(keyName: "showMemDumpWindowRect", value: buff, defValue: &str, valueLen: 256);
  if ( sscanf(
         string: buff,
         format: "%d %d %d %d",
...
void __cdecl ShowMemDump_Summary()
{
  ...
  int time; // [esp+450h] [ebp-34h]
  int totalCount; // [esp+454h] [ebp-30h]
  int lte128; // [esp+458h] [ebp-2Ch]
  int lte16; // [esp+45Ch] [ebp-28h]
  int currentCount; // [esp+460h] [ebp-24h]
  int lte32; // [esp+464h] [ebp-20h]
  int peakOverheadSize; // [esp+468h] [ebp-1Ch]
  int peakCount; // [esp+46Ch] [ebp-18h]
  int allocSize; // [esp+470h] [ebp-14h]
  int i; // [esp+474h] [ebp-10h]
  int committedSize; // [esp+478h] [ebp-Ch]
  int gt1024; // [esp+47Ch] [ebp-8h]
  int lte1024; // [esp+480h] [ebp-4h]

  v0 = g_showMemDump_numMemory;
  v1 = 0;
  v2 = 0;
  v3 = 0;
  if ( g_showMemDump_format == 1 )
  {
    ...
    {
      v9 = g_showMemDump_pMemory;
      do
      {
        v10 = V_strncasecmp(s1: *(const char **)((char *)&v9->detail.pAllocationName + v1), s2: "Totals,", n: 7) == 0;
        v9 = g_showMemDump_pMemory;
        if ( !v10 )
        {
          committedSize += *(_DWORD *)&g_showMemDump_pMemory->detail.peakSizeBuff[v1 + 32];
          i += *(_DWORD *)&g_showMemDump_pMemory->detail.totalSizeBuff[v1 + 32];
          peakOverheadSize += *(_DWORD *)&g_showMemDump_pMemory->detail.overheadSizeBuff[v1 + 32];
          time += *(_DWORD *)&g_showMemDump_pMemory->detail.peakOverheadSizeBuff[v1 + 32];
          currentCount += *(_DWORD *)&g_showMemDump_pMemory->detail.timeBuff[v1 + 32];
          peakCount += *(_DWORD *)&g_showMemDump_pMemory->detail.currentCountBuff[v1 + 32];
          totalCount += *(_DWORD *)&g_showMemDump_pMemory->detail.peakCountBuff[v1 + 32];
          lte16 += *(_DWORD *)&g_showMemDump_pMemory->detail.totalCountBuff[v1 + 32];
          v11 = *(_DWORD *)&g_showMemDump_pMemory->detail.lte32Buff[v1 + 32];
          lte32 += *(_DWORD *)&g_showMemDump_pMemory->detail.lte16Buff[v1 + 32];
          lte128 += v11;
          v12 = *(_DWORD *)&g_showMemDump_pMemory->detail.lte1024Buff[v1 + 32];
          v2 += *(int *)((char *)&g_showMemDump_pMemory->detail.currentSize + v1);
          v3 += *(_DWORD *)&g_showMemDump_pMemory->detail.currentSizeBuff[v1 + 32];
          lte1024 += *(_DWORD *)&g_showMemDump_pMemory->detail.lte128Buff[v1 + 32];
          gt1024 += v12;
        }
        v0 = g_showMemDump_numMemory;
        v1 += 728;
      ...
      while ( allocSize < g_showMemDump_numMemory );
    }
    sprintf(
      string: buff,
      format: "Entries:\t\t\t%d\n"
      ...
      (float)((float)v3 * 0.00000095367432),
  ...
  else
  {
    ...
    {
      p_allocated = &g_showMemDump_pMemory[1].pool.allocated;
      v7 = ((unsigned int)(g_showMemDump_numMemory - 2) >> 1) + 1;
      i = 2 * v7;
      do
      {
        gt1024 += *(p_allocated - 182) * *(p_allocated - 191);
        v5 += *(p_allocated - 155);
        v1 += p_allocated[27];
        lte1024 += *p_allocated * *(p_allocated - 9);
        p_allocated += 364;
        --v7;
      }
      while ( v7 != 0 );
    ...
    if ( v4 < v0 )
    {
      v8 = &g_showMemDump_pMemory[v4];
      v3 = v8->pool.committedSize;
      v2 = v8->pool.size * v8->pool.allocated;
    }
    sprintf(
      string: buff,
      format: "Alloc Size:\t\t\t%.2f MB\nTotal Memory:\t\t%.2f MB\n",
      (float)((float)(v2 + gt1024 + lte1024) * 0.00000095367432),
      (float)((float)(v3 + v5 + v1) * 0.00000095367432));
    MessageBoxA(hWnd: g_showMemDump_hWnd, lpText: buff, lpCaption: "Memory Dump Summary", uType: 0);
  }
}
...
{
  MessageBoxA(hWnd: g_hDlgMain, lpText: "ShowMemDump_RefreshView is not yet implemented", lpCaption: "Oops!", uType: 0);
}
...
// Source: json
//------------------------------------------------------------------------------
int __cdecl rc_MemDump()
{
  MessageBoxA(hWnd: g_hDlgMain, lpText: "rc_MemDump is not yet implemented", lpCaption: "Oops!", uType: 0);
  return -1;
}

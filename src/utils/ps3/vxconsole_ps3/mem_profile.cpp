...
// Original: utils/ps3/vxconsole_ps3/mem_profile.cpp
// Functions: 13
// ============================================================
...
void __cdecl MemProfile_LoadConfig()
{
  ...
  Sys_GetRegistryString(keyName: "MemProfileWindowRect", value: buff, defValue: &str, valueLen: 256);
  if ( sscanf(
         string: buff,
         format: "%d %d %d %d",
...
void __cdecl MemProfile_Draw(HDC__ *hdc, tagRECT *clientRect)
{
  ...
  char labelBuff[128]; // [esp+10h] [ebp-BCh] BYREF
  tagRECT rect; // [esp+90h] [ebp-3Ch] BYREF
  HFONT__ *hFontOld; // [esp+A0h] [ebp-2Ch]
  HPEN__ *hPenOld; // [esp+A4h] [ebp-28h]
  int gapY; // [esp+A8h] [ebp-24h]
  HPEN__ *hGreyPen; // [esp+ACh] [ebp-20h]
  HPEN__ *hBlackPen; // [esp+B0h] [ebp-1Ch]
  int windowHeight; // [esp+B4h] [ebp-18h]
  int memoryHeight; // [esp+B8h] [ebp-14h]
  HPEN__ *hNullPen; // [esp+BCh] [ebp-10h]
  int numTicks; // [esp+C0h] [ebp-Ch]
  int h; // [esp+C4h] [ebp-8h]
  int x; // [esp+C8h] [ebp-4h]
  int y; // [esp+D8h] [ebp+Ch]
  int ya; // [esp+D8h] [ebp+Ch]
...
// Source: json
//------------------------------------------------------------------------------
int __stdcall MemProfile_WndProc(HWND__ *hwnd, UINT message, HMENU wParam, LONG *lParam)
{
  int result; // eax
  HDC v5; // eax
    ...
    if ( message != 256 )
    {
      switch ( message )
      {
        ...
        case 0xFu:
          ...
          MemProfile_Draw(hdc: v5, clientRect: &rect);

...
// Original: utils/scenemanager/snd_wave_mixer_adpcm.cpp
// Functions: 8
// ============================================================

#include "utils\scenemanager\snd_wave_mixer_adpcm.h"
...
CAudioMixerWaveADPCM *__cdecl CreateADPCMMixer(CWaveData *data)
{
  ...
  v1 = (CAudioMixerWaveADPCM *)operator new(nSize: 0x4Cu);
  if ( v1 != nullptr )
        ...
        bool forward)
{
  ...
  if ( samplePosition / this->m_pFormat->wSamplesPerBlock != this->m_currentBlock
    && !this->SetSamplePosition(this, a2: samplePosition) )
  {
    return 0;
  }
...
//------------------------------------------------------------------------------
// Address: 0x004034D0
// Name: public: virtual bool CAudioMixerWaveADPCM::SetSamplePosition(int)
// Source: json
//------------------------------------------------------------------------------
char __thiscall CAudioMixerWaveADPCM::SetSamplePosition(CAudioMixerWaveADPCM *this, int position)
{
  int v3; // edi
  int v4; // ebx
  CAudioSourceWave *v5; // eax
  int v7; // edi
  int firstSample; // [esp+14h] [ebp+8h]

  v3 = position < 0 ? 0 : position;
  CAudioMixerWave::SetSamplePosition(this, position: v3);
  v4 = v3 / this->m_pFormat->wSamplesPerBlock;
  firstSample = v4 * this->m_pFormat->wSamplesPerBlock;
  v5 = this->m_pData->Source(this: this->m_pData);
  if ( firstSample >= v5->SampleCount(this: v5) )
    return 0;
  v7 = v3 - firstSample;
  if ( v4 != this->m_currentBlock )
  {
    this->m_offset = v4 * this->m_blockSize;
    if ( CAudioMixerWaveADPCM::DecodeBlock(this) == 0 )
      return 0;
  }
  this->m_samplePosition = v7;
  return 1;
}

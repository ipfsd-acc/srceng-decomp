...
// Original: utils/vmap/materialpatch.cpp
// Functions: 11
// ============================================================
...
void __usercall InsertKeyValues(KeyValues *dst@<edi>, KeyValues *src@<ecx>, bool bCheckForExistence)
{
  ...
  char *v7; // eax
  char *v8; // eax
  char *defaultValue; // [esp+0h] [ebp-Ch]
...
// Source: json
//------------------------------------------------------------------------------
char __cdecl LoadKeyValuesFromPackOrFile(char *pFileName, KeyValues *pKeyValues)
{
  IZip *PakFile; // eax
  char v3; // al
  char v4; // bl
...
void __cdecl ExpandPatchFile(KeyValues *keyValues)
{
  ...
  while ( 1 )
  {
    ...
    if ( _V_stricmp(s1: Name, s2: "patch") != 0 )
      break;
    String = KeyValues::GetString(this: keyValues, keyName: "include", defaultValue: var);
    if ( String == nullptr )
      return;
...
KeyValues *__cdecl LoadMaterialKeyValues(const char *pMaterialName, char nFlags)
{
  ...
  if ( LoadKeyValuesFromPackOrFile(pFileName: pFullMaterialName, pKeyValues: v3) != 0 )
  {
    if ( (nFlags & 1) != 0 )
...
// Source: json
//------------------------------------------------------------------------------
bool __cdecl GetValueFromPatchedMaterial(const char *pMaterialName, char *pKey, char *pValue, int len)
{
  KeyValues *v4; // eax
  KeyValues *v5; // esi
  char *String; // eax
  ...
  if ( v4 != nullptr )
    ...
    v5 = nullptr;
  V_snprintf(pDest: pFullMaterialName, maxLen: 512, pFormat: "materials/%s.vmt", pMaterialName);
  if ( LoadKeyValuesFromPackOrFile(pFileName: pFullMaterialName, pKeyValues: v5) != 0 )
  {
...
// Source: json
//------------------------------------------------------------------------------
const char *__cdecl GetOriginalMaterialNameForPatchedMaterial(const char *pPatchMaterialName)
{
  const char *v1; // esi
  int v2; // eax
...
// Source: json
//------------------------------------------------------------------------------
void *__cdecl FindOriginalMaterial(const char *materialName, bool *pFound, bool bComplain)
{
...
// Source: json
//------------------------------------------------------------------------------
void __cdecl AddNewTranslation(const char *pOriginalMaterialName, const char *pNewMaterialName)
{
  int v2; // eax
  int v3; // esi

...
face_t *__cdecl AllocFace()
{
  ...
  v0 = (unsigned __int8 *)MemAlloc_Alloc(nSize: 0xCCu);
  memset(dst: v0, value: 0, count: 0xCCu);
  v1 = s_FaceId;
  ++c_faces;
  *(_DWORD *)v0 = s_FaceId;
  s_FaceId = v1 + 1;
...
face_t *__cdecl NewFaceFromFace(face_t *f)
{
  ...
  v1 = (unsigned __int8 *)MemAlloc_Alloc(nSize: 0xCCu);
  memset(dst: v1, value: 0, count: 0xCCu);
  v2 = s_FaceId;
  *(_DWORD *)v1 = s_FaceId;
  ++c_faces;
  qmemcpy(v1, f, 0xCCu);
...
void __cdecl FaceFromSuperverts(face_t **pListHead, face_t *f, int base)
{
  ...
  if ( numsuperverts > 32 )
  {
    ...
    {
      ++c_faceoverflows;
      v4 = (unsigned __int8 *)MemAlloc_Alloc(nSize: 0xCCu);
      memset(dst: v4, value: 0, count: 0xCCu);
      v5 = s_FaceId;
      *(_DWORD *)v4 = s_FaceId;
      ++c_faces;
      s_FaceId = v5 + 1;
      ...
      *((_DWORD *)v4 + 13) = 32;
      ...
      while ( v6 < 32 );
      v9 = (unsigned __int8 *)MemAlloc_Alloc(nSize: 0xCCu);
      memset(dst: v9, value: 0, count: 0xCCu);
      v10 = s_FaceId;
      *(_DWORD *)v9 = s_FaceId;
      ++c_faces;
      s_FaceId = v10 + 1;
...
face_t *__cdecl FaceFromPortal(portal_t *p, int pside)
{
  ...
  if ( side == nullptr )
    return nullptr;
  v4 = MemAlloc_Alloc(nSize: 0xCCu);
  memset(dst: (unsigned __int8 *)v4, value: 0, count: 0xCCu);
  v5 = s_FaceId;
  *(_DWORD *)v4 = s_FaceId;
  ++c_faces;
  *((_DWORD *)v4 + 47) = side;
...
face_t *__cdecl TryMerge(face_t *f1, face_t *f2, Vector *planenormal)
{
  ...
  v4 = TryMergeWinding(f1: f1->w, f2: w, planenormal);
  if ( v4 == nullptr )
    return nullptr;
  ++c_merge;
  result = NewFaceFromFace(f: f1);
  result->w = v4;
  f1->merged = result;
  f2->merged = result;
  return result;
    ...
    if ( i->merged == nullptr && i->split[0] == nullptr && i->split[1] == nullptr )
    {
      ...
      if ( *pList != i )
      {
        while ( 1 )
        {
          ...
          {
            v3 = TryMerge(f1: i, f2: v2, planenormal: &mapplanes + 2 * i->planenum);
            if ( v3 != nullptr )
              break;
          }
          v2 = v2->next;
  ...
  if ( m_Size == 3 )
  {
    ...
    do
    {
      ...
      if ( v6 + 1 > m_nAllocationCount )
        CUtlMemory<HemiLightData_t *,int>::Grow(this: (CUtlMemory<S3RGBA,int> *)out, num: v6 - m_nAllocationCount + 1);
      ++out->m_Size;
...
LABEL_21:
      ...
      CUtlMemory<HemiLightData_t *,int>::Grow(this: (CUtlMemory<S3RGBA,int> *)&in2, num: 1);
...
int __cdecl GetEdge2(int v1, int v2, face_t *f)
{
  ...
  ++c_tryedges;
  if ( noshare != 0 || (v3 = 0, (v29 = dword_1EDD28C[5 * v1]) <= 0) )
  {
LABEL_9:
    if ( numedges >= 256000 )
      _Error(a1: "Too many edges in map, max == %d", 256000);
    v6 = dword_1EDD28C[5 * v1];
    v7 = *(&g_VertEdgeList.m_nAllocationCount + 5 * v1);
    v8 = (CUtlMemory<S3RGBA,int> *)((char *)&g_VertEdgeList + 20 * v1);
    if ( v6 + 1 > v7 )
      CUtlMemory<HemiLightData_t *,int>::Grow(this: v8, num: v6 - v7 + 1);
    ++v8[1].m_pMemory;
  ...
  else
  {
    ...
    {
      v5 = *v4;
      if ( v1 == (unsigned __int16)word_374A4EA[2 * *(_DWORD *)v4]
        && v2 == dedges[*(_DWORD *)&v5].v[0]
        && edgefaces[*(_DWORD *)&v5][0]->contents == f->contents
        && dword_1C68A34[2 * *(_DWORD *)&v5] == 0 )
      {
        break;
      }
      ++v3;
    ...
    dword_1C68A34[2 * *(_DWORD *)&v5] = (int)f;
    return -*(int *)&v5;
  }
}

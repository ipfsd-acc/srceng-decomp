...
// Original: utils/vbsp/portals.cpp
// Functions: 39
// ============================================================

#include "utils\vbsp\portals.h"
...
winding_t *__cdecl BaseWindingForNode(winding_t *node)
{
  ...
  v1 = node;
  result = BaseWindingForPlane(
             normal: &g_MainMap->mapplanes[(int)node->p].normal,
             dist: g_MainMap->mapplanes[(int)node->p].dist);
  maxpoints = (winding_t *)v1->maxpoints;
  for ( node = result; maxpoints != nullptr; maxpoints = (winding_t *)maxpoints->maxpoints )
  {
    if ( result == nullptr )
      break;
    p_normal = &g_MainMap->mapplanes[(int)maxpoints->p].normal;
    if ( maxpoints[2].next == v1 )
    {
      ChopWindingInPlace(inout: &node, normal: p_normal, dist: p_normal[1].x, epsilon: 0.001);
    }
    else
    {
      normal.x = vec3_origin.x - p_normal->x;
      normal.y = vec3_origin.y - p_normal->y;
      normal.z = vec3_origin.z - p_normal->z;
      ChopWindingInPlace(
        inout: &node,
        &normal,
        dist: COERCE_FLOAT(LODWORD(p_normal[1].x) ^ _mask__NegFloat_),
        epsilon: 0.001);
    }
    result = node;
    v1 = maxpoints;
...
LABEL_9:
    if ( v3 != nullptr )
    {
      ...
      else
      {
        ...
        v7 = dword_4AFD5D4;
        *(_DWORD *)v6 = dword_4AFD5D4;
        dword_4AFD5D4 = v7 + 1;
        v8 = &g_MainMap->mapplanes[(int)v1->p];
        *((float *)v6 + 1) = v8->normal.x;
        *((float *)v6 + 2) = v8->normal.y;
        *((float *)v6 + 3) = v8->normal.z;
        *((float *)v6 + 4) = v8->dist;
        *((_DWORD *)v6 + 5) = v8->type;
        *((_DWORD *)v6 + 6) = v8->hash_chain;
        v9 = *((_DWORD *)v6 + 8) == 0;
        *((_DWORD *)v6 + 7) = v1;
        *((_DWORD *)v6 + 12) = node;
...
void __cdecl MakeHeadnodePortals(tree_t *tree)
{
  ...
  int v14; // esi
  plane_t *v15; // edi
  portal_t *portals[6]; // [esp+14h] [ebp-CCh]
  plane_t bplanes[6]; // [esp+2Ch] [ebp-B4h] BYREF
  Vector bounds[2]; // [esp+BCh] [ebp-24h]
  node_t *node; // [esp+D4h] [ebp-Ch]
  node_t *p_outside_node; // [esp+D8h] [ebp-8h]
  int i; // [esp+DCh] [ebp-4h]
  tree_t *treea; // [esp+E8h] [ebp+8h]
  ...
  while ( 1 )
  {
    ...
    *(_DWORD *)v6 = dword_4AFD5D4;
    dword_4AFD5D4 = v7 + 1;
    v8 = &bplanes[v5];
    *(_QWORD *)&v8->normal.x = 0;
    *(_QWORD *)&v8->normal.z = 0;
    portals[v5] = (portal_t *)v6;
    *(_QWORD *)&v8->type = 0;
    if ( v3 != 0 )
    {
      *(float *)((char *)&v8->normal.x + (_DWORD)treea) = -1.0;
      LODWORD(v8->dist) = *((_DWORD *)&bounds[0].x + v5) ^ _mask__NegFloat_;
    }
    else
    {
      v9 = *(float *)((char *)&bounds[0].x + (_DWORD)treea);
      *(float *)((char *)&v8->normal.x + (_DWORD)treea) = 1.0;
      v8->dist = v9;
    }
    *((float *)v6 + 1) = v8->normal.x;
    ...
    *((_DWORD *)v6 + 5) = v8->type;
    *((_DWORD *)v6 + 6) = v8->hash_chain;
    v10 = BaseWindingForPlane(normal: &bplanes[v5].normal, dist: v8->dist);
    v11 = *((_DWORD *)v6 + 8) == 0;
    *((_DWORD *)v6 + 12) = v10;
    if ( !v11 || *((_DWORD *)v6 + 9) != 0 )
      _Error(a1: "AddPortalToNode: allready included");
    v12 = node;
    *((_DWORD *)v6 + 8) = node;
    *((_DWORD *)v6 + 10) = v12->portals;
    v12->portals = (portal_t *)v6;
    v13 = p_outside_node;
    *((_DWORD *)v6 + 9) = p_outside_node;
    ++v3;
    *((_DWORD *)v6 + 11) = v13->portals;
    v13->portals = (portal_t *)v6;
    if ( v3 >= 2 )
    {
      ++i;
      v3 = 0;
      treea = (tree_t *)((char *)treea + 4);
      if ( (int)treea >= 12 )
        break;
    }
  }
  do
  {
    ...
    {
      if ( v14 != v3 )
        ChopWindingInPlace(inout: &portals[v3]->winding, normal: &v15->normal, dist: v15->dist, epsilon: 0.1);
      ++v14;
      ++v15;
    }
    while ( v14 < 6 );
...
void __cdecl SplitNodePortals(node_t *node)
{
  ...
  v1 = node;
  portals = node->portals;
  v3 = &g_MainMap->mapplanes[node->planenum];
  v4 = node->children[1];
  v5 = node->children[0];
  side = 0;
  plane = v3;
        ...
        CUtlVector<portal_t *,CUtlMemory<portal_t *,int> > *portals)
{
  ...
  for ( ; pHeadNode->planenum != -1; pHeadNode = pHeadNode->children[1] )
    FindPortalsLeadingToArea_R(pHeadNode: pHeadNode->children[0], iSrcArea, iDestArea, pPlane, portals);
  for ( i = pHeadNode->portals; i != nullptr; i = i->next[!s] )
  {
    v6 = i->nodes[0];
    s = v6 == pHeadNode;
    if ( v6->occupied != 0 )
    {
      v7 = i->nodes[1];
      if ( v7->occupied != 0
        && ((area = v7->area) == iDestArea && v6->area == iSrcArea || v6->area == iDestArea && area == iSrcArea) )
      {
        y = g_MainMap->mapplanes[i->onnode->planenum].normal.y;
        z = g_MainMap->mapplanes[i->onnode->planenum].normal.z;
        if ( COERCE_FLOAT(
               COERCE_UNSIGNED_INT(
                 1.0
               - COERCE_FLOAT(
                   COERCE_UNSIGNED_INT(
                     (float)((float)(g_MainMap->mapplanes[i->onnode->planenum].normal.x * pPlane->normal.x)
                           + (float)(pPlane->normal.y * y))
                   + (float)(pPlane->normal.z * z))
                 & _mask__AbsFloat_))
             & _mask__AbsFloat_) < 0.0099999998 )
        {
          dist = pPlane->dist;
          v13 = g_MainMap->mapplanes[i->onnode->planenum].dist;
          x = g_MainMap->mapplanes[i->onnode->planenum].normal.x;
          if ( (float)((float)((float)((float)((float)(pPlane->normal.y * dist) - (float)(y * v13))
                                     * (float)((float)(pPlane->normal.y * dist) - (float)(y * v13)))
                             + (float)((float)((float)(pPlane->normal.x * dist) - (float)(x * v13))
                                     * (float)((float)(pPlane->normal.x * dist) - (float)(x * v13))))
                     + (float)((float)((float)(dist * pPlane->normal.z) - (float)(z * v13))
                             * (float)((float)(dist * pPlane->normal.z) - (float)(z * v13)))) < 0.0099999998 )
          {
...
void __cdecl EmitClipPortalGeometry(node_t *pHeadNode, portal_t *pPortal, int iSrcArea, dareaportal_t *dp)
{
  ...
  FindPortalsLeadingToArea_R(pHeadNode, iSrcArea, iDestArea: otherarea, pPlane: &pPortal->plane, &portals);
  m_Size = 0;
  v6 = 0;
      ...
      for ( bb = (*nodes)->brushlist; bb != nullptr; bb = bb->next )
      {
        ...
        if ( (v3 & original->contents) != 0 )
        {
          ...
          if ( numsides > 0 )
          {
            ...
            while ( 1 )
            {
              if ( !original_sides->bevel && original_sides->texinfo != -1 )
              {
                ...
                winding = v1->winding;
                v10 = 0.0;
                p_x = &g_MainMap->mapplanes[v8].normal.x;
                v12 = 0;
                if ( winding->numpoints > 0 )
                {
                  v13 = p_x[1];
                  ...
                  do
                  {
                    v10 = v10
                        + COERCE_FLOAT(
                            COERCE_UNSIGNED_INT(
                              (float)((float)((float)(v17[1] * v13) + (float)(*v17 * v14)) + (float)(v17[2] * v15))
                            - v16)
                          & _mask__AbsFloat_);
                    if ( v10 > v4 )
                      break;
...
void __cdecl MarkVisibleSides(tree_t *tree, int startbrush, int endbrush, int detailScreen)
{
  ...
  if ( startbrush < endbrush )
  {
    ...
    do
    {
      if ( detailScreen == 0 || (detailScreen == 1) == ((v5->mapbrushes[v4].contents & 0x8000000) != 0) )
      {
        numsides = v5->mapbrushes[v4].numsides;
        if ( numsides > 0 )
        {
          p_original_sides = &v5->mapbrushes[v4].original_sides;
          v8 = 0;
          do
          {
            (*p_original_sides)[v8++].visible = false;
            --numsides;
          }
          while ( numsides != 0 );
          v5 = g_MainMap;
  ...
  if ( tree->headnode->planenum == -1 )
  {
    ...
    {
      for ( i = headnode->portals; i != nullptr; i = i->next[!v11] )
      {
        v11 = i->nodes[0] == headnode;
        if ( i->onnode != nullptr )
        {
          if ( i->sidefound == 0 )
            FindPortalSide(p: i);
...
void __cdecl MarkVisibleSides(tree_t *tree, mapbrush_t **ppBrushes, int nCount)
{
  ...
  for ( i = 0; i < nCount; ++i )
  {
    ...
    if ( numsides > 0 )
    {
      ...
      do
      {
        v4->original_sides[v6++].visible = false;
        --numsides;
      }
      while ( numsides != 0 );
    }

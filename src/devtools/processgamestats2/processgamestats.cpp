...
// Original: devtools/processgamestats2/processgamestats.cpp
// Functions: 385
// ============================================================

#include "devtools\processgamestats2\processgamestats.h"
...
//------------------------------------------------------------------------------
// Address: 0x004011C0
// Name: public: CParseContext::CParseContext(void)
// Source: json
//------------------------------------------------------------------------------
CParseContext *__thiscall CParseContext::CParseContext(CParseContext *this)
{
  this->mysql = nullptr;
  this->pDBInfo = nullptr;
  this->szFilename[0] = 0;
  this->fileTime = 0;
  this->nFilesFound = 0;
  this->nFilesSkipped = 0;
...
//------------------------------------------------------------------------------
// Address: 0x00401210
// Name: bool CheckFileSignature(char const __near *,long __near *)
// Source: json
//------------------------------------------------------------------------------
bool __usercall CheckFileSignature@<al>(
        int a1@<ebx>,
        int a2@<edi>,
        int a3@<esi>,
        ...
        int *pFileTimeOut)
{
  void *v5; // eax
  void *v6; // esi
  char szBuf[64]; // [esp+0h] [ebp-4Ch] BYREF
  char szSignature[12]; // [esp+40h] [ebp-Ch] BYREF

  szSignature[10] = 0;
  *pFileTimeOut = 0;
  qmemcpy(szSignature, "\"gamestats", 10);
  v5 = (void *)((int (__thiscall *)(IBaseFileSystem *, const char *, const char *, _DWORD, int, int, int))g_pFullFileSystem->Open)(
  ...
  if ( v5 == nullptr )
  {
    printf(format: "Warning: couldn't open file '%s'\r\n", szFileName);
    return false;
  }
  if ( g_pFullFileSystem->ReadEx(this: g_pFullFileSystem, a2: szBuf, a3: 64, a4: 10, a5: v5) == 0 )
    return false;
  g_pFullFileSystem->Close(this: &g_pFullFileSystem->IBaseFileSystem, a2: v6);
  *pFileTimeOut = ((int (__thiscall *)(IBaseFileSystem *))g_pFullFileSystem->GetFileTime)(a1: &g_pFullFileSystem->IBaseFileSystem);
  return V_strnicmp(s1: szSignature, s2: szBuf, n: 10) == 0;
}
...
{
  printf(format: "processgamestats:\r\n");
  printf(format: "processgamestats inputdir [optional params]\r\n\r\n");
  printf(format: "optional params:\r\n");
  printf(format: "\t-server <server>: MySQL server to connect to.  (Default: gamestats)\r\n");
  printf(format: "\t-user <user>: MySQL user to connect as as.  (Default: 'stats')\r\n");
  printf(format: "\t-password <password>: password for SQL user.  (Default: password for 'stats')\r\n");
  printf(format: "\t-schemadir <directory>: directory containing schema.txt to use.  (Default: . )\r\n");
  printf(format: "\t-steambeta: do processing for steam beta.  (appends _beta to all database names)\r\n");
  printf(format: "\t-noinsert: don't insert any new data\r\n");
  printf(format: "\t-norollup: don't generate rollup after inserting data\r\n");
  printf(format: "\t-noageout: don't age out old data\r\n");
  printf(
    format: "\t-cleanup <tablename1,tablename2,..>: remove unused columns in tables, or tables themselves -- only if they *aren't"
    "* specified in schema.txt.  Relatively safe way to remove unused tables and columns.\r\n");
  printf(format: "\t-optimize: optimize table indexes -- will take a while\r\n");
  exit(code: 1);
}

//------------------------------------------------------------------------------
// Address: 0x00401370
// Name: char const __near * FormatElapsedTime(int)
// Source: json
//------------------------------------------------------------------------------
char *__cdecl FormatElapsedTime(int iSeconds)
{
  ...
  v2 = iSeconds % 3600 / 60;
  if ( iSeconds / 3600 <= 0 )
    V_snprintf(pDest: szTime, maxLen: 32, pFormat: "%02d:%02d", v2, v3);
  else
    V_snprintf(pDest: szTime, maxLen: 32, pFormat: "%02d:%02d:%02d", iSeconds / 3600, v2, v3);
  return szTime;
}

//------------------------------------------------------------------------------
// Address: 0x00401400
// Name: public: CSimpleTimer::~CSimpleTimer(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CSimpleTimer::~CSimpleTimer(CSimpleTimer *this)
{
  *this->m_pResult = _time64(timeptr: nullptr) - this->m_iStartTime;
}
...
//------------------------------------------------------------------------------
// Address: 0x00401540
// Name: public: void CUtlMemory<class CColumn,int>::Grow(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlMemory<CColumn,int>::Grow(CUtlMemory<CColumn,int> *this, int num)
{
  int m_nGrowSize; // esi
  int m_nAllocationCount; // eax
  int v5; // ecx
  CColumn *m_pMemory; // edx
  unsigned int v7; // [esp-4h] [ebp-Ch]
...
LABEL_13:
    m_pMemory = this->m_pMemory;
    this->m_nAllocationCount = m_nAllocationCount;
    v7 = 258 * m_nAllocationCount;
    if ( m_pMemory != nullptr )
      this->m_pMemory = (CColumn *)_g_pMemAlloc->Realloc_2(this: _g_pMemAlloc, a2: m_pMemory, a3: v7);
    else
      this->m_pMemory = (CColumn *)_g_pMemAlloc->Alloc_2(this: _g_pMemAlloc, a2: v7);
  }
}

//------------------------------------------------------------------------------
// Address: 0x004015E0
// Name: public: void CUtlMemory<class CTable,int>::Grow(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlMemory<CTable,int>::Grow(CUtlMemory<CTable,int> *this, int num)
{
  int m_nGrowSize; // esi
  int m_nAllocationCount; // eax
  int v5; // ecx
  CTable *m_pMemory; // edx
  unsigned int v7; // [esp-4h] [ebp-Ch]
...
LABEL_13:
    ...
    if ( m_pMemory != nullptr )
      this->m_pMemory = (CTable *)_g_pMemAlloc->Realloc_2(this: _g_pMemAlloc, a2: m_pMemory, a3: v7);
    else
      this->m_pMemory = (CTable *)_g_pMemAlloc->Alloc_2(this: _g_pMemAlloc, a2: v7);
  }
}

//------------------------------------------------------------------------------
// Address: 0x00401680
// Name: public: void CUtlMemory<class CDatabase,int>::Grow(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlMemory<CDatabase,int>::Grow(CUtlMemory<CDatabase,int> *this, int num)
{
  int m_nGrowSize; // esi
  int m_nAllocationCount; // eax
  int v5; // ecx
  CDatabase *m_pMemory; // edx
  unsigned int v7; // [esp-4h] [ebp-Ch]
...
LABEL_13:
    ...
    if ( m_pMemory != nullptr )
      this->m_pMemory = (CDatabase *)_g_pMemAlloc->Realloc_2(this: _g_pMemAlloc, a2: m_pMemory, a3: v7);
    else
      this->m_pMemory = (CDatabase *)_g_pMemAlloc->Alloc_2(this: _g_pMemAlloc, a2: v7);
  }
}

//------------------------------------------------------------------------------
// Address: 0x00401720
// Name: public: void CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>::Grow(int)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>::Grow(
        CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> *this,
        int num)
{
  int m_nGrowSize; // esi
  int m_nAllocationCount; // eax
  int v5; // ecx
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *m_pMemory; // edx
  unsigned int v7; // [esp-4h] [ebp-Ch]
  ...
  if ( m_nGrowSize >= 0 )
  {
    ...
    v7 = 16 * m_nAllocationCount;
    if ( m_pMemory != nullptr )
      this->m_pMemory = (UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *)_g_pMemAlloc->Realloc_2(this: _g_pMemAlloc, a2: m_pMemory, a3: v7);
    else
      this->m_pMemory = (UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *)_g_pMemAlloc->Alloc_2(this: _g_pMemAlloc, a2: v7);
  }
}

//------------------------------------------------------------------------------
// Address: 0x004017D0
// Name: public: unsigned short CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(unsigned short)const
// Source: json
//------------------------------------------------------------------------------
unsigned __int16 __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 i)
{
  if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
      & 1) == 0 )
  {
    `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard' |= 1u;
    `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel = -1;
    dword_425F24 = 0x1FFFF;
  }
  if ( i == 0xFFFF )
    return dword_425F24;
  else
...
//------------------------------------------------------------------------------
// Address: 0x00401830
// Name: bool GetValueAsString(class CParseContext __near *,class KeyValues __near *,class CColumn __near &,char __near *,int)
// Source: json
//------------------------------------------------------------------------------
char __cdecl GetValueAsString(CParseContext *pParseContext, KeyValues *pKV, CColumn *column, char *szOut, int sizeOut)
{
  int fileTime; // eax
  const char *String; // eax
  CFmtStrN<256> *v8; // eax
  CFmtStrN<256> v9; // [esp+8h] [ebp-130h] BYREF
  tm the_time; // [esp+114h] [ebp-24h] BYREF

  *szOut = 0;
  if ( _V_stricmp(s1: "timesubmitted", s2: column->m_szName) != 0 )
  {
    if ( !KeyValues::IsEmpty(this: pKV, keyName: column->m_szName)
      && (String = KeyValues::GetString(this: pKV, keyName: column->m_szName, defaultValue: str)) != nullptr
      && *String != 0 )
    {
      V_strncpy(pDest: szOut, pSrc: String, maxLen: sizeOut);
      OverWriteCharsWeHate(pStr: szOut);
      v8 = CFmtStrN<256>::CFmtStrN<256>(this: &v9, pszFormat: "'%s'", szOut);
      V_snprintf(pDest: szOut, maxLen: sizeOut, pFormat: v8->m_szBuf);
      return 1;
    }
    else
    {
  ...
  {
    V_strncpy(pDest: szOut, pSrc: "Now()", maxLen: sizeOut);
    fileTime = pParseContext->fileTime;
    if ( fileTime > 0 )
    {
      _Plat_ConvertToLocalTime(a1: fileTime, a2: fileTime >> 31, a3: &the_time);
      V_snprintf(
        pDest: szOut,
        maxLen: sizeOut,
...
//------------------------------------------------------------------------------
// Address: 0x00401930
// Name: bool InsertRow(class CParseContext __near *,class CUtlVector<class KeyValues __near *,class CUtlMemory<class KeyValues __near *,int>> __near &,class CTable __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl InsertRow(
        CParseContext *pParseContext,
        ...
        CTable *pTable)
{
  ...
  int v5; // esi
  int v6; // eax
  char pDest[2048]; // [esp+4h] [ebp-140Ch] BYREF
  char v9[2048]; // [esp+804h] [ebp-C0Ch] BYREF
  char szOut[1024]; // [esp+1004h] [ebp-40Ch] BYREF
  int v11; // [esp+1404h] [ebp-Ch]
  unsigned int v12; // [esp+1408h] [ebp-8h]
  int v13; // [esp+140Ch] [ebp-4h]
  ...
  v13 = 0;
  v11 = 0;
  if ( pTable->m_vecColumn.m_Size > 0 )
  {
    v12 = 0;
    do
    {
      v4 = &v3->m_vecColumn.m_Memory.m_pMemory[v12 / 0x102];
      v5 = 0;
      if ( vecKVStack->m_Size > 0 )
      {
        while ( 1 )
        {
          ...
          if ( GetValueAsString(
                 pParseContext,
                 pKV: vecKVStack->m_Memory.m_pMemory[v5],
                 column: v4,
                 szOut,
                 sizeOut: 1024) != 0 )
            break;
          if ( ++v5 >= vecKVStack->m_Size )
            goto LABEL_10;
        }
        if ( v13 > 0 )
        {
          V_strncat(pDest, pSrc: ",", destBufferSize: 0x800u, max_chars_to_copy: -1);
          V_strncat(pDest: v9, pSrc: ",", destBufferSize: 0x800u, max_chars_to_copy: -1);
        }
        V_strncat(pDest, pSrc: v4->m_szName, destBufferSize: 0x800u, max_chars_to_copy: -1);
        V_strncat(pDest: v9, pSrc: szOut, destBufferSize: 0x800u, max_chars_to_copy: -1);
        ++v13;
LABEL_10:
        v3 = pTable;
      }
    ...
    while ( v11 < v3->m_vecColumn.m_Size );
    if ( v13 > 0 )
    {
      V_snprintf(pDest: q, maxLen: 0x4000, pFormat: "replace into %s (%s) values (%s);", v3->m_szName, pDest, v9);
      v6 = _CommandLine();
      if ( (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v6 + 40))(a1: v6, a2: "-noinsert") == 0 )
      {
        if ( !SelectDatabase(
                mysql: pParseContext->mysql,
                szDatabase: pParseContext->pDBInfo->m_vecDatabase.m_Memory.m_pMemory[pTable->m_iDatabase].m_szName)
          || ExecuteStmt(mysql: pParseContext->mysql, szStmt: q) != 0 )
        {
          return 0;
        }
        ++pParseContext->nRowsInserted;
      }
    }
  }
  return 1;
...
//------------------------------------------------------------------------------
// Address: 0x00401B20
// Name: bool ExecuteRollupStatement(class CParseContext __near *,class CTable __near &,class CTable __near &,class CRollup __near &,char const __near *,char const __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl ExecuteRollupStatement(
        CParseContext *pParseContext,
        CTable *tableSrc,
        CTable *tableDst,
        CRollup *rollup,
        ...
        char *szTimeStart)
{
  IMySQLRowSet *v7; // eax
  IMySQLRowSet *v8; // edi
  int (__thiscall *NumFields)(IMySQLRowSet *); // eax
  int v10; // esi
  char *v11; // eax
  IMySQLRowSet_vtbl *v12; // edx
  const char *v13; // eax
  CFmtStrN<256> *v14; // eax
  int v15; // esi
  bool v16; // zf
  CFmtStrN<256> *v17; // eax
  int (__thiscall *v18)(IMySQLRowSet *); // eax
  char pDest[2048]; // [esp+4h] [ebp-111Ch] BYREF
  char v20[2048]; // [esp+804h] [ebp-91Ch] BYREF
  CFmtStrN<256> v21; // [esp+1004h] [ebp-11Ch] BYREF
  const char *v22; // [esp+1110h] [ebp-10h]
  int v23; // [esp+1114h] [ebp-Ch]
  unsigned int v24; // [esp+1118h] [ebp-8h]
  char *s2; // [esp+111Ch] [ebp-4h]

  if ( ExecuteStmt(mysql: pParseContext->mysql, szStmt) != 0 )
  {
    printf(format: "Rollup from %s->%s failed executing '%s'\r\n", tableSrc->m_szName, tableDst->m_szName, q);
    return 0;
  }
  v7 = pParseContext->mysql->DuplicateRowSet(this: pParseContext->mysql);
  v8 = v7;
  if ( v7 == nullptr )
    return 1;
  if ( !v7->NextRow(this: v7) )
  {
LABEL_18:
    v8->Release(this: v8);
    return 1;
  ...
  while ( 1 )
  {
    strcpy(v20, "TimeSubmitted");
    memset(dst: (unsigned __int8 *)&v20[14], value: 0, count: 0x7F2u);
    memset(pDest, 0, sizeof(pDest));
    V_strncat(pDest, pSrc: szTimeStart, destBufferSize: 0x800u, max_chars_to_copy: -1);
    NumFields = v8->NumFields;
    v10 = 0;
    v23 = 0;
    if ( NumFields(this: v8) > 0 )
    {
      do
      {
        v11 = (char *)v8->GetFieldName(this: v8, a2: v10);
        v12 = v8->__vftable;
        s2 = v11;
        v13 = v12->GetColumnValue_String(this: v8, a2: v10);
        v16 = *v13 == 0;
        v22 = v13;
        if ( !v16 )
        {
          v14 = CFmtStrN<256>::CFmtStrN<256>(this: &v21, pszFormat: ",%s", s2);
          V_strncat(pDest: v20, pSrc: v14->m_szBuf, destBufferSize: 0x800u, max_chars_to_copy: -1);
          v15 = 0;
          v16 = tableDst->m_vecColumn.m_Size == 0;
          if ( tableDst->m_vecColumn.m_Size > 0 )
          {
            v24 = 0;
            do
            {
              if ( _V_stricmp(s1: tableDst->m_vecColumn.m_Memory.m_pMemory[v24 / 0x102].m_szName, s2) != 0 )
                break;
              v24 += 258;
              ++v15;
            }
            while ( v15 < tableDst->m_vecColumn.m_Size );
            v16 = v15 == tableDst->m_vecColumn.m_Size;
          }
          if ( v16 )
          {
            printf(format: "Error: rollup query %s returned unknown column %s\r\n", q, s2);
            v8->Release(this: v8);
            return 0;
          }
          v17 = CFmtStrN<256>::CFmtStrN<256>(this: &v21, pszFormat: ",'%s'", v22);
          V_strncat(pDest, pSrc: v17->m_szBuf, destBufferSize: 0x800u, max_chars_to_copy: -1);
          v10 = v23;
        }
        v18 = v8->NumFields;
        v23 = ++v10;
      }
      while ( v10 < v18(this: v8) );
    }
    V_snprintf(pDest: q, maxLen: 0x4000, pFormat: "insert into %s (%s) values (%s);", tableDst->m_szName, v20, pDest);
    if ( ExecuteStmt(mysql: pParseContext->mysql, szStmt: q) != 0 )
      break;
    if ( !v8->NextRow(this: v8) )
      goto LABEL_18;
  }
  printf(format: "Error: rollup query %s failed\r\n", q);
  v8->Release(this: v8);
  return 0;
}
...
//------------------------------------------------------------------------------
// Address: 0x00401DB0
// Name: bool AgeOutData(class CParseContext __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl AgeOutData(CParseContext *pParseContext)
{
  CParseContext *v1; // esi
  CDBInfo *pDBInfo; // eax
  CDatabase *v3; // edi
  CTable *v4; // esi
  int v5; // ebx
  ...
  char *v7; // eax
  _iobuf *v8; // eax
  int iDatabase; // [esp+14h] [ebp-20h]
  int iTable; // [esp+18h] [ebp-1Ch]
  int iElapsedSec; // [esp+1Ch] [ebp-18h]
  int i; // [esp+20h] [ebp-14h]
  int v14; // [esp+24h] [ebp-10h]

  printf(format: "\r\n");
  v1 = pParseContext;
  pDBInfo = pParseContext->pDBInfo;
  iDatabase = 0;
  if ( pDBInfo->m_vecDatabase.m_Size <= 0 )
    return 1;
  for ( i = 0; ; ++i )
  {
    v3 = &pDBInfo->m_vecDatabase.m_Memory.m_pMemory[i];
    SelectDatabase(mysql: v1->mysql, szDatabase: v3->m_szName);
    iTable = 0;
    if ( v3->m_vecTable.m_Size > 0 )
      break;
LABEL_10:
    pDBInfo = v1->pDBInfo;
    if ( ++iDatabase >= pDBInfo->m_vecDatabase.m_Size )
      return 1;
  }
  v14 = 0;
  while ( 1 )
  {
    v4 = &v3->m_vecTable.m_Memory.m_pMemory[v14];
    if ( v4->m_iMaxDays > 0 )
      break;
LABEL_8:
    ++v14;
    if ( ++iTable >= v3->m_vecTable.m_Size )
    {
      v1 = pParseContext;
      goto LABEL_10;
    }
  ...
  printf(format: "Deleting data older than %d days from %s.%s ", v4->m_iMaxDays, v3->m_szName, v4->m_szName);
  v6 = __iob_func();
  fflush(stream: v6 + 1);
  V_snprintf(
    pDest: q,
    maxLen: 0x4000,
    pFormat: "delete from %s where TimeSubmitted < ADDDATE(now(), INTERVAL -%d DAY);",
    v4->m_szName,
    v4->m_iMaxDays);
  if ( ExecuteStmt(mysql: pParseContext->mysql, szStmt: q) == 0 )
  {
    iElapsedSec = _time64(timeptr: nullptr) - v5;
    v7 = FormatElapsedTime(iSeconds: iElapsedSec);
    printf(format: "(%s)\r\n", v7);
    v8 = __iob_func();
    fflush(stream: v8 + 1);
    goto LABEL_8;
  }
  printf(format: "Error: failed to delete old data from %s.%s\r\n", v3->m_szName, v4->m_szName);
  _time64(timeptr: nullptr);
  return 0;
}

//------------------------------------------------------------------------------
// Address: 0x00401F60
// Name: bool OptimizeTables(class CParseContext __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl OptimizeTables(CParseContext *pParseContext)
{
  CParseContext *v1; // esi
  CDBInfo *pDBInfo; // eax
  int v3; // ebx
  ...
  char *v7; // eax
  _iobuf *v8; // eax
  int timer_4; // [esp+10h] [ebp-20h]
  int iDatabase; // [esp+14h] [ebp-1Ch]
  int iTable; // [esp+18h] [ebp-18h]
  int iElapsedSec; // [esp+1Ch] [ebp-14h]
  ...
  for ( i = 0; ; ++i )
  {
    v4 = &pDBInfo->m_vecDatabase.m_Memory.m_pMemory[i];
    SelectDatabase(mysql: v1->mysql, szDatabase: v4->m_szName);
    iTable = 0;
    if ( v4->m_vecTable.m_Size > 0 )
      break;
LABEL_7:
    pDBInfo = v1->pDBInfo;
    if ( ++iDatabase >= pDBInfo->m_vecDatabase.m_Size )
  ...
  while ( 1 )
  {
    m_szName = v4->m_vecTable.m_Memory.m_pMemory[v3].m_szName;
    timer_4 = _time64(timeptr: nullptr);
    printf(format: "Optimizing %s.%s ", v4->m_szName, m_szName);
    v6 = __iob_func();
    fflush(stream: v6 + 1);
    V_snprintf(pDest: q, maxLen: 0x4000, pFormat: "optimize table %s;", m_szName);
    if ( ExecuteStmt(mysql: pParseContext->mysql, szStmt: q) != 0 )
      break;
    iElapsedSec = _time64(timeptr: nullptr) - timer_4;
    v7 = FormatElapsedTime(iSeconds: iElapsedSec);
    printf(format: "(%s)\r\n", v7);
    v8 = __iob_func();
    fflush(stream: v8 + 1);
    ++v3;
    if ( ++iTable >= v4->m_vecTable.m_Size )
    {
      v1 = pParseContext;
      v3 = 0;
      goto LABEL_7;
...
//------------------------------------------------------------------------------
// Address: 0x004020D0
// Name: public: unsigned short CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(unsigned short)const
// Source: json
//------------------------------------------------------------------------------
unsigned __int16 __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 i)
{
  if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
  ...
  if ( i == 0xFFFF )
    return `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
  else
...
//------------------------------------------------------------------------------
// Address: 0x00402130
// Name: public: unsigned short CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(unsigned short)const
// Source: json
//------------------------------------------------------------------------------
unsigned __int16 __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 i)
{
  if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
  ...
  if ( i == 0xFFFF )
    return HIWORD(`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel);
  else
...
//------------------------------------------------------------------------------
// Address: 0x00402190
// Name: protected: void CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::FindInsertionPosition(struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t const __near &,unsigned short __near &,bool __near &)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::FindInsertionPosition(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        const CUtlMap<char const *,CTable *,unsigned short>::Node_t *insert,
...
//------------------------------------------------------------------------------
// Address: 0x00402280
// Name: public: CUtlMemory<class KeyValues __near *,int>::~CUtlMemory<class KeyValues __near *,int>(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlMemory<KeyValues *,int>::~CUtlMemory<KeyValues *,int>(CUtlMemory<KeyValues *,int> *this)
{
...
//------------------------------------------------------------------------------
// Address: 0x004022B0
// Name: public: unsigned short CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::NewNode(void)
// Source: json
//------------------------------------------------------------------------------
int __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::NewNode(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this)
{
  int m_FirstFree; // ecx
  int m_nAllocationCount; // eax
  ...
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *m_pMemory; // edx
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *v12; // edx
  ...
  if ( (_WORD)m_FirstFree == 0xFFFF )
  {
    ...
    {
      CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>::Grow(
...
//------------------------------------------------------------------------------
// Address: 0x004023B0
// Name: protected: void CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateLeft(unsigned short)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateLeft(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 elem)
{
  ...
  v4 = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(
         this,
         i: elem);
  v5 = v4;
  this->m_Elements.m_pMemory[elem].m_Right = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
                                               this,
                                               i: v4);
  elema = elem;
  if ( CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
         this,
         i: v5) != 0xFFFF )
    this->m_Elements.m_pMemory[CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
                                 this,
                                 i: v5)].m_Parent = elem;
  if ( (_WORD)v5 != 0xFFFF )
    this->m_Elements.m_pMemory[v5].m_Parent = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
                                                this,
                                                i: elem);
  if ( elem == this->m_Root )
  {
  ...
  {
    v6 = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
           this,
           i: elem);
    if ( CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
           this,
           i: v6) == elem )
      this->m_Elements.m_pMemory[CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
                                   this,
                                   i: elem)].m_Left = v5;
    else
      this->m_Elements.m_pMemory[CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
...
//------------------------------------------------------------------------------
// Address: 0x004024A0
// Name: protected: void CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateRight(unsigned short)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateRight(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 elem)
{
  unsigned __int16 v4; // ax
  ...
  v4 = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
         this,
         i: elem);
  v5 = v4;
  this->m_Elements.m_pMemory[elem].m_Left = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(
                                              this,
                                              i: v4);
  elema = elem;
  if ( CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(
         this,
         i: v5) != 0xFFFF )
    this->m_Elements.m_pMemory[CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RightChild(
                                 this,
                                 i: v5)].m_Parent = elem;
  if ( (_WORD)v5 != 0xFFFF )
    this->m_Elements.m_pMemory[v5].m_Parent = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Parent(
...
//------------------------------------------------------------------------------
// Address: 0x004025D0
// Name: bool PerformRollups(class CParseContext __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl PerformRollups(CParseContext *pParseContext)
{
  int v1; // eax
  CDBInfo *pDBInfo; // eax
  CDatabase *v4; // ebx
  CTable *m_pMemory; // ecx
  const char *m_szName; // edi
  int m_iSrcTable; // eax
  CRollup *p_m_rollup; // esi
  _iobuf *v9; // eax
  ...
  const char *v16; // eax
  float v17; // xmm0_4
  float m_flHistogramMax; // xmm1_4
  char v19; // al
  float v20; // xmm0_4
  float v21; // xmm1_4
  char *v22; // eax
  _iobuf *v23; // eax
  IMySQL *mysql; // [esp-14h] [ebp-8454h]
  IMySQL *v25; // [esp+0h] [ebp-8440h]
  char pIn[16384]; // [esp+18h] [ebp-8428h] BYREF
  char pOut[16384]; // [esp+4018h] [ebp-4428h] BYREF
  char pDest[256]; // [esp+8018h] [ebp-428h] BYREF
  CFmtStrN<256> v29; // [esp+8118h] [ebp-328h] BYREF
  char szTimeStart[128]; // [esp+8224h] [ebp-21Ch] BYREF
  char v31[32]; // [esp+82A4h] [ebp-19Ch] BYREF
  char pReplaceWith[32]; // [esp+82C4h] [ebp-17Ch] BYREF
  char szValueBuf[128]; // [esp+82E4h] [ebp-15Ch] BYREF
  tm tb; // [esp+8364h] [ebp-DCh] BYREF
  tm v35; // [esp+8388h] [ebp-B8h] BYREF
  tm v36; // [esp+83ACh] [ebp-94h] BYREF
  __int64 v37; // [esp+83D0h] [ebp-70h]
  int v38; // [esp+83D8h] [ebp-68h]
  int *v39; // [esp+83DCh] [ebp-64h]
  int v40; // [esp+83E0h] [ebp-60h]
  int v41; // [esp+83E4h] [ebp-5Ch] BYREF
  CTable *tableDst; // [esp+83E8h] [ebp-58h]
  int v43; // [esp+83ECh] [ebp-54h]
  int v44; // [esp+83F0h] [ebp-50h] BYREF
  int v45; // [esp+83F4h] [ebp-4Ch] BYREF
  int pVal; // [esp+83F8h] [ebp-48h] BYREF
  CDatabase *v47; // [esp+83FCh] [ebp-44h]
  int v48; // [esp+8400h] [ebp-40h]
  int v49; // [esp+8404h] [ebp-3Ch]
  int v50; // [esp+8408h] [ebp-38h] BYREF
  int v51; // [esp+840Ch] [ebp-34h] BYREF
  unsigned int v52; // [esp+8410h] [ebp-30h]
  unsigned int v53; // [esp+8414h] [ebp-2Ch]
  unsigned int v54; // [esp+8418h] [ebp-28h]
  int v55; // [esp+841Ch] [ebp-24h] BYREF
  int v56; // [esp+8420h] [ebp-20h] BYREF
  float m_flHistogramBase; // [esp+8424h] [ebp-1Ch]
  __int64 v58; // [esp+8428h] [ebp-18h]
  CTable *tableSrc; // [esp+8430h] [ebp-10h]
  int v60; // [esp+843Ch] [ebp-4h]

  v1 = _CommandLine();
  if ( (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v1 + 40))(a1: v1, a2: "-norollup") != 0 )
    return 1;
  pDBInfo = pParseContext->pDBInfo;
  v43 = 0;
  if ( pDBInfo->m_vecDatabase.m_Size <= 0 )
  ...
  while ( 1 )
  {
    v4 = &pDBInfo->m_vecDatabase.m_Memory.m_pMemory[v52 / 0x94];
    v49 = 0;
    v47 = v4;
    if ( v4->m_vecTable.m_Size > 0 )
      break;
LABEL_42:
    pDBInfo = pParseContext->pDBInfo;
    v52 += 148;
    if ( ++v43 >= pDBInfo->m_vecDatabase.m_Size )
      return 1;
  }
  v54 = 0;
  while ( 1 )
  {
    m_pMemory = v4->m_vecTable.m_Memory.m_pMemory;
    m_szName = m_pMemory[v54 / 0xC4].m_szName;
    m_iSrcTable = m_pMemory[v54 / 0xC4].m_rollup.m_iSrcTable;
    p_m_rollup = &m_pMemory[v54 / 0xC4].m_rollup;
    tableDst = &m_pMemory[v54 / 0xC4];
    if ( m_iSrcTable < 0 )
      goto LABEL_41;
    tableSrc = &m_pMemory[m_iSrcTable];
    v51 = 0;
    v50 = 0;
    v55 = 0;
    v45 = 0;
    ...
    v44 = 0;
    v56 = 0;
    printf(format: "Performing rollup: %s: %s -> %s ", v4->m_szName, tableSrc->m_szName, m_szName);
    v9 = __iob_func();
    fflush(stream: v9 + 1);
    v39 = &v56;
    v40 = _time64(timeptr: nullptr);
    mysql = pParseContext->mysql;
    v60 = 0;
    if ( !SelectDatabase(mysql, szDatabase: v4->m_szName) )
      goto LABEL_46;
    memset(szValueBuf, 0, sizeof(szValueBuf));
    v10 = CFmtStrN<256>::CFmtStrN<256>(this: &v29, pszFormat: "select max(TimeSubmitted) from %s;", m_szName);
    if ( !ExecuteStmtGetSingleValue(mysql: pParseContext->mysql, szStmt: v10->m_szBuf, szValueBuf, cValueBuf: 128) )
    {
      printf(format: "Error: finding max existing time on rollup destination table %s failed\r\n", m_szName);
      goto LABEL_48;
    }
    if ( szValueBuf[0] == 0 || _V_stricmp(s1: szValueBuf, s2: "NULL") == 0 )
    {
      v11 = CFmtStrN<256>::CFmtStrN<256>(
              this: &v29,
              pszFormat: "select min(TimeSubmitted) from %s;",
              tableSrc->m_szName);
      if ( !ExecuteStmtGetSingleValue(mysql: pParseContext->mysql, szStmt: v11->m_szBuf, szValueBuf, cValueBuf: 128)
        || szValueBuf[0] == 0
        || _V_stricmp(s1: szValueBuf, s2: "NULL") == 0 )
      {
        printf(format: "Warning: no source data found for %s, skipping rollup for that table\r\n", v4->m_szName);
        v60 = -1;
        v56 = _time64(timeptr: nullptr) - v40;
        goto LABEL_41;
      }
    }
    sscanf(string: szValueBuf, format: "%d-%d-%d", &v55, &v50, &v51);
    v12 = CFmtStrN<256>::CFmtStrN<256>(this: &v29, pszFormat: "select max(TimeSubmitted) from %s;", tableSrc->m_szName);
    if ( !ExecuteStmtGetSingleValue(mysql: pParseContext->mysql, szStmt: v12->m_szBuf, szValueBuf, cValueBuf: 128)
      || szValueBuf[0] == 0
      || _V_stricmp(s1: szValueBuf, s2: "NULL") == 0 )
    {
      break;
    }
    sscanf(string: szValueBuf, format: "%d-%d-%d", &v44, &v41, &v45);
    v36.tm_mday = v51 - 1;
    v36.tm_mon = v50 - 1;
    *(_QWORD *)&v36.tm_year = (unsigned int)(v55 - 1900);
    *(_QWORD *)&v36.tm_yday = 0;
    v35.tm_mday = v45 - 1;
    v35.tm_mon = v41 - 1;
    memset(&v35.tm_wday, 0, 12);
    v35.tm_year = v44 - 1900;
    memset(&v36, 0, 12);
    v35.tm_sec = 59;
    v35.tm_min = 59;
    v35.tm_hour = 23;
    V_snprintf(
      pDest: q,
      maxLen: 0x4000,
      pFormat: "delete from %s where TimeSubmitted >= '%04d-%02d-%02d 00:00:00'",
      m_szName,
      v55,
      v50,
      v51);
    if ( ExecuteStmt(mysql: pParseContext->mysql, szStmt: q) != 0 )
      goto LABEL_46;
    v48 = 0;
    if ( p_m_rollup->m_vecQuery.m_Size > 0 )
    {
      v53 = 0;
      while ( 1 )
      {
        ...
        v13 = _mktime64(&tb);
        v58 = v13;
        v37 = _mktime64(tb: &v35);
        if ( v13 <= v37 )
          break;
LABEL_38:
        v53 += 16;
        if ( ++v48 >= p_m_rollup->m_vecQuery.m_Size )
          goto LABEL_39;
      }
      while ( 1 )
      {
        _Plat_ConvertToLocalTime(a1: v13, a2: HIDWORD(v13), a3: &tb);
        v14 = tb.tm_mday + 1;
        v15 = tb.tm_mon + 1;
        v38 = tb.tm_year + 1900;
        V_snprintf(
          pDest,
          maxLen: 256,
          pFormat: "TimeSubmitted >= '%04d-%02d-%02d 00:00:00' and TimeSubmitted <= '%04d-%02d-%02d 23:59:59'",
          tb.tm_year + 1900,
          tb.tm_mon + 1,
          tb.tm_mday + 1,
          tb.tm_year + 1900,
          tb.tm_mon + 1,
          tb.tm_mday + 1);
        V_snprintf(pDest: q, maxLen: 0x4000, pFormat: "select count(*) from %s where %s\r\n", tableSrc->m_szName, pDest);
        v25 = pParseContext->mysql;
        pVal = 0;
        if ( !ExecuteStmtGetSingleValue(mysql: v25, szStmt: q, &pVal) )
          break;
        V_snprintf(pDest: szTimeStart, maxLen: 128, pFormat: "'%04d-%02d-%02d 00:00:00'", v38, v15, v14);
        if ( pVal > 0 )
        {
          v16 = CUtlString::Get(this: &p_m_rollup->m_vecQuery.m_Memory.m_pMemory[v53 / 0x10]);
          V_StrSubst(pIn: v16, pMatch: "%dateclause%", pReplaceWith: pDest, pOut, outLen: 0x4000, bCaseSensitive: false);
          if ( p_m_rollup->m_bHistogram )
          {
            m_flHistogramMax = p_m_rollup->m_flHistogramMax;
            v19 = 1;
            m_flHistogramBase = p_m_rollup->m_flHistogramBase;
            v17 = m_flHistogramBase;
            if ( m_flHistogramMax >= m_flHistogramBase )
            {
              do
              {
                if ( v19 == 0 )
                ...
                if ( v17 == (float)(int)v17 )
                  V_snprintf(pDest: pReplaceWith, maxLen: 32, pFormat: "%d", (int)v17);
                else
                  V_snprintf(pDest: pReplaceWith, maxLen: 32, pFormat: "%f", v17);
                v20 = p_m_rollup->m_flHistogramIncrement + m_flHistogramBase;
                if ( v20 == (float)(int)v20 )
                  V_snprintf(pDest: v31, maxLen: 32, pFormat: "%d", (int)v20);
                else
                  V_snprintf(pDest: v31, maxLen: 32, pFormat: "%f", v20);
                V_StrSubst(
                  pIn: pOut,
                  pMatch: "%bucketstart%",
                  pReplaceWith,
                ...
                v19 = ExecuteRollupStatement(
                        ...
                        szStmt: q,
                        szTimeStart);
                v17 = p_m_rollup->m_flHistogramIncrement + m_flHistogramBase;
                v21 = p_m_rollup->m_flHistogramMax;
                m_flHistogramBase = v17;
              }
              while ( v21 >= v17 );
            }
          }
          ...
          {
            ExecuteRollupStatement(pParseContext, tableSrc, tableDst, rollup: p_m_rollup, szStmt: pOut, szTimeStart);
          }
        }
        v58 += 86400;
        if ( v58 > v37 )
...
LABEL_39:
    ...
    v60 = -1;
    v56 = _time64(timeptr: nullptr) - v40;
    v22 = FormatElapsedTime(iSeconds: v56);
    printf(format: "(%s)\r\n", v22);
    v23 = __iob_func();
    fflush(stream: v23 + 1);
LABEL_41:
    v54 += 196;
    if ( ++v49 >= v4->m_vecTable.m_Size )
      goto LABEL_42;
  }
  printf(format: "Error: finding max existing time on rollup source table %s failed\r\n", m_szName);
LABEL_48:
  _time64(timeptr: nullptr);
  return 0;
}
...
//------------------------------------------------------------------------------
// Address: 0x00402E40
// Name: public: CUtlVector<class KeyValues __near *,class CUtlMemory<class KeyValues __near *,int>>::~CUtlVector<class KeyValues __near *,class CUtlMemory<class KeyValues __near *,int>>(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>::~CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>(
...
//------------------------------------------------------------------------------
// Address: 0x00402ED0
// Name: public: unsigned short CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Find(struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t const __near &)const
// Source: json
//------------------------------------------------------------------------------
unsigned __int16 __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Find(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        const CUtlMap<char const *,CTable *,unsigned short>::Node_t *search)
{
...
//------------------------------------------------------------------------------
// Address: 0x00402FD0
// Name: public: int CUtlVector<class CColumn,class CUtlMemory<class CColumn,int>>::InsertBefore(int)
// Source: json
//------------------------------------------------------------------------------
int __thiscall CUtlVector<CColumn,CUtlMemory<CColumn,int>>::InsertBefore(
        CUtlVector<CColumn,CUtlMemory<CColumn,int> > *this,
        int elem)
{
  int m_Size; // eax
  int m_nAllocationCount; // ecx
  CColumn *m_pMemory; // ecx
  int v6; // eax
  CColumn *v7; // eax
  ...
  if ( m_Size + 1 > m_nAllocationCount )
    CUtlMemory<CColumn,int>::Grow(this: &this->m_Memory, num: m_Size - m_nAllocationCount + 1);
  ++this->m_Size;
  m_pMemory = this->m_Memory.m_pMemory;
  v6 = this->m_Size - elem - 1;
  this->m_pElements = this->m_Memory.m_pMemory;
  if ( v6 > 0 )
    _V_memmove(dest: &m_pMemory[elem + 1], src: &m_pMemory[elem], count: 258 * v6);
  v7 = &this->m_Memory.m_pMemory[elem];
  if ( v7 != nullptr )
  {
    v7->m_szName[0] = 0;
    v7->m_szType[0] = 0;
    *(_WORD *)&v7->m_bIndexed = 0;
  }
  return elem;
}
...
//------------------------------------------------------------------------------
// Address: 0x00403050
// Name: public: void CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RemoveAll(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RemoveAll(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this)
{
  unsigned __int16 index; // cx
    ...
    if ( this->m_Elements.m_nAllocationCount > 0 )
    {
      do
      {
        if ( v3 < this->m_Elements.m_nAllocationCount
          && v3 <= index
          && CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::LeftChild(
               this,
               i: it) != v3 )
        {
          v4 = v3;
...
//------------------------------------------------------------------------------
// Address: 0x00403100
// Name: protected: void CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::InsertRebalance(unsigned short)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::InsertRebalance(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this,
        unsigned __int16 elem)
{
  ...
  if ( elem != this->m_Root )
  {
    while ( 1 )
    {
      v4 = `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard';
      if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
          & 1) == 0 )
      {
        `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel = -1;
      ...
      if ( v2 == 0xFFFF )
        v5 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( (v4 & 1) == 0 )
      {
        ...
        dword_425F24 = 0x1FFFF;
        v4 |= 1u;
        `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard' = v4;
      }
      if ( (_WORD)v6 == 0xFFFF )
        v7 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( v2 == 0xFFFF )
        v8 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( (_WORD)v9 == 0xFFFF )
        v10 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( (_WORD)v9 == 0xFFFF )
        v12 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( (_WORD)v13 == 0xFFFF )
        v14 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
      else
      ...
      if ( *(_WORD *)v14 == (_WORD)v9 )
      {
        ...
        if ( (_WORD)v11 == 0xFFFF )
          v15 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
        ...
        if ( (_WORD)v16 == 0xFFFF )
          v17 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
        ...
        if ( elem == 0xFFFF )
          v18 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
          v18 = (int *)&this->m_Elements.m_pMemory[elem];
        v19 = *((unsigned __int16 *)v18 + 2);
        if ( (v4 & 1) == 0 )
        {
          `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard' = v4 | 1;
          `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel = -1;
          dword_425F24 = 0x1FFFF;
        }
        if ( (_WORD)v19 == 0xFFFF )
          v20 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
          v20 = (int *)&this->m_Elements.m_pMemory[v19];
        if ( *((_WORD *)v20 + 1) == elem )
        {
          elem = v9;
          CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateLeft(
            this,
            elem: v9);
          v21 = `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard';
          if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
              & 1) == 0 )
          {
            v21 = `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
          ...
          if ( (_WORD)v9 == 0xFFFF )
            v22 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
          else
            v22 = (int *)&this->m_Elements.m_pMemory[v9];
          v9 = *((unsigned __int16 *)v22 + 2);
          if ( (v21 & 1) == 0 )
          {
            `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard' = v21 | 1;
            `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel = -1;
            dword_425F24 = 0x1FFFF;
          }
          if ( (_WORD)v9 == 0xFFFF )
            v23 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
          else
        ...
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateRight(
          this,
          elem: v11);
      }
      else
      {
        ...
        if ( (_WORD)v11 == 0xFFFF )
          v24 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
        ...
        if ( (_WORD)v16 == 0xFFFF )
          v25 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
...
LABEL_81:
        ...
        if ( elem == 0xFFFF )
          v26 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
        ...
        if ( (_WORD)v27 == 0xFFFF )
          v28 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
        else
        ...
        if ( *(_WORD *)v28 == elem )
        {
          ...
          CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RotateRight(
            this,
            elem: v9);
          v29 = `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard';
          if ( (`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
              & 1) == 0 )
          {
            v29 = `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard'
          ...
          if ( (_WORD)v9 == 0xFFFF )
            v30 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
          else
            v30 = (int *)&this->m_Elements.m_pMemory[v9];
          v9 = *((unsigned __int16 *)v30 + 2);
          if ( (v29 & 1) == 0 )
          {
            `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::`local static guard' = v29 | 1;
            `CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel = -1;
            dword_425F24 = 0x1FFFF;
          }
          if ( (_WORD)v9 == 0xFFFF )
            v31 = &`CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Links'::`2'::s_Sentinel;
          else
...
//------------------------------------------------------------------------------
// Address: 0x004036F0
// Name: bool UpdateSchemaIfNecessary(class IMySQL __near *,class CDBInfo __near &,class CDBInfo __near &)
// Source: json
//------------------------------------------------------------------------------
bool __cdecl UpdateSchemaIfNecessary(IMySQL *mysql, CDBInfo *dbInfoDesired, CDBInfo *dbInfoActual)
{
  int v3; // esi
  int v4; // eax
  const char *v5; // eax
  ...
  CDatabase *v32; // ebx
  _iobuf *v33; // eax
  CUtlVector<char *,CUtlMemory<char *,int> > vecTablesToClean; // [esp+4h] [ebp-4Ch] BYREF
  int iDatabase; // [esp+18h] [ebp-38h]
  int iTableActual; // [esp+1Ch] [ebp-34h]
  unsigned int v37; // [esp+20h] [ebp-30h]
  int v38; // [esp+24h] [ebp-2Ch]
  int iColumnActual; // [esp+28h] [ebp-28h]
  unsigned int v40; // [esp+2Ch] [ebp-24h]
  CDatabase *database; // [esp+30h] [ebp-20h]
  CTable *table; // [esp+34h] [ebp-1Ch]
  CTable *tableActual; // [esp+38h] [ebp-18h]
  CDatabase *databaseActual; // [esp+3Ch] [ebp-14h]
  bool bRet; // [esp+43h] [ebp-Dh]
  int v46; // [esp+4Ch] [ebp-4h]

  v3 = 0;
  bRet = false;
  memset(&vecTablesToClean, 0, sizeof(vecTablesToClean));
  v46 = 0;
  v4 = _CommandLine();
  v5 = (const char *)(*(int (__thiscall **)(int, const char *, const char *))(*(_DWORD *)v4 + 32))(
                       a1: v4,
                       a2: "-cleanup",
                       a3: str);
  if ( *v5 != 0 )
    V_SplitString(pString: v5, pSeparator: ",", outStrings: &vecTablesToClean);
  iDatabase = 0;
  if ( dbInfoDesired->m_vecDatabase.m_Size <= 0 )
    goto LABEL_82;
  v37 = 0;
  while ( 1 )
  {
    v6 = &dbInfoDesired->m_vecDatabase.m_Memory.m_pMemory[v37 / 0x94];
    v7 = dbInfoActual->m_vecDatabase.m_Size <= 0;
    database = v6;
    if ( !v7 )
    {
      table = nullptr;
      ...
      {
        if ( _V_stricmp(
               s1: &table->m_szName[(unsigned int)dbInfoActual->m_vecDatabase.m_Memory.m_pMemory],
               s2: v6->m_szName) == 0 )
          break;
        table = (CTable *)((char *)table + 148);
        ++v3;
      }
      while ( v3 < dbInfoActual->m_vecDatabase.m_Size );
    }
    if ( v3 == dbInfoActual->m_vecDatabase.m_Size )
      break;
    databaseActual = &dbInfoActual->m_vecDatabase.m_Memory.m_pMemory[v3];
    if ( !SelectDatabase(mysql, szDatabase: v6->m_szName) )
      goto exit;
    iTableActual = 0;
    if ( v6->m_vecTable.m_Size <= 0 )
      goto LABEL_57;
    v38 = 0;
    do
    {
      v10 = databaseActual;
      v11 = (CTable *)((char *)v6->m_vecTable.m_Memory.m_pMemory + v38);
      v12 = 0;
      if ( databaseActual->m_vecTable.m_Size > 0 )
      {
        table = nullptr;
        while ( _V_stricmp(s1: &table->m_szName[(unsigned int)v10->m_vecTable.m_Memory.m_pMemory], s2: v11->m_szName) != 0 )
        {
          ++table;
          if ( ++v12 >= databaseActual->m_vecTable.m_Size )
            break;
          v10 = databaseActual;
        }
        v10 = databaseActual;
      }
      if ( v12 == v10->m_vecTable.m_Size )
      {
        printf(format: "Adding table '%s' to database '%s'\r\n", v11->m_szName, v6->m_szName);
        if ( CreateTable(mysql, database: v6, table: v11) )
          goto LABEL_56;
        goto exit;
      }
      v16 = &v10->m_vecTable.m_Memory.m_pMemory[v12];
      table = nullptr;
      tableActual = v16;
      if ( v11->m_vecColumn.m_Size <= 0 )
        goto LABEL_56;
      iColumnActual = 0;
      do
      {
        v17 = (int)&v11->m_vecColumn.m_Memory.m_pMemory->m_szName[iColumnActual];
        v18 = 0;
        if ( tableActual->m_vecColumn.m_Size > 0 )
        {
          v40 = 0;
          do
          {
            if ( _V_stricmp(
                   s1: tableActual->m_vecColumn.m_Memory.m_pMemory[v40 / 0x102].m_szName,
                   s2: (const char *)v17) == 0 )
              break;
            v40 += 258;
            ++v18;
          }
          while ( v18 < tableActual->m_vecColumn.m_Size );
        }
        if ( v18 == tableActual->m_vecColumn.m_Size )
        {
          v19 = database;
          printf(
            format: "Adding column '%s' to table '%s.%s'\r\n",
            (const char *)v17,
            database->m_szName,
            v11->m_szName);
          v20 = __iob_func();
          fflush(stream: v20 + 1);
          v21 = AddColumn(mysql, database: v19, table: v11, iColumn: (int)table);
        }
        else
        {
          v22 = &tableActual->m_vecColumn.m_Memory.m_pMemory[v18];
          if ( *(_BYTE *)(v17 + 256) == 0 || v22->m_bIndexed )
            goto LABEL_54;
          v23 = database;
          printf(
            format: "Adding index on '%s' to table '%s.%s'\r\n",
            (const char *)v17,
            database->m_szName,
            v11->m_szName);
          v24 = __iob_func();
          fflush(stream: v24 + 1);
          v21 = AddIndex(mysql, database: v23, table: v11, iColumn: (int)table);
        }
        if ( !v21 )
          goto exit;
LABEL_54:
        iColumnActual += 258;
        table = (CTable *)((char *)table + 1);
      }
      while ( (int)table < v11->m_vecColumn.m_Size );
      v6 = database;
LABEL_56:
      v38 += 196;
      ++iTableActual;
    }
    while ( iTableActual < v6->m_vecTable.m_Size );
LABEL_57:
    iTableActual = 0;
    if ( databaseActual->m_vecTable.m_Size > 0 )
    {
      ...
      do
      {
        v25 = (CTable *)((char *)tableActual + (unsigned int)databaseActual->m_vecTable.m_Memory.m_pMemory);
        v26 = 0;
        v27 = v6->m_vecTable.m_Size == 0;
        if ( v6->m_vecTable.m_Size > 0 )
        {
          v38 = 0;
          do
          {
            if ( _V_stricmp(s1: &v6->m_vecTable.m_Memory.m_pMemory->m_szName[v38], s2: v25->m_szName) == 0 )
              break;
            v38 += 196;
            ++v26;
          }
          while ( v26 < v6->m_vecTable.m_Size );
          v27 = v26 == v6->m_vecTable.m_Size;
        }
        if ( v27 )
        {
          if ( IsTableInList(szTable: v25->m_szName, vecTables: &vecTablesToClean) )
          {
            v28 = databaseActual;
            printf(format: "Notice: dropping unused table '%s' from '%s'\r\n", v25->m_szName, databaseActual->m_szName);
            v29 = __iob_func();
            fflush(stream: v29 + 1);
            DropTable(mysql, database: v28, table: v25);
          }
          else
          {
            printf(
              format: "Notice: table '%s.%s' exists in database but not defined in schema, ignoring.  Use \"-cleanup %s\" if you "
              "want to remove this table.\r\n",
              databaseActual->m_szName,
              v25->m_szName,
              v25->m_szName);
          }
        }
        ...
        {
          table = &v6->m_vecTable.m_Memory.m_pMemory[v26];
          iColumnActual = 0;
          if ( v25->m_vecColumn.m_Size > 0 )
          {
            v40 = 0;
            do
            {
              m_szName = v25->m_vecColumn.m_Memory.m_pMemory[v40 / 0x102].m_szName;
              v31 = 0;
              if ( table->m_vecColumn.m_Size > 0 )
              {
                v38 = 0;
                do
                {
                  if ( _V_stricmp(s1: &table->m_vecColumn.m_Memory.m_pMemory->m_szName[v38], s2: m_szName) == 0 )
                    break;
                  v38 += 258;
                  ++v31;
                }
                while ( v31 < table->m_vecColumn.m_Size );
              }
              if ( v31 == table->m_vecColumn.m_Size )
              {
                if ( IsTableInList(szTable: v25->m_szName, vecTables: &vecTablesToClean) )
                {
                  v32 = databaseActual;
                  printf(
                    format: "Notice: dropping unused column '%s' from '%s.%s'\r\n",
                    m_szName,
                    databaseActual->m_szName,
                    v25->m_szName);
                  v33 = __iob_func();
                  fflush(stream: v33 + 1);
                  DropColumn(mysql, database: v32, table: v25, iColumn: iColumnActual);
                }
                else
                {
                  printf(
                    format: "Notice: column '%s' in '%s.%s' exists in database but not defined in schema, ignoring. Use \"-cleanu"
                    "p %s\" if you want to remove this column.\r\n",
                    m_szName,
                    databaseActual->m_szName,
                    v25->m_szName,
                    v25->m_szName);
            ...
            while ( iColumnActual < v25->m_vecColumn.m_Size );
            v6 = database;
          }
        }
        ++tableActual;
      ...
      while ( iTableActual < databaseActual->m_vecTable.m_Size );
    }
LABEL_81:
    v37 += 148;
    v3 = 0;
    if ( ++iDatabase >= dbInfoDesired->m_vecDatabase.m_Size )
    {
LABEL_82:
      bRet = true;
      goto exit;
  ...
  if ( CreateDatabase(mysql, database: v6) && SelectDatabase(mysql, szDatabase: v6->m_szName) )
  {
    v8 = 0;
    if ( v6->m_vecTable.m_Size <= 0 )
      goto LABEL_81;
    v9 = 0;
    while ( CreateTable(mysql, database: v6, table: &v6->m_vecTable.m_Memory.m_pMemory[v9]) )
    {
      ++v8;
      ++v9;
      if ( v8 >= v6->m_vecTable.m_Size )
        goto LABEL_81;
    }
  }
exit:
  for ( i = 0; i < vecTablesToClean.m_Size; ++i )
    free(pMem: vecTablesToClean.m_Memory.m_pMemory[i]);
  m_pMemory = vecTablesToClean.m_Memory.m_pMemory;
  vecTablesToClean.m_Size = 0;
  if ( vecTablesToClean.m_Memory.m_nGrowSize >= 0 )
  {
    if ( vecTablesToClean.m_Memory.m_pMemory != nullptr )
    {
      _g_pMemAlloc->Free_2(this: _g_pMemAlloc, a2: vecTablesToClean.m_Memory.m_pMemory);
      m_pMemory = nullptr;
      vecTablesToClean.m_Memory.m_pMemory = nullptr;
    }
    vecTablesToClean.m_Memory.m_nAllocationCount = 0;
  }
  vecTablesToClean.m_pElements = m_pMemory;
  v46 = 1;
  vecTablesToClean.m_Size = 0;
  if ( vecTablesToClean.m_Memory.m_nGrowSize >= 0 )
  {
  ...
  vecTablesToClean.m_pElements = m_pMemory;
  v46 = -1;
  if ( vecTablesToClean.m_Memory.m_nGrowSize >= 0 && m_pMemory != nullptr )
    _g_pMemAlloc->Free_2(this: _g_pMemAlloc, a2: m_pMemory);
  return bRet;
}
...
//------------------------------------------------------------------------------
// Address: 0x00403CB0
// Name: public: int CUtlVector<class KeyValues __near *,class CUtlMemory<class KeyValues __near *,int>>::AddToHead(class KeyValues __near * const __near &)
// Source: json
//------------------------------------------------------------------------------
int __thiscall CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>::AddToHead(
        CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int> > *this,
        KeyValues **src)
{
  int m_Size; // eax
  ...
  m_nAllocationCount = this->m_Memory.m_nAllocationCount;
  if ( m_Size + 1 > m_nAllocationCount )
    CUtlMemory<char *,int>::Grow((CUtlMemory<char *,int> *)this, num: m_Size - m_nAllocationCount + 1);
  ++this->m_Size;
  m_pMemory = this->m_Memory.m_pMemory;
  v6 = this->m_Size - 1;
  this->m_pElements = this->m_Memory.m_pMemory;
...
//------------------------------------------------------------------------------
// Address: 0x00403D10
// Name: bool RecursiveProcessKeys(class CParseContext __near *,class CUtlVector<class KeyValues __near *,class CUtlMemory<class KeyValues __near *,int>> __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl RecursiveProcessKeys(CParseContext *pParseContext, CUtlMemory<char *,int> *vecKVStack)
{
  CDBInfo *pDBInfo; // ebx
  ...
  pDBInfo = pParseContext->pDBInfo;
  FirstTrueSubKey = KeyValues::GetFirstTrueSubKey(this: *(KeyValues **)vecKVStack->m_pMemory);
  ...
  while ( 1 )
  {
    ...
    v6 = (KeyValues **)vecKVStack->m_pMemory;
    v7 = (int)vecKVStack[1].m_pMemory - 1;
    vecKVStack[1].m_nAllocationCount = (int)vecKVStack->m_pMemory;
    if ( v7 > 0 )
      _V_memmove(dest: v6 + 1, src: v6, count: 4 * v7);
    ...
    {
      search.key = Name;
      v9 = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Find(
             this: &pDBInfo->m_mapTable.m_Tree,
             &search);
      if ( v9 != 0xFFFF
        && InsertRow(
             pParseContext,
             (CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int> > *)vecKVStack,
             pTable: pDBInfo->m_mapTable.m_Tree.m_Elements.m_pMemory[v9].m_Data.elem) == 0 )
      {
        break;
      }
    }
    if ( !RecursiveProcessKeys(pParseContext, (CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int> > *)vecKVStack) )
      break;
...
//------------------------------------------------------------------------------
// Address: 0x00403E20
// Name: public: CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short,class CUtlMap<char const __near *,class CTable __near *,unsigned short>::CKeyLess,class CUtlMemory<struct UtlRBTreeNode_t<struct CUtlMap<char const __near *,class CTable __near *,unsigned short>::Node_t,unsigned short>,unsigned short>>(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>(
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short> > *this)
{
  CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::RemoveAll(this);
  this->m_FirstFree = -1;
...
//------------------------------------------------------------------------------
// Address: 0x00403F30
// Name: bool ProcessFile(class CParseContext __near *)
// Source: json
//------------------------------------------------------------------------------
char __usercall ProcessFile@<al>(int a1@<ebx>, int a2@<esi>, KeyValues *pParseContext)
{
  CParseContext *v3; // edi
  CParseContext *v5; // eax
  CParseContext *v6; // esi
  IBaseFileSystem *v7; // eax
  const char *Name; // eax
  char v9; // al
  char v10; // bl
  CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int> > vecKVStack; // [esp+4h] [ebp-20h] BYREF
  int v12; // [esp+20h] [ebp-4h]

  v3 = (CParseContext *)pParseContext;
  if ( CheckFileSignature(
         a1,
         a2: (int)pParseContext,
         a3: a2,
         szFileName: (const char *)&pParseContext->m_wsValue,
         pFileTimeOut: (int *)&pParseContext[7].m_pPeer) )
  {
    v5 = (CParseContext *)KeyValues::operator new(iAllocSize: 0x24u);
    pParseContext = (KeyValues *)v5;
    v12 = 0;
    if ( v5 != nullptr )
      v6 = (CParseContext *)KeyValues::KeyValues(this: (KeyValues *)v5, setName: "gamestats");
    else
      v6 = nullptr;
    v12 = -1;
    pParseContext = (KeyValues *)v6;
    ...
    if ( KeyValues::LoadFromFile(
           this: (KeyValues *)v6,
           filesystem: v7,
           resourceName: v3->szFilename,
           pathID: nullptr,
           pfnEvaluateSymbolProc: nullptr) )
    {
      memset(&vecKVStack, 0, sizeof(vecKVStack));
      v12 = 1;
      CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>::AddToHead(this: &vecKVStack, src: &pParseContext);
      Name = KeyValues::GetName(this: (KeyValues *)v6);
      if ( _V_stricmp(s1: Name, s2: "gamestats_infested") != 0 )
        v9 = RecursiveProcessKeys(pParseContext: v3, (CUtlMemory<char *,int> *)&vecKVStack);
      else
        v9 = Infested_RecursiveProcessKeys(pParseContext: v3, &vecKVStack);
      v10 = v9;
      if ( vecKVStack.m_Size - 1 > 0 )
        _V_memmove(
          dest: vecKVStack.m_Memory.m_pMemory,
          src: vecKVStack.m_Memory.m_pMemory + 1,
          count: 4 * (vecKVStack.m_Size - 1));
      --vecKVStack.m_Size;
      KeyValues::deleteThis(this: (KeyValues *)v6);
      if ( v10 != 0 )
        ++v3->nFilesProcessed;
      else
        ++v3->nFilesFailed;
      v12 = -1;
      CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>::~CUtlVector<KeyValues *,CUtlMemory<KeyValues *,int>>(this: &vecKVStack);
      return 1;
    }
    else
    {
...
//------------------------------------------------------------------------------
// Address: 0x004040A0
// Name: bool ProcessDirectory(class CParseContext __near *,char const __near *,bool)
// Source: json
//------------------------------------------------------------------------------
char __cdecl ProcessDirectory(CParseContext *pParseContext, const char *szDirectory, bool bCountOnly)
{
  const char *v3; // ebx
  _iobuf *v4; // eax
  CFmtStrN<256> *v6; // eax
  ...
  float v10; // xmm0_4
  _iobuf *v11; // eax
  CFmtStrN<256> v12; // [esp+14h] [ebp-398h] BYREF
  char szSearch[264]; // [esp+120h] [ebp-28Ch] BYREF
  _stat64i32 buf; // [esp+228h] [ebp-184h] BYREF
  _WIN32_FIND_DATAA wfd; // [esp+258h] [ebp-154h] BYREF
  char szExtension[16]; // [esp+398h] [ebp-14h] BYREF
  void *ff; // [esp+3A8h] [ebp-4h]

  v3 = szDirectory;
  if ( _stat64i32(name: szDirectory, &buf) == -1 )
  {
    printf(format: "Error: directory %s not found.\r\n", szDirectory);
    v4 = __iob_func();
    fflush(stream: v4 + 1);
    return 0;
  }
  V_snprintf(pDest: szSearch, maxLen: 261, pFormat: "%s\\*.*", szDirectory);
  ff = FindFirstFileA(lpFileName: szSearch, lpFindFileData: &wfd);
  if ( ff == (void *)-1 )
  {
    printf(format: "No files found in directory %s.\r\n", szDirectory);
    return 1;
  }
  while ( 1 )
  {
    ...
    {
      if ( _V_stricmp(s1: wfd.cFileName, s2: ".") != 0 && _V_stricmp(s1: wfd.cFileName, s2: "..") != 0 )
      {
        v6 = CFmtStrN<256>::CFmtStrN<256>(this: &v12, pszFormat: "%s\\%s", v3, wfd.cFileName);
        ProcessDirectory(pParseContext, szDirectory: v6->m_szBuf, bCountOnly);
      }
      goto LABEL_18;
    }
    V_ExtractFileExtension(path: wfd.cFileName, dest: szExtension, destSize: 16);
    if ( _V_stricmp(s1: szExtension, s2: "dat") == 0 )
      break;
LABEL_18:
    if ( !FindNextFileA(hFindFile: ff, lpFindFileData: &wfd) )
      return 1;
  }
  V_snprintf(pDest: pParseContext->szFilename, maxLen: 261, pFormat: "%s\\%s", v3, wfd.cFileName);
  strlwr(string: pParseContext->szFilename);
  V_FixSlashes(pname: pParseContext->szFilename, separator: 92);
  if ( bCountOnly )
  {
    ++pParseContext->nFilesFound;
    goto LABEL_18;
  }
  v7 = ProcessFile(a1: (int)v3, a2: (int)pParseContext, (KeyValues *)pParseContext);
  v8 = pParseContext->nFilesSkipped + pParseContext->nFilesProcessed + pParseContext->nFilesFailed;
  if ( v8 % 100 == 0 )
  {
    nFilesFound = pParseContext->nFilesFound;
    v10 = 0.0;
    if ( nFilesFound > 0 )
      v10 = (float)((float)v8 / (float)nFilesFound) * 100.0;
    printf(format: "Processing:  [%06d/%06d] %.2f%% complete\r\n", v8, nFilesFound, v10);
    v11 = __iob_func();
    fflush(stream: v11 + 1);
  }
  if ( v7 != 0 )
...
//------------------------------------------------------------------------------
// Address: 0x004042D0
// Name: bool ProcessAllFiles(class CParseContext __near *,char const __near *)
// Source: json
//------------------------------------------------------------------------------
char __cdecl ProcessAllFiles(CParseContext *pParseContext, const char *szDirectory)
{
  char result; // al

  result = ProcessDirectory(pParseContext, szDirectory, bCountOnly: true);
  if ( result != 0 )
    return ProcessDirectory(pParseContext, szDirectory, bCountOnly: false);
  return result;
}

//------------------------------------------------------------------------------
// Address: 0x00404300
// Name: public: void CUtlVector<class CTable,class CUtlMemory<class CTable,int>>::RemoveAll(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlVector<CTable,CUtlMemory<CTable,int>>::RemoveAll(CUtlVector<CTable,CUtlMemory<CTable,int> > *this)
{
  int v1; // eax
  int v2; // eax
  char *v3; // edi
  ...
  void **v6; // esi
  void *v7; // eax
  CUtlVector<CTable,CUtlMemory<CTable,int> > *v8; // [esp+8h] [ebp-18h]
  ...
  else
  {
    ...
    for ( j = v2; ; v2 = j )
    {
      v3 = &this->m_Memory.m_pMemory->m_szName[v2];
      v4 = (void **)(v3 + 160);
      CUtlVector<CUtlString,CUtlMemory<CUtlString,int>>::RemoveAll(this: (CUtlVector<CUtlString,CUtlMemory<CUtlString,int> > *)v3 + 8);
      if ( *((int *)v3 + 42) >= 0 )
      {
        if ( *v4 != nullptr )
        {
...
//------------------------------------------------------------------------------
// Address: 0x00404450
// Name: public: int CUtlVector<class CDatabase,class CUtlMemory<class CDatabase,int>>::InsertBefore(int)
// Source: json
//------------------------------------------------------------------------------
int __thiscall CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::InsertBefore(
        CUtlVector<CDatabase,CUtlMemory<CDatabase,int> > *this,
        int elem)
{
  int m_Size; // eax
  int m_nAllocationCount; // ecx
  CDatabase *m_pMemory; // ecx
  int v6; // eax
  CDatabase *v7; // eax
  ...
  m_nAllocationCount = this->m_Memory.m_nAllocationCount;
  if ( m_Size + 1 > m_nAllocationCount )
    CUtlMemory<CDatabase,int>::Grow(this: &this->m_Memory, num: m_Size - m_nAllocationCount + 1);
  ++this->m_Size;
  m_pMemory = this->m_Memory.m_pMemory;
  v6 = this->m_Size - elem - 1;
  this->m_pElements = this->m_Memory.m_pMemory;
  if ( v6 > 0 )
    _V_memmove(dest: &m_pMemory[elem + 1], src: &m_pMemory[elem], count: 148 * v6);
  v7 = &this->m_Memory.m_pMemory[elem];
  if ( v7 != nullptr )
  {
    v7->m_vecTable.m_Memory.m_pMemory = nullptr;
    v7->m_vecTable.m_Memory.m_nAllocationCount = 0;
    v7->m_vecTable.m_Memory.m_nGrowSize = 0;
    v7->m_vecTable.m_Size = 0;
    v7->m_vecTable.m_pElements = nullptr;
  }
  return elem;
}
...
//------------------------------------------------------------------------------
// Address: 0x00404500
// Name: public: void CUtlVector<class CDatabase,class CUtlMemory<class CDatabase,int>>::RemoveAll(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::RemoveAll(
        CUtlVector<CDatabase,CUtlMemory<CDatabase,int> > *this)
{
  int v1; // ebx
  int v2; // edx
  CUtlVector<CTable,CUtlMemory<CTable,int> > *v3; // esi
  CTable *m_pMemory; // eax
  CUtlVector<CDatabase,CUtlMemory<CDatabase,int> > *v5; // [esp+8h] [ebp-14h]
  ...
  else
  {
    ...
    for ( i = 148 * v1; ; v2 = i )
    {
      v3 = (CUtlVector<CTable,CUtlMemory<CTable,int> > *)((char *)&this->m_Memory.m_pMemory->m_vecTable + v2);
      CUtlVector<CTable,CUtlMemory<CTable,int>>::RemoveAll(this: v3);
...
//------------------------------------------------------------------------------
// Address: 0x004045E0
// Name: bool LoadSchemaFile(char const __near *,bool,class CDBInfo __near &)
// Source: json
//------------------------------------------------------------------------------
char __cdecl LoadSchemaFile(KeyValues *szSchemaDir, bool bIsSteamBeta, CDBInfo *dbInfoOut)
{
  KeyValues *v3; // eax
  KeyValues *v4; // esi
  ...
  char *v55; // eax
  bool v56; // cc
  CUtlMap<char const *,CTable *,unsigned short> *p_m_mapTable; // esi
  CTable *v58; // edi
  unsigned __int16 v59; // ax
  unsigned __int16 v60; // dx
  unsigned __int16 v61; // cx
  int v62; // edi
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *v63; // eax
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *m_pMemory; // eax
  int v65; // edx
  KeyValues *v66; // ecx
  const char *v67; // eax
  const char *v68; // eax
  ...
  const char *v73; // eax
  const char *v74; // eax
  UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short> *v75; // ecx
  CUtlMap<char const *,CTable *,unsigned short>::Node_t *p_m_Data; // edi
  _iobuf *v77; // eax
  const char *defaultValue; // [esp+10h] [ebp-29Ch]
  CFmtStrN<256> v79; // [esp+28h] [ebp-284h] BYREF
  char szFileName[264]; // [esp+134h] [ebp-178h] BYREF
  void *v81; // [esp+23Ch] [ebp-70h]
  CUtlString v82; // [esp+240h] [ebp-6Ch] BYREF
  char szSchemaFile[12]; // [esp+250h] [ebp-5Ch] BYREF
  const char *szReservedTableNames[2]; // [esp+25Ch] [ebp-50h] BYREF
  CUtlMap<char const *,CTable *,unsigned short>::Node_t insert; // [esp+264h] [ebp-48h] BYREF
  int iDatabase; // [esp+26Ch] [ebp-40h]
  const char *v87; // [esp+270h] [ebp-3Ch]
  int iTable; // [esp+274h] [ebp-38h]
  const char *v89; // [esp+278h] [ebp-34h]
  CUtlVector<char *,CUtlMemory<char *,int> > outStrings; // [esp+27Ch] [ebp-30h] BYREF
  CDatabase *database; // [esp+290h] [ebp-1Ch]
  const char *szType; // [esp+294h] [ebp-18h] BYREF
  KeyValues *pValue; // [esp+298h] [ebp-14h]
  int iSrcTable; // [esp+29Ch] [ebp-10h]
  int v95; // [esp+2A8h] [ebp-4h]

  strcpy(szSchemaFile, "schema.txt");
  memset(szFileName, 0, 261);
  if ( szSchemaDir != nullptr && *(_BYTE *)szSchemaDir != 0 )
  {
    V_ComposeFileName(path: (const char *)szSchemaDir, filename: szSchemaFile, dest: szFileName, destSize: 261);
  }
  else
  {
  ...
  HIBYTE(szSchemaDir) = HIBYTE(v3);
  v95 = 0;
  if ( v3 != nullptr )
    v4 = KeyValues::KeyValues(this: v3, setName: "schema");
  else
    v4 = nullptr;
  v95 = -1;
  if ( g_pFullFileSystem != nullptr )
  ...
  if ( !KeyValues::LoadFromFile(
          ...
          pfnEvaluateSymbolProc: nullptr) )
  {
    printf(format: "Error: Failed to load schema file %s\r\n", szFileName);
    return 0;
  }
  printf(format: "Loading schema file %s... ", szFileName);
  v7 = __iob_func();
  fflush(stream: v7 + 1);
  szReservedTableNames[0] = "gamestats";
  szReservedTableNames[1] = "map";
  FirstTrueSubKey = KeyValues::GetFirstTrueSubKey(this: v4);
  iTable = (int)FirstTrueSubKey;
  if ( FirstTrueSubKey == nullptr )
  {
LABEL_96:
    database = nullptr;
    if ( dbInfoOut->m_vecDatabase.m_Size <= 0 )
    {
LABEL_137:
      printf(format: "schema loaded successfully.\n");
      v77 = __iob_func();
      fflush(stream: v77 + 1);
      return 1;
    }
    else
    {
      ...
      while ( 1 )
      {
        v55 = &dbInfoOut->m_vecDatabase.m_Memory.m_pMemory->m_szName[iSrcTable];
        iTable = 0;
        v56 = *((_DWORD *)v55 + 35) <= 0;
        iDatabase = (int)v55;
        if ( !v56 )
...
LABEL_136:
        iSrcTable += 148;
        database = (CDatabase *)((char *)database + 1);
        if ( (int)database >= dbInfoOut->m_vecDatabase.m_Size )
          goto LABEL_137;
      }
      p_m_mapTable = &dbInfoOut->m_mapTable;
      pValue = nullptr;
      while ( 1 )
      {
        v58 = (CTable *)((char *)pValue + *((_DWORD *)v55 + 32));
        szReservedTableNames[0] = (const char *)v58;
        if ( CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::Find(
               this: &p_m_mapTable->m_Tree,
               search: (const CUtlMap<char const *,CTable *,unsigned short>::Node_t *)szReservedTableNames) != 0xFFFF )
          break;
        szType = (const char *)0xFFFF;
        insert.key = (const char *)v58;
        insert.elem = v58;
        HIBYTE(szSchemaDir) = 0;
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::FindInsertionPosition(
          this: &p_m_mapTable->m_Tree,
          &insert,
          parent: (unsigned __int16 *)&szType,
          leftchild: (bool *)&szSchemaDir + 3);
        v59 = CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::NewNode(this: &p_m_mapTable->m_Tree);
        v60 = (unsigned __int16)szType;
        v61 = v59;
        v62 = v59;
        v63 = &p_m_mapTable->m_Tree.m_Elements.m_pMemory[v62];
        v63->m_Right = -1;
        v63->m_Left = -1;
        v63->m_Parent = v60;
        v63->m_Tag = 0;
        if ( v60 == 0xFFFF )
        {
          p_m_mapTable->m_Tree.m_Root = v61;
        }
        else
        {
          m_pMemory = p_m_mapTable->m_Tree.m_Elements.m_pMemory;
          v65 = v60;
          if ( HIBYTE(szSchemaDir) != 0 )
            m_pMemory[v65].m_Left = v61;
          else
            m_pMemory[v65].m_Right = v61;
        }
        CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::InsertRebalance(
          this: &p_m_mapTable->m_Tree,
          elem: v61);
        v75 = p_m_mapTable->m_Tree.m_Elements.m_pMemory;
        ++p_m_mapTable->m_Tree.m_NumElements;
        p_m_Data = &v75[v62].m_Data;
        if ( p_m_Data != nullptr )
          *p_m_Data = insert;
        v55 = (char *)iDatabase;
        pValue = (KeyValues *)((char *)pValue + 196);
        if ( ++iTable >= *(_DWORD *)(iDatabase + 140) )
          goto LABEL_136;
      }
      printf(
        format: "Error: duplicate table name '%s' defined in schema.  Table names must be globally unique, including across databases.\r\n",
        v58->m_szName);
      return 0;
    }
  }
  ...
  {
    Name = KeyValues::GetName(this: FirstTrueSubKey);
    if ( _V_stricmp(s1: Name, s2: "database") != 0 )
    {
      printf(format: "Error: Invalid schema, 'database' node not found\r\n");
      return 0;
    }
    String = KeyValues::GetString(this: FirstTrueSubKey, keyName: "name", defaultValue: str);
    v11 = String;
    v89 = String;
    if ( String == nullptr || *String == 0 )
      break;
    iDatabase = CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::InsertBefore(
                  this: &dbInfoOut->m_vecDatabase,
                  elem: dbInfoOut->m_vecDatabase.m_Size);
    database = &dbInfoOut->m_vecDatabase.m_Memory.m_pMemory[iDatabase];
    v12 = database;
    V_strncpy(pDest: database->m_szName, pSrc: v11, maxLen: 128);
    if ( bIsSteamBeta )
      V_strncat(pDest: v12->m_szName, pSrc: "_beta", destBufferSize: 0x80u, max_chars_to_copy: -1);
    v13 = (char *)KeyValues::GetFirstTrueSubKey(this: FirstTrueSubKey);
    szSchemaDir = (KeyValues *)v13;
    if ( v13 != nullptr )
    {
      while ( 1 )
      {
        v14 = KeyValues::GetName(this: (KeyValues *)v13);
        if ( _V_stricmp(s1: v14, s2: "table") == 0 )
        {
          v15 = szSchemaDir;
          v16 = KeyValues::GetString(this: szSchemaDir, keyName: "name", defaultValue: str);
          v17 = v16;
          v87 = v16;
          ...
          for ( i = 0; i < 2; ++i )
          {
            if ( _V_stricmp(s1: szReservedTableNames[i], s2: v17) == 0 )
            {
              printf(format: "Error: Invalid schema, '%s' is reserved and cannot be used as a table name\r\n", v17);
              return 0;
            }
          }
          m_Size = v12->m_vecTable.m_Size;
          m_nAllocationCount = v12->m_vecTable.m_Memory.m_nAllocationCount;
          if ( m_Size + 1 > m_nAllocationCount )
            CUtlMemory<CTable,int>::Grow(this: &v12->m_vecTable.m_Memory, num: m_Size - m_nAllocationCount + 1);
          ++v12->m_vecTable.m_Size;
          v21 = v12->m_vecTable.m_Memory.m_pMemory;
          v22 = v12->m_vecTable.m_Size - m_Size - 1;
          v12->m_vecTable.m_pElements = v21;
          if ( v22 > 0 )
            _V_memmove(dest: &v21[m_Size + 1], src: &v21[m_Size], count: 196 * v22);
          v23 = m_Size;
          v24 = &v12->m_vecTable.m_Memory.m_pMemory[v23];
          if ( v24 != nullptr )
          {
            v24->m_vecColumn.m_Memory.m_pMemory = nullptr;
            v24->m_vecColumn.m_Memory.m_nAllocationCount = 0;
            v24->m_vecColumn.m_Memory.m_nGrowSize = 0;
            v24->m_vecColumn.m_Size = 0;
            v24->m_vecColumn.m_pElements = nullptr;
            v24->m_rollup.m_vecQuery.m_Memory.m_pMemory = nullptr;
            v24->m_rollup.m_vecQuery.m_Memory.m_nAllocationCount = 0;
            v24->m_rollup.m_vecQuery.m_Memory.m_nGrowSize = 0;
            v24->m_rollup.m_vecQuery.m_Size = 0;
            v24->m_rollup.m_vecQuery.m_pElements = nullptr;
            v24->m_rollup.m_iSrcTable = -1;
            v24->m_rollup.m_bHistogram = false;
            v24->m_rollup.m_flHistogramBase = 0.0;
            v24->m_rollup.m_flHistogramIncrement = 0.0;
            v24->m_rollup.m_flHistogramMax = 0.0;
            v24->m_szName[0] = 0;
            v24->m_iDatabase = -1;
            v24->m_iMaxDays = 0;
          }
          v25 = &v12->m_vecTable.m_Memory.m_pMemory[v23];
          v95 = -1;
          V_strncpy(pDest: v25->m_szName, pSrc: v17, maxLen: 128);
          v26 = szSchemaDir;
          v25->m_iDatabase = iDatabase;
          Key = KeyValues::FindKey(this: v26, keyName: "columns", bCreate: false);
          if ( Key == nullptr )
          {
            printf(format: "Error: required subkey 'columns' not found for table'%s'\r\n", v17);
            return 0;
          }
          ...
          if ( FirstValue != nullptr )
          {
            while ( 1 )
            {
              v29 = KeyValues::GetString(this: FirstValue, keyName: nullptr, defaultValue: str);
              v30 = v29;
              szType = v29;
              if ( v29 == nullptr || *v29 == 0 )
              {
                v74 = KeyValues::GetName(this: FirstValue);
                printf(format: "Error: %s.%s, column %s has no type defined\r\n", v89, v87, v74);
                return 0;
              }
              memset(&outStrings, 0, sizeof(outStrings));
              v95 = 3;
              V_SplitString(pString: v29, pSeparator: ",", &outStrings);
              if ( outStrings.m_Size > 3 )
                break;
              m_szName = v25->m_vecColumn.m_Memory.m_pMemory[CUtlVector<CColumn,CUtlMemory<CColumn,int>>::InsertBefore(
                                                               this: &v25->m_vecColumn,
                                                               elem: v25->m_vecColumn.m_Size)].m_szName;
              v32 = KeyValues::GetName(this: FirstValue);
              V_strncpy(pDest: m_szName, pSrc: v32, maxLen: 128);
              V_strncpy(pDest: m_szName + 128, pSrc: *(const char **)outStrings.m_Memory.m_pMemory, maxLen: 128);
              if ( outStrings.m_Size > 1 )
              {
                for ( j = 1; j < outStrings.m_Size; ++j )
                {
                  if ( _V_stricmp(s1: outStrings.m_Memory.m_pMemory[j], s2: "indexed") != 0 )
                  {
                    if ( _V_stricmp(s1: outStrings.m_Memory.m_pMemory[j], s2: "autoincrement") != 0 )
                    {
                      defaultValue = szType;
                      v73 = KeyValues::GetName(this: pValue);
                      printf(
                        format: "Error: %s.%s, column %s has unknown definition after the comma in '%s'\r\n",
                        v89,
                        v87,
                        v73,
                        defaultValue);
                      v71 = outStrings.m_Memory.m_pMemory;
                      v95 = 5;
                      outStrings.m_Size = 0;
...
LABEL_118:
              ...
              v34 = outStrings.m_Memory.m_pMemory;
              v95 = 6;
              outStrings.m_Size = 0;
              if ( outStrings.m_Memory.m_nGrowSize >= 0 )
              {
              ...
              outStrings.m_pElements = v34;
              v95 = -1;
              if ( outStrings.m_Memory.m_nGrowSize >= 0 && v34 != nullptr )
                _g_pMemAlloc->Free_2(this: _g_pMemAlloc, a2: v34);
              FirstValue = KeyValues::GetNextValue(this: pValue);
              pValue = FirstValue;
              if ( FirstValue == nullptr )
                goto LABEL_55;
            }
            v70 = KeyValues::GetName(this: FirstValue);
            printf(format: "Error: %s.%s, column %s has too many delimiters in definition '%s'\r\n", v89, v87, v70, v30);
            v71 = outStrings.m_Memory.m_pMemory;
            v95 = 4;
            outStrings.m_Size = 0;
            if ( outStrings.m_Memory.m_nGrowSize >= 0 )
...
LABEL_120:
            outStrings.m_pElements = v71;
            v95 = -1;
            if ( outStrings.m_Memory.m_nGrowSize >= 0 && v71 != nullptr )
...
LABEL_55:
          if ( v25->m_vecColumn.m_Size == 0 )
          {
            printf(format: "Error: No columns defined for table '%s.%s'\r\n", v89, v87);
            return 0;
          }
          v35 = KeyValues::FindKey(this: szSchemaDir, keyName: "rollup", bCreate: false);
          pValue = v35;
          if ( v35 != nullptr )
          {
            v36 = v35;
            v37 = KeyValues::GetString(this: v35, keyName: "srctable", defaultValue: str);
            szType = v37;
            if ( v37 == nullptr || *v37 == 0 )
            {
              v68 = KeyValues::GetName(this: v36);
              printf(format: "Error: missing required value '%s' from node '%s'\r\n", "srctable", v68);
              return 0;
            }
            v38 = 0;
            iSrcTable = -1;
            if ( database->m_vecTable.m_Size > 0 )
            {
              v39 = 0;
              do
              {
                if ( _V_stricmp(s1: database->m_vecTable.m_Memory.m_pMemory[v39].m_szName, s2: szType) == 0 )
                  iSrcTable = v38;
                ++v38;
                ++v39;
              }
              while ( v38 < database->m_vecTable.m_Size );
              if ( iSrcTable != -1 )
              {
                v40 = pValue;
                v25->m_rollup.m_iSrcTable = iSrcTable;
                if ( !KeyValues::IsEmpty(this: v40, keyName: "histogrambase") )
                {
                  v25->m_rollup.m_bHistogram = true;
                  v25->m_rollup.m_flHistogramBase = KeyValues::GetFloat(
                                                      this: v40,
                                                      keyName: "histogrambase",
                                                      defaultValue: 0.0);
                  v25->m_rollup.m_flHistogramIncrement = KeyValues::GetFloat(
                                                           this: v40,
                                                           keyName: "histogramincrement",
                                                           defaultValue: 0.0);
                  v25->m_rollup.m_flHistogramMax = KeyValues::GetFloat(
                                                     this: v40,
                                                     keyName: "histogrammax",
                                                     defaultValue: 0.0);
                }
                v41 = KeyValues::GetString(this: v40, keyName: "query", defaultValue: str);
                if ( v41 != nullptr && *v41 != 0 )
                {
                  v42 = v41;
                  for ( iSrcTable = 2; ; ++iSrcTable )
                  {
                    if ( V_stristr(pStr: v42, pSearch: "%dateclause%") == nullptr )
                    {
                      printf(
                        format: "Error: rollup query '%s' does not contain required replacement parameter '%%dateclause%%'\r\n",
                        v42);
                      return 0;
                    }
                    if ( v25->m_rollup.m_bHistogram )
                    {
                      if ( V_stristr(pStr: v42, pSearch: "%bucketstart%") == nullptr )
                      {
                        printf(
                          format: "Error: rollup query '%s' does not contain required replacement parameter '%%bucketstart%%' for histogram\r\n",
                          v42);
                        return 0;
                      }
                      if ( V_stristr(pStr: v42, pSearch: "%bucketnext%") == nullptr )
                        break;
                    }
                    szType = (const char *)CUtlString::CUtlString(this: &v82, pString: v42);
                    v43 = v25->m_rollup.m_vecQuery.m_Size;
                    v44 = v25->m_rollup.m_vecQuery.m_Memory.m_nAllocationCount;
                    v95 = 7;
                    if ( v43 + 1 > v44 )
                      CUtlMemory<CUtlString,int>::Grow(this: &v25->m_rollup.m_vecQuery.m_Memory, num: v43 - v44 + 1);
                    ++v25->m_rollup.m_vecQuery.m_Size;
                    v45 = v25->m_rollup.m_vecQuery.m_Memory.m_pMemory;
                    v46 = v25->m_rollup.m_vecQuery.m_Size - v43 - 1;
                    v25->m_rollup.m_vecQuery.m_pElements = v45;
                    if ( v46 > 0 )
                      _V_memmove(dest: &v45[v43 + 1], src: &v45[v43], count: 16 * v46);
                    v47 = v43;
                    v48 = &v25->m_rollup.m_vecQuery.m_Memory.m_pMemory[v47] == nullptr;
                    v49 = (CTable *)&v25->m_rollup.m_vecQuery.m_Memory.m_pMemory[v47];
                    v81 = v49;
                    insert.elem = v49;
                    LOBYTE(v95) = 8;
                    if ( !v48 )
                      CUtlString::CUtlString(this: (CUtlString *)v49, string: (const CUtlString *)szType);
                    v95 = -1;
                    v82.m_Storage.m_nActualLength = 0;
                    if ( v82.m_Storage.m_Memory.m_nGrowSize >= 0 )
                    {
                      if ( v82.m_Storage.m_Memory.m_pMemory != nullptr )
                      {
                        _g_pMemAlloc->Free_2(this: _g_pMemAlloc, a2: v82.m_Storage.m_Memory.m_pMemory);
                        v82.m_Storage.m_Memory.m_pMemory = nullptr;
                      }
                      v82.m_Storage.m_Memory.m_nAllocationCount = 0;
                    }
                    v50 = CFmtStrN<256>::CFmtStrN<256>(this: &v79, pszFormat: "query%d", iSrcTable);
                    v51 = KeyValues::GetString(this: pValue, keyName: v50->m_szBuf, defaultValue: str);
                    v42 = v51;
                    if ( v51 == nullptr || *v51 == 0 )
                      goto LABEL_87;
                  }
                  printf(
                    format: "Error: rollup query '%s' does not contain required replacement parameter '%%bucketnext%%' for histogram\r\n",
                    v42);
                  return 0;
                }
                else
                {
                  v69 = KeyValues::GetName(this: v40);
                  printf(format: "Error: missing required value '%s' from node '%s'\r\n", "query", v69);
                  return 0;
                }
              }
            ...
            printf(format: "Error: srctable '%s' in rollup is not a known table name in this database\r\n", v37);
            return 0;
          }
LABEL_87:
          Int = KeyValues::GetInt(this: szSchemaDir, keyName: "maxdays", defaultValue: 0);
          v25->m_iMaxDays = Int;
          if ( Int > 0 )
          {
            v53 = 0;
...
LABEL_92:
              printf(
                format: "Error: %s.%s sets a maxdays value but is missing required datetime column 'TimeSubmitted'\r\n",
                v89,
                v87);
              return 0;
            }
            v54 = 0;
            while ( _V_stricmp(s1: v25->m_vecColumn.m_Memory.m_pMemory[v54].m_szName, s2: aTimesubm) != 0 )
            {
              ++v53;
              ++v54;
              if ( v53 >= v25->m_vecColumn.m_Size )
                goto LABEL_92;
            }
          }
          v12 = database;
          FirstTrueSubKey = (KeyValues *)iTable;
        }
        szSchemaDir = KeyValues::GetNextTrueSubKey(this: szSchemaDir);
        if ( szSchemaDir == nullptr )
          break;
        v13 = (char *)szSchemaDir;
      }
    }
    FirstTrueSubKey = KeyValues::GetNextTrueSubKey(this: FirstTrueSubKey);
    iTable = (int)FirstTrueSubKey;
  ...
  v66 = FirstTrueSubKey;
LABEL_106:
  v67 = KeyValues::GetName(this: v66);
  printf(format: "Error: missing required value '%s' from node '%s'\r\n", "name", v67);
  return 0;
}
...
//------------------------------------------------------------------------------
// Address: 0x004052A0
// Name: bool GetActualRunningSchema(class IMySQL __near *,class CDBInfo __near &,class CDBInfo __near &)
// Source: json
//------------------------------------------------------------------------------
char __usercall GetActualRunningSchema@<al>(int a1@<ebx>, IMySQL *mysql, CDBInfo *dbInfoDesired, CDBInfo *dbInfoOut)
{
  IMySQL *v4; // edi
  IMySQLRowSet *v6; // esi
  const char *v7; // eax
  int v8; // eax
  const char *(__thiscall *GetColumnValue_String)(IMySQLRowSet *, int); // edx
  CDatabase *v10; // ebx
  const char *v11; // eax
  IMySQLRowSet *v12; // esi
  int m_nAllocationCount; // eax
  CUtlMemory<CTable,int> *p_m_Memory; // esi
  int m_Size; // ebx
  CTable *m_pMemory; // ecx
  int v17; // eax
  int m_szName; // eax
  ...
  const char *v33; // eax
  bool v34; // al
  int v35; // [esp-14h] [ebp-60h]
  int m_iColumn; // [esp-10h] [ebp-5Ch]
  int v37; // [esp-Ch] [ebp-58h]
  CColumnValue columnType; // [esp+Ch] [ebp-40h] BYREF
  CColumnValue columnName; // [esp+14h] [ebp-38h] BYREF
  int v41[2]; // [esp+1Ch] [ebp-30h] BYREF
  CColumnValue columnIndex; // [esp+24h] [ebp-28h] BYREF
  CColumnValue value; // [esp+2Ch] [ebp-20h] BYREF
  IMySQLRowSet *pRowSetDatabases; // [esp+34h] [ebp-18h]
  CDatabase *databaseActual; // [esp+38h] [ebp-14h]
  IMySQLRowSet *pRowSetTables; // [esp+3Ch] [ebp-10h]
  int v47; // [esp+48h] [ebp-4h]

  v4 = mysql;
  if ( ExecuteStmt(mysql, szStmt: "show databases;") != 0 )
    return 0;
  v6 = mysql->DuplicateRowSet(this: mysql);
  pRowSetDatabases = v6;
  if ( v6 == nullptr )
  {
    printf(format: "Error: failed to get database list\r\n");
    return 0;
  }
  if ( !v6->NextRow(this: v6) )
  {
LABEL_36:
    v6->Release(this: v6);
    return 1;
  ...
  while ( 1 )
  {
    v6->GetColumnValue_2(this: v6, result: &value, a3: 0);
    v7 = value.m_pSQL->GetColumnValue_String(this: value.m_pSQL, a2: value.m_iColumn);
    if ( V_stristr(pStr: v7, pSearch: "playtest") == nullptr )
      break;
LABEL_35:
    if ( !v6->NextRow(this: v6) )
      goto LABEL_36;
  }
  v8 = CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::InsertBefore(
         this: &dbInfoOut->m_vecDatabase,
         elem: dbInfoOut->m_vecDatabase.m_Size);
  GetColumnValue_String = value.m_pSQL->GetColumnValue_String;
  databaseActual = &dbInfoOut->m_vecDatabase.m_Memory.m_pMemory[v8];
  v10 = databaseActual;
  v11 = (const char *)((int (__stdcall *)(int))GetColumnValue_String)(a1: value.m_iColumn);
  V_strncpy(pDest: v10->m_szName, pSrc: v11, maxLen: 128);
  if ( !SelectDatabase(mysql: v4, szDatabase: v10->m_szName) )
    return 0;
  if ( ExecuteStmt(mysql: v4, szStmt: "show tables;") != 0 )
  {
    printf(format: "Error: show tables on '%s' failed\r\n", v10->m_szName);
    return 0;
  }
  v12 = v4->DuplicateRowSet(this: v4);
  pRowSetTables = v12;
  if ( v12 == nullptr )
  {
    printf(format: "Error: failed to get table names\r\n");
    return 0;
  }
  if ( !v12->NextRow(this: v12) )
  {
LABEL_34:
  ...
  while ( 1 )
  {
    ((void (__thiscall *)(IMySQLRowSet *, int *, _DWORD, int))v12->GetColumnValue_2)(a1: v12, a2: v41, a3: 0, a4: a1);
    m_nAllocationCount = v10->m_vecTable.m_Memory.m_nAllocationCount;
    p_m_Memory = &v10->m_vecTable.m_Memory;
    m_Size = v10->m_vecTable.m_Size;
    if ( m_Size + 1 > m_nAllocationCount )
      CUtlMemory<CTable,int>::Grow(this: p_m_Memory, num: m_Size - m_nAllocationCount + 1);
    ++p_m_Memory[1].m_pMemory;
    m_pMemory = p_m_Memory->m_pMemory;
    v17 = (int)&p_m_Memory[1].m_pMemory->m_szName[-m_Size - 1];
    p_m_Memory[1].m_nAllocationCount = (int)p_m_Memory->m_pMemory->m_szName;
    if ( v17 > 0 )
      _V_memmove(dest: &m_pMemory[m_Size + 1], src: &m_pMemory[m_Size], count: 196 * v17);
    m_szName = (int)p_m_Memory->m_pMemory[m_Size].m_szName;
    if ( m_szName != 0 )
    {
      *(_DWORD *)(m_szName + 128) = 0;
      *(_DWORD *)(m_szName + 132) = 0;
      *(_DWORD *)(m_szName + 136) = 0;
      *(_DWORD *)(m_szName + 140) = 0;
      *(_DWORD *)(m_szName + 144) = 0;
      *(_DWORD *)(m_szName + 160) = 0;
      *(_DWORD *)(m_szName + 164) = 0;
      *(_DWORD *)(m_szName + 168) = 0;
      *(_DWORD *)(m_szName + 172) = 0;
      *(_DWORD *)(m_szName + 176) = 0;
      *(_DWORD *)(m_szName + 156) = -1;
      *(_BYTE *)(m_szName + 180) = 0;
      *(_DWORD *)(m_szName + 184) = 0;
      *(_DWORD *)(m_szName + 188) = 0;
      *(_DWORD *)(m_szName + 192) = 0;
      *(_BYTE *)m_szName = 0;
      *(_DWORD *)(m_szName + 148) = -1;
      *(_DWORD *)(m_szName + 152) = 0;
    }
    v19 = (int)p_m_Memory->m_pMemory[m_Size].m_szName;
    v20 = *(int (__thiscall **)(int))(*(_DWORD *)v41[0] + 28);
    v47 = -1;
    a1 = 128;
    v37 = v41[1];
    v21 = (const char *)v20(a1: v41[0]);
    V_strncpy(pDest: (char *)v19, pSrc: v21, maxLen: v37);
    V_snprintf(pDest: q, maxLen: 0x4000, pFormat: "describe %s;", (const char *)v19);
    if ( ExecuteStmt(mysql: v4, szStmt: q) != 0 )
    {
      printf(format: "Error: failed to describe table %s.%s\r\n", databaseActual->m_szName, (const char *)v19);
      return 0;
    }
    v22 = (int)v4->DuplicateRowSet(this: v4);
    v23 = v22;
    if ( v22 == 0 )
      break;
    if ( (*(unsigned __int8 (__thiscall **)(int))(*(_DWORD *)v22 + 12))(a1: v22) != 0 )
    {
      do
      {
        v24 = *(_DWORD *)(v19 + 140);
        v25 = *(_DWORD *)(v19 + 132);
        if ( v24 + 1 > v25 )
          CUtlMemory<CColumn,int>::Grow(this: (CUtlMemory<CColumn,int> *)(v19 + 128), num: v24 - v25 + 1);
        ++*(_DWORD *)(v19 + 140);
        v26 = *(_DWORD *)(v19 + 128);
        v27 = *(_DWORD *)(v19 + 140) - v24 - 1;
        *(_DWORD *)(v19 + 144) = v26;
        if ( v27 > 0 )
          _V_memmove(dest: (void *)(258 * v24 + v26 + 258), src: (const void *)(258 * v24 + v26), count: 258 * v27);
        v28 = 258 * v24 + *(_DWORD *)(v19 + 128);
        if ( v28 != 0 )
        {
          *(_BYTE *)v28 = 0;
          *(_BYTE *)(v28 + 128) = 0;
        ...
        v29 = (char *)(258 * v24 + *(_DWORD *)(v19 + 128));
        (*(void (__thiscall **)(int, CColumnValue *, const char *))(*(_DWORD *)v23 + 20))(
          a1: v23,
          a2: &columnName,
          a3: "field");
        (*(void (__thiscall **)(int, CColumnValue *, const char *))(*(_DWORD *)v23 + 20))(
        ...
        (*(void (__thiscall **)(int, CColumnValue *, const char *))(*(_DWORD *)v23 + 20))(
          ...
          a2: &columnIndex,
          a3: "key");
        m_iColumn = columnName.m_iColumn;
        v30 = (const char *)((int (__thiscall *)(IMySQLRowSet *))columnName.m_pSQL->GetColumnValue_String)(a1: columnName.m_pSQL);
        V_strncpy(pDest: v29, pSrc: v30, maxLen: m_iColumn);
        v35 = columnType.m_iColumn;
        v31 = (const char *)((int (__thiscall *)(IMySQLRowSet *))columnType.m_pSQL->GetColumnValue_String)(a1: columnType.m_pSQL);
        V_strncpy(pDest: v29 + 128, pSrc: v31, maxLen: v35);
        v32 = (const char *)((int (__thiscall *)(IMySQLRowSet *, int, int, int))columnIndex.m_pSQL->GetColumnValue_String)(
                              a1: columnIndex.m_pSQL,
                              a2: columnIndex.m_iColumn,
                              a3: 128,
                              a4: 128);
        if ( _V_stricmp(s1: v32, s2: "MUL") != 0 )
        {
          v33 = columnIndex.m_pSQL->GetColumnValue_String(this: columnIndex.m_pSQL, a2: columnIndex.m_iColumn);
          if ( _V_stricmp(s1: v33, s2: "PRI") != 0 )
            continue;
        }
        v29[256] = 1;
      }
      while ( (*(unsigned __int8 (__thiscall **)(int))(*(_DWORD *)v23 + 12))(a1: v23) != 0 );
      v4 = mysql;
    }
    (**(void (__thiscall ***)(int))v23)(a1: v23);
    v34 = pRowSetTables->NextRow(this: pRowSetTables);
    v12 = pRowSetTables;
    if ( !v34 )
      goto LABEL_34;
    v10 = databaseActual;
  }
  printf(format: "Error: failed to get table columns\r\n");
  return 0;
}

//------------------------------------------------------------------------------
// Address: 0x00405790
// Name: public: CUtlVector<class CDatabase,class CUtlMemory<class CDatabase,int>>::~CUtlVector<class CDatabase,class CUtlMemory<class CDatabase,int>>(void)
// Source: json
//------------------------------------------------------------------------------
void __thiscall CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::~CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>(
        CUtlVector<CDatabase,CUtlMemory<CDatabase,int> > *this)
{
  bool v2; // sf
  CDatabase *m_pMemory; // eax

  CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::RemoveAll(this);
  if ( this->m_Memory.m_nGrowSize >= 0 )
  {
    if ( this->m_Memory.m_pMemory != nullptr )
    {
...
//------------------------------------------------------------------------------
// Address: 0x00405830
// Name: public: CDBInfo::CDBInfo(void)
// Source: json
//------------------------------------------------------------------------------
CDBInfo *__thiscall CDBInfo::CDBInfo(CDBInfo *this)
{
  CDBInfo *result; // eax

  result = this;
  this->m_vecDatabase.m_Memory.m_pMemory = nullptr;
  this->m_vecDatabase.m_Memory.m_nAllocationCount = 0;
  this->m_vecDatabase.m_Memory.m_nGrowSize = 0;
  this->m_vecDatabase.m_Size = 0;
  this->m_vecDatabase.m_pElements = nullptr;
  this->m_mapTable.m_Tree.m_LessFunc.m_LessFunc = nullptr;
  this->m_mapTable.m_Tree.m_Elements.m_pMemory = nullptr;
  this->m_mapTable.m_Tree.m_Elements.m_nAllocationCount = 0;
  this->m_mapTable.m_Tree.m_Elements.m_nGrowSize = 0;
  this->m_mapTable.m_Tree.m_Root = -1;
  this->m_mapTable.m_Tree.m_NumElements = 0;
  this->m_mapTable.m_Tree.m_FirstFree = -1;
  this->m_mapTable.m_Tree.m_LastAlloc.index = -1;
  this->m_mapTable.m_Tree.m_pElements = this->m_mapTable.m_Tree.m_Elements.m_pMemory;
  if ( this->m_mapTable.m_Tree.m_LessFunc.m_LessFunc == nullptr )
    this->m_mapTable.m_Tree.m_LessFunc.m_LessFunc = (bool (__cdecl *)(const char *const *, const char *const *))CDefOps<char const *>::LessFunc;
  return result;
}
...
// Source: json
//------------------------------------------------------------------------------
void __thiscall CDBInfo::~CDBInfo(CDBInfo *this)
{
  CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>(this: &this->m_mapTable.m_Tree);
  CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::~CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>(this: &this->m_vecDatabase);
}
...
int __cdecl main(int argc, const char **argv, const char **envp)
{
  ...
  int v10; // eax
  int v11; // eax
  const char *CurrentTimeString; // eax
  _iobuf *v13; // eax
  IMySQL *v14; // edi
  const char *v16; // edi
  __int64 v17; // rax
  ...
  int v24; // eax
  const char *v25; // eax
  int nFilesFound; // [esp+18h] [ebp-1D0h]
  int nFilesProcessed; // [esp+1Ch] [ebp-1CCh]
  int nFilesFailed; // [esp+20h] [ebp-1C8h]
  int nFilesSkipped; // [esp+24h] [ebp-1C4h]
  int nRowsInserted; // [esp+28h] [ebp-1C0h]
  const char *v31; // [esp+2Ch] [ebp-1BCh]
  CParseContext parseContext; // [esp+40h] [ebp-1A8h] BYREF
  CDBInfo dbInfoActual; // [esp+168h] [ebp-80h] BYREF
  CDBInfo dbInfo; // [esp+198h] [ebp-50h] BYREF
  const char *szPassword; // [esp+1C8h] [ebp-20h]
  BOOL bIsSteamBeta; // [esp+1CCh] [ebp-1Ch]
  int iElapsedSec; // [esp+1D0h] [ebp-18h] BYREF
  int *p_iElapsedSec; // [esp+1D4h] [ebp-14h]
  const char *szSchemaDir; // [esp+1D8h] [ebp-10h]
  int v40; // [esp+1E4h] [ebp-4h]
  const char *argca; // [esp+1F0h] [ebp+8h]
  ...
  v7 = (const char *)(*(int (__thiscall **)(int, const char *, const char *))(*(_DWORD *)v6 + 32))(
                       ...
                       a3: "stats");
  v8 = _CommandLine();
  szPassword = (const char *)(*(int (__thiscall **)(int, const char *, const char *))(*(_DWORD *)v8 + 32))(
                               a1: v8,
                               a2: "-password",
                               a3: "gamestats");
  v9 = _CommandLine();
  szSchemaDir = (const char *)(*(int (__thiscall **)(int, const char *, const char *))(*(_DWORD *)v9 + 32))(
                                a1: v9,
                                a2: "-schemadir",
                                a3: str);
  v10 = _CommandLine();
  v11 = (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v10 + 40))(a1: v10, a2: "-steambeta");
  LOBYTE(bIsSteamBeta) = v11 != 0;
  argca = "beta";
  if ( v11 == 0 )
    argca = "public";
  CurrentTimeString = GetCurrentTimeString();
  printf(format: "Processing for Steam %s started at %s\n", argca, CurrentTimeString);
  v13 = __iob_func();
  fflush(stream: v13 + 1);
  v14 = ConnectToMySQL(szHost: v5, szUserName: v7, szPassword);
  if ( v14 == nullptr )
  {
    printf(format: "Error: unable to connect to MySQL\n");
    return -1;
  }
  CDBInfo::CDBInfo(this: &dbInfo);
  v40 = 0;
  CDBInfo::CDBInfo(this: &dbInfoActual);
  LOBYTE(v7) = 1;
  LOBYTE(v40) = 1;
  if ( LoadSchemaFile((KeyValues *)szSchemaDir, bIsSteamBeta, dbInfoOut: &dbInfo) == 0 )
  {
    LOBYTE(v40) = 2;
    CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>(this: &dbInfoActual.m_mapTable.m_Tree);
    LOBYTE(v40) = 0;
    CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::~CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>(this: &dbInfoActual.m_vecDatabase);
    v40 = 3;
LABEL_9:
    CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>(this: &dbInfo.m_mapTable.m_Tree);
    v40 = -1;
    CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>::~CUtlVector<CDatabase,CUtlMemory<CDatabase,int>>(this: &dbInfo.m_vecDatabase);
    return -1;
  }
  if ( GetActualRunningSchema(a1: (int)v7, mysql: v14, dbInfoDesired: &dbInfo, dbInfoOut: &dbInfoActual) == 0 )
  {
    LOBYTE(v40) = 4;
    CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>::~CUtlRBTree<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short,CUtlMap<char const *,CTable *,unsigned short>::CKeyLess,CUtlMemory<UtlRBTreeNode_t<CUtlMap<char const *,CTable *,unsigned short>::Node_t,unsigned short>,unsigned short>>(this: &dbInfoActual.m_mapTable.m_Tree);
    LOBYTE(v40) = 0;
  ...
  if ( !UpdateSchemaIfNecessary(mysql: v14, dbInfoDesired: &dbInfo, &dbInfoActual) )
    goto LABEL_15;
  CParseContext::CParseContext(this: &parseContext);
  parseContext.mysql = v14;
  parseContext.pDBInfo = &dbInfo;
  p_iElapsedSec = &iElapsedSec;
  v16 = (const char *)_time64(timeptr: nullptr);
  szSchemaDir = v16;
  v31 = argv[1];
  LOBYTE(v40) = 6;
  if ( ProcessAllFiles(pParseContext: &parseContext, szDirectory: v31) == 0 )
  {
    v17 = _time64(timeptr: nullptr);
    szSchemaDir = (const char *)HIDWORD(v17);
    iElapsedSec = v17 - (_DWORD)v16;
LABEL_15:
    LOBYTE(v40) = 0;
    CDBInfo::~CDBInfo(this: &dbInfoActual);
    v40 = -1;
  ...
  LOBYTE(v40) = 1;
  v18 = _time64(timeptr: nullptr);
  v19 = v18 - (_DWORD)v16;
  szSchemaDir = (const char *)HIDWORD(v18);
  iElapsedSec = v19;
  v20 = 0.0;
  if ( v19 > 0 )
    v20 = (float)parseContext.nRowsInserted / (float)v19;
  nRowsInserted = parseContext.nRowsInserted;
  nFilesSkipped = parseContext.nFilesSkipped;
  nFilesFailed = parseContext.nFilesFailed;
  nFilesProcessed = parseContext.nFilesProcessed;
  nFilesFound = parseContext.nFilesFound;
  v21 = FormatElapsedTime(iSeconds: v19);
  printf(
    format: "\r\n"
    "Incoming data processed (%s).\r\n"
    "\r\n"
    "\t%d files found.\r\n"
    "\t\t%d files processed.\r\n"
    "\t\t%d files failed processing.\r\n"
    "\t\t%d files wrong format or invalid.\r\n"
    "\t%d rows inserted (avg %.1f TPS).\r\n"
    "\r\n",
    v21,
    nFilesFound,
    nFilesProcessed,
  ...
  Flush();
  v22 = _CommandLine();
  if ( (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v22 + 40))(a1: v22, a2: "-norollup") == 0
    && PerformRollups(pParseContext: &parseContext) == 0 )
  {
    goto LABEL_15;
  }
  v23 = _CommandLine();
  if ( (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v23 + 40))(a1: v23, a2: "-noageout") == 0 )
    AgeOutData(pParseContext: &parseContext);
  v24 = _CommandLine();
  if ( (*(int (__thiscall **)(int, const char *))(*(_DWORD *)v24 + 40))(a1: v24, a2: "-optimize") != 0 )
    OptimizeTables(pParseContext: &parseContext);
  v25 = GetCurrentTimeString();
...
TrailDown3_0:
  ...
  else
  {
    if ( ((unsigned __int8)v4 & 3) != 0 )
    {
      ...
      __asm { jmp     dword ptr ds:TrailUp0_0[ecx*4]; jumptable 0040D1A9 case 0 }
    }
    v5 = count >> 2;
    switch ( v5 )
    {
    ...
    if ( ndec > 0 )
    {
      ...
      *v13 = *_loc_update.localeinfo.locinfo->lconv->decimal_point;
      ...
      if ( v15 < 0 )
      {
        ...
        memset(dst: v13 + 1, value: 0x30u, count: ndec);
      }
    }
    if ( _loc_update.updated )
      _loc_update.ptd->_ownlocale &= ~2u;
...
LABEL_77:
              ...
              {
                v34 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_424C80);
                v34(a1: p_buffer, a2: &_loc_update);
              }
              if ( v81 == 103 && v33 == 0 )
              {
                v35 = (void (__cdecl *)(_output_l::__l2::<unnamed_type_buffer> *, _LocaleUpdate *))DecodePointer(Ptr: off_424C7C);
        ...
        unsigned int dwExceptionFlags)
{
  ...
  memset(dst: (unsigned __int8 *)&ExceptionRecord.ExceptionFlags, value: 0, count: 0x4Cu);
    ...
    else if ( rterrnum != 252 )
    {
      if ( wcscpy_s(_Dst: outmsg, _SizeInWords: 0x314u, _Src: L"Runtime Error!\n\nProgram: ") != 0
        ...
        && (v2 = wcslen(wcs: &outmsg[25]),
            wcsncpy_s(
              _Dst: (unsigned __int16 *)&lclcritsects[13].SpinCount + v2,
              _SizeInWords: 763 - (((char *)&lclcritsects[13].SpinCount + 2 * v2 - (char *)&outmsg[25]) >> 1),
              _Src: L"...",
              _Count: 3u) != 0)
...
f_incwidth2:
      ...
      if ( started != 0 )
      {
        if ( suppress == 0 )
        {
          ...
          pFloatStr[v22] = 0;
          v62 = (char)last - 1;
          v37 = (void (__cdecl *)(int, void *, char *, _LocaleUpdate *))DecodePointer(Ptr: off_424C78);
